...
for script in validate-provenance-block.sh validate-audit-anchors.sh validate-locked-anchor.sh validate-retro-evidence.sh validate-mandatory-rules.sh validate-ci-gates.sh validate-layer-entries.sh validate-compact-window.sh validate-reattach-budget.sh validate-draft-stamps.sh validate-steering-budget.sh validate-artifact-budget.sh validate-h2-attestation.sh validate-adversarial-convergence.sh validate-escalation-resolution.sh validate-gate-adjudication.sh stamp-story-provenance.sh sprint-status.sh sync-taught-schema.sh wait-for-deliverable.sh verdict.sh gen-architecture-index.js; do
...
for fixture_dir in check-1c-bypass check-15-bypass check-17-bypass check-17-counts check-3b-locked-anchor check-23-draft-stamps check-24-adversarial-convergence adversarial-citation escalation-citation check-25-steering-conduct check-h1-recursion check-manifest-bypass context-sensor layer-catalog-collision layer-readopt-gate handoff-resume-guard divergence-hard-block taught-schema gate-adjudication setup-config-drift relabel-theirs-collision known-skills-extension reconcile-blocking-list reconcile-emit-report apply-drift-refile apply-restamp-theirs core-write-guard audit-anchors-schema dispatch-model-guard subagent-probe sprint-status-lifecycle route-defect-classification story-provenance implementation-join-yield wait-stale-deliverable validate-mandatory-rules-revive; do
