...
# that would go quiet.
#
# BOTH SIDES DERIVED, and neither written here. The expression is extracted from each hook
# by its assignment shape, so this cannot pass by agreeing with a literal kept in this file.
i81_hooks="core/hooks/ai-dlc-continue.sh core/hooks/ai-dlc-acknowledge.sh"
# The extractor: the SERIES assignment whose right-hand side globs the artifact dir.
i81_expr_of() { sed -n 's/^[[:space:]]*SERIES="\$(ls -t .*adversarial.*$/&/p' "$1" | sed 's/^[[:space:]]*//'; }
i81_missing=""
...
if [ -n "$i81_missing" ]; then
  ...
  i81_na="$(printf '%s\n' "$i81_a" | grep -c .)"
  i81_nb="$(printf '%s\n' "$i81_b" | grep -c .)"
  if [ "$i81_na" -ne 1 ] || [ "$i81_nb" -ne 1 ]; then
    err "I81 extracted $i81_na live-series expression(s) from ai-dlc-continue.sh and $i81_nb from ai-dlc-acknowledge.sh; each must have exactly ONE. Zero means the grammar or the assignment moved and this invariant is scanning for something that no longer exists -- which reads exactly like agreement. More than one means the hook decides the live series in two places."
...
The PreToolUse hook is the one that DENIES a dispatch; if it resolves a different series than the Stop hook that reports the state, the pipeline reports a stall it will not enforce. Make them one expression."
    ...
    # It must be the FILTERING form. Byte-identity alone is satisfied by two copies of the
    # OLD, vulnerable expression -- so assert the property, not just the agreement.
    case "$i81_a" in
    ...
    # PROBE, written here so a run proves the arm above can FAIL. It is deliberately
    # INDEPENDENT of the shipped expression: it asserts that the extractor SEES the old
    # newest-then-strip form and that the filtering-form test REJECTS it. Comparing the
    # probe to the shipped value instead would entangle this with the arm above -- a hook
    # reverted to the old form would trip both, and two failures from one cause is how an
    # assertion becomes vacuous without anyone noticing.
    i81_probe="$(mktemp 2>/dev/null || echo /tmp/i81probe.$$)"
    printf '%s\n' 'SERIES="$(ls -t "${ART_DIR}"/*adversarial*p*.md 2>/dev/null | head -1)"' > "$i81_probe"
    i81_pexpr="$(i81_expr_of "$i81_probe")"
    rm -f "$i81_probe"
    if [ -z "$i81_pexpr" ]; then
      err "I81's own probe returned nothing: the extractor does not match the OLD assignment form, so it is not reading these lines at all and the agreement it reported above means nothing."
    else
      case "$i81_pexpr" in
        *"sed -E -n 's/(pass|p)[0-9]+\.md\$//p'"*)
