...

{
  ...
  echo "#"
  echo "# fixtures mapped: $MAPPED    entries: $TOTAL_PATHS"
  [ -n "$OMITTED" ] && echo "# OMITTED (always run): $OMITTED"
  LC_ALL=C sort "$WORK/map"
} > "$MAP"

say "wrote $MAP -- $MAPPED fixtures, $TOTAL_PATHS entries, $(LC_ALL=C cut -f2 "$WORK/map" | sort -u | wc -l | tr -d ' ') distinct paths"
