#!/usr/bin/env bash
# enforcement-map-derivations-b — shard 'b' of the enforcement-map-derivations assertion set.
#
...
# globs — so the sibling was the heaviest unit in the suite by CPU, and an 8-way pool inside
# one directory could not change that.
#
...
# the same resolution `enforcement-map-sites-b` uses for its own subject fixture, and the
# one I33 permits.
#
# Usage: run.sh
...
IMPL="$HERE/../enforcement-map-derivations/run.sh"
[ -f "$IMPL" ] || {
  echo "FIXTURE ERROR: sibling enforcement-map-derivations/run.sh not found — shard 'b' has no assertions to run, and a shard that runs nothing passes everything it never checked" >&2
  exit 2
}
