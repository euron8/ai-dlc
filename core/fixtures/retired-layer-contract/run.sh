...
if printf '%s\n' "$PONLY" | grep -qF 'path:core/skills/ai-dlc/steps/route.md'; then
...
if rlcmut m2-no-subtraction -e 's@^  RETIRED_PATHS="\$(comm -23 <(printf .%s\\n. "\$RB_BASE") <(printf .%s\\n. "\$RB_THEIRS"))"$@  RETIRED_PATHS="$RB_BASE"@'; then
  ...
  if printf '%s\n' "$M2_OUT" | awk -F'\t' '$2 ~ /path-survivor\.md/ && $3 ~ /^path:/' | grep -q .; then
...
if rlcmut m3-regex-not-literal -e 's@        grep -qF -- "\$_sp" <<< "\$body" || continue@        grep -qE -- "$_sp" <<< "$body" || continue@'; then
  ...
  if printf '%s\n' "$M3_OUT" | awk -F'\t' '$2 ~ /path-dot-near-miss\.md/ && $3 ~ /^path:/' | grep -q .; then
  ...
  if printf '%s\n' "$M3_OUT" | awk -F'\t' '$2 ~ /path-entry-spelling\.md/ && $3 ~ /^path:/' | grep -q .; then
...
if rlcmut m5-bare-filename-spelling \
  ...
  if printf '%s\n' "$M5_OUT" | awk -F'\t' '$2 ~ /path-bare-filename\.md/ && $3 ~ /^path:/' | grep -q .; then
  ...
  if printf '%s\n' "$M5_OUT" | awk -F'\t' '$2 ~ /path-root-level-true\.md/ && $3 ~ /^path:/' | grep -q .; then
