...
| E2 | `register-drift.sh` REFUSES (rc 2, consumer file cmp-identical, no override) rather than revert an edit it did not carry: (a) `diff` failing for one of two edited sections, (b) the same as a dry run, (c) the override path a directory, (d) one section's body extraction failing (`mktemp` shim, call index derived by a counting run); (e) the unshimmed seed registers BOTH edits, and (a') the keyed shim over an unkeyed seed does not fire | BL-293: a diff that did not run read as "template substitution only", the section was skipped, and the revert destroyed it with rc 0 and nothing on stderr |
...
| E2 M1: layer 1 reverted (`substitution_only` two-valued, the caller refusal gone), the conservation check LEFT IN | E2 (a) and (b) only — the conservation check re-asks the same classifier, gets the same wrong `yes`, and passes the loss |
| E2 M2: the conservation check's refusal a no-op | E2 (d) only |
| E2 M3: the not-a-regular-file guard removed | E2 (c) only — `mv` onto a directory succeeds by moving the file inside it |
