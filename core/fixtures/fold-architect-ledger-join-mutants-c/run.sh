#!/usr/bin/env bash
# fold-architect-ledger-join-mutants-c — shard 'c' of the fold-architect-ledger-join mutation
# battery.
#
...
[ -f "$IMPL" ] || {
  echo "FIXTURE ERROR: sibling fold-architect-ledger-join-mutants/run.sh not found — shard 'c' has no mutants to run, and a shard that runs nothing passes everything it never checked" >&2
  exit 2
}
