#!/usr/bin/env bash
# enforcement-map-sites-c — shard 'c' of the enforcement-map-sites assertion set.
#
...
[ -f "$IMPL" ] || {
  echo "FIXTURE ERROR: sibling enforcement-map-sites/run.sh not found — shard 'c' has no assertions to run, and a shard that runs nothing passes everything it never checked" >&2
  exit 2
}
