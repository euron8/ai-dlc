...
if [ "$FINISH" = 0 ]; then
  if [ -f "$UNION_REPORT" ]; then
    ...
    if [ "$_ug_rc" -eq 0 ]; then
      ...
      _ug_cause="$(printf '%s\n' "$_ug_verr" | sed -n 's/^  cause: //p' | head -1)"
      if [ -n "$_ug_at" ]; then
        ...
        err "the report at ${UNION_REPORT#"${CONSUMER}"/} does not match what the detectors render at this run's theirs ($THEIRS) — and the only difference in the blocking list is that the approved region lists HARD-* row(s) the detectors no longer render, with upstream unchanged. ${_ug_cause:-cause: BLOCKERS-RESOLVED} Those blockers were resolved after the report was rendered — step 7's own work (a --stamp readopt, a register row), not a finding hidden from the approval — but the operator approved a region that no longer describes this tree, and a resolution can leave rows they have not seen. One step closes it: re-render the region with emit-report.sh $DIST $BASE $CONSUMER $THEIRS from the tree as it now stands, re-approve it, then re-run apply with the same four arguments. NOTHING HAS BEEN WRITTEN."
      else
        err "the report at ${UNION_REPORT#"${CONSUMER}"/} does not match what the detectors render at this run's theirs ($THEIRS). SKILL.md step 7 lets apply write only after emit-report.sh --verify exits 0, and it does not. ${_ug_cause:+$_ug_cause }Either upstream moved after that report was rendered — which is what this gate exists to catch — or the region was hand-edited or a detector now renders a finding the approval never saw, or an earlier apply of this same range already moved this tree and its stamp is still withheld (a re-run after a withheld run). Re-run the dry run at $THEIRS, or name the ref the report describes explicitly so the two agree, then re-approve. NOTHING HAS BEEN WRITTEN."
      fi
    fi
  else
