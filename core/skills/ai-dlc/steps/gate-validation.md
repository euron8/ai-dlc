...
1. **Stop all in-flight teammates first.** Call `TaskStop` on
   every task in `in_progress` status and halt any Agent-spawned
   teammate not bound to a task. Wait until every teammate has
   returned before proceeding. Record stopped teammates and any
   in-flight artifacts in the snapshot's Open Items section in
   Step 3. Skipping this step lets teammates commit work after the
   snapshot is finalized, stranding it outside the resume contract.
2. `git add` and `git commit` any in-flight work with a descriptive
   message, including work teammates left in the working tree
   before Step 1 stopped them.
3. Finalize the pipeline snapshot — one last update capturing
   in-flight state, recent decisions, current sub-step within the
   active step file, and the stopped-teammate record from Step 1.
4. Output the distinguishing auto-handoff line (substitute the
   ...
   template) pointing at the snapshot. The resume prompt MUST be
   wrapped in `----` delimiter lines (one before, one after) per
   SKILL.md Handoff Protocol so the user can identify exactly what
   to copy/paste:
