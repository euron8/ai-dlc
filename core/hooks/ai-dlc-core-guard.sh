...
route_and_deny() {
  ...
  case "$REL" in
    ...
    scripts/ai-dlc/*)
      ...
      ;;
    .claude/skills/ai-dlc/templates/*)
      # THE SECOND SHARED DIRECTORY. `templates/*.md` claims every .md a consumer puts
      # here, so a consumer-authored template is [core]-owned by a GLOB while upstream has
      # never held the file. Without this branch the generic arm below sends the author to
      # overrides/ or extensions/, and neither has any grain for a template — a template is
      # an artifact SHAPE, not a rule to shadow or a check to add. Worse, retro.md's
      # disposition for a [core] finding is "file a push candidate", which for a file
      # upstream does not have is unactionable in both directions while the consumer's
      # pre-push blocks on the tier-1 finding. Measured on the reference consumer: one such
      # file, authored there at 37b3d15c3, never upstream at any commit (control: a sibling
      # core template resolves 1).
      reason="${REL} is an upstream-owned CORE template (Rule 27 core-manifest, glob \`templates/*.md\`). A template is an artifact SHAPE, not rulebook and not machinery — there is NO consumer layer for one: no overrides/ entry shadows a template and no extensions/ entry adds one. Do NOT edit it in place; the next /ai-dlc-update overwrites core and clobbers the change. If the SHAPE is wrong, that is an upstream change — take it upstream and pull it with /ai-dlc-update. If this is YOUR template rather than ours, it is in the wrong directory: \`templates/*.md\` is core-owned as a glob, so this deny stands whether or not the distribution ships a file by that name — and a consumer-authored file here is worse than clobbered, because the rule-file audit will report it as a [core] finding whose only sanctioned disposition is a push candidate against a file upstream does not have. Consumer-authored templates go in \`.claude/skills/ai-dlc/templates-local/\` (\`templates_local_home:\` in core-manifest.md), which core never reads, never writes and never overwrites."
      ctx="AI/DLC core-layer guard: .claude/skills/ai-dlc/templates/*.md is core-owned as a GLOB, so the deny stands whether or not upstream ships this name. A template has no overrides/ or extensions/ grain — it is an artifact shape, so the generic layer-routing advice does not apply. A consumer-authored file here also becomes an unfixable [core] rule-file finding: retro.md's remedy is a push candidate, and there is nothing upstream to file it against. Consumer templates belong in .claude/skills/ai-dlc/templates-local/."
      ;;
    tests/fixtures/*)
