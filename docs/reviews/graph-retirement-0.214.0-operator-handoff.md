...
| **graph freeze** | **LIFTED 2026-07-30** at the row-7 merge, `39f0248ff` on `main`. Sprint work in graph may resume. (Was: IN FORCE until row 7 merged and said the freeze was over.) |
...
> **Row 5 correction — the predicted extra wrong figure, found.** The finding reproduces exactly:
> `architecture-history.md` = **0**, at both 914 and 782 lines. **Its control does not.**
> `architecture.md` appears **2** times, not once — at `18e00ef40` (914 lines) and unchanged after
> row 2 (782 lines), at `:97` (the home list) and `:324` (an unrelated HPE-rule citation). The
> figure **1** is `docs/architecture.md`, with the `docs/` prefix, reported under the bare token.
> A control that undercounts by half still discriminated here — 0 against 2 — so the conclusion
> stands, but it is a slip of the class §3 says to report rather than adjust to. Not a core defect,
> so no ai-dlc session: the brief's own grep pattern, not graph's tree.
...
| # | Row | Repo | Status |
|---|---|---|---|
| 1 | Pre-flight and baseline. Confirm the three absorbers, record the four baseline tallies. **Write nothing.** | graph | ✅ 2026-07-30 @ `18e00ef40`. Tree 4 entries, all `_bmad-output/`. Shim present 314 bytes; `core.hooksPath` unset (rc=1, no output). Absorbers all 3 pairs identical: `89ff1b7fc261` / `a1ec071b3ca7` / `392a592c5a53`. Baselines — (i) layer-entries rc=**0**, `0 error(s), 5 warning(s)`, **5** OUT OF BAND (checks 33/34/35 + rules 31/32); (ii) gate-manifest rc=**1**, `UNLOADABLE: 19b 2s 33 35`, `MISSING none`, `ORPHAN none`, `manifest source: core`, `extension gate_types: 34->implementation`, `manifest ids: 42   anchors: 42`; (iii) drivability rc=**0**, `undeclared : 0`, **114** fixture dirs; (iv) `--strays` rc=**0**, PASS, 894 files carried the envelope. Controls: bogus token 0 in both validator logs, universal token 1. All four match §6 row 1 verbatim; **no deviation** |
| 2 | Excise checks 34/33/35 **and** the `gate_types:` frontmatter line — §6.2 | graph | ✅ branch `chore/retire-duplicate-enforcement-0.214.0`. **132 lines removed** from `gate-validation-domain.md` (914 → 782): block 784–914 = 131 + the `gate_types: implementation` frontmatter line = 1. Boundary pre-verified: 783 = check-32 sentence, 784 blank, 785 = `## Check 34`; anchor `CHECK_LOADED: 34` at 788 was the tree's only one (control: other 3 files in `extensions/checks/` = 0, universal token 10/2/60/1). Post-state — gate-manifest rc=**1** (not the wedge 2), `UNLOADABLE: 19b 2s`, `MISSING none`, `ORPHAN none`, `manifest source: core`, `extension gate_types: none`; layer-entries rc=**0**, `0 error(s), 2 warning(s)`, both survivors `Rule 31`/`Rule 32`. **New datum, no stated expectation:** `manifest ids: 42 anchors: 42` → **`41 / 41`**, still balanced — the retired `34->implementation` mapping was one claimed id and its one anchor |
| 3 | Subject 1: delete `check-protected-core-paths.sh`, repoint the 3 role lines — §6.3 | graph | ✅ `git rm scripts/check-protected-core-paths.sh` (**162** lines). 3 role lines repointed to `scripts/ai-dlc/core-paths.sh --audit-diff <base-ref> HEAD`, each read before rewriting and each keeping its own REJECT/STOP voice: `code-reviewer-domain.md:41`, `dev-domain.md:15`, `qa-domain.md:17`. Exit contract carried into all three verbatim — 0 / **1** / **2 fail-closed, NOT a pass** — plus **`DORMANT` is not a pass** (graph IS layered, so it means the base ref is wrong) and **`PASS (with citation)` reports only that a citation exists, never that it covers these touches** (`pending.md` = **8** `Operator authorization:` lines at HEAD, re-measured). Controls — old token in live paths: **0 lines, rc=1**; same-shape probe `core-paths.sh`: **17 files, rc=0**, the 3 role files among them. Mode proven non-vacuous: `--audit-diff 170ff9d8c HEAD` (3 commits) exits **0** with `PASS: core path(s) touched, every touching commit is an /ai-dlc-update reconcile` naming the paths; `--audit-diff ZZZ_NOT_A_REF HEAD` exits **2** `ref not resolvable`. (`--audit-diff main HEAD` is `N/A: no diff in range` — HEAD is still at `main`, edits uncommitted; that run proves nothing and is not the evidence) |
| 4 | Subject 2: delete `check-mutation-red-anchor.sh`, its test, and its fixture — §6.4 | graph | ✅ All 3 line counts reproduced before deleting: detector **72**, test **87**, fixture **62** (README+run.sh+seed.sh). Core replacement `tests/fixtures/mutation-red-replay/` confirmed present with `run.sh` + `seed.sh`. **Fixture dirs 114 → 113, `undeclared : 0`, rc=0** (disk control: 113). Controls — `check-mutation-red-anchor` in live paths: **0 lines, rc=1**; `validate-mutation-red`: **6 files, rc=0** (`team-roles/code-reviewer.md`, `dev.md`, `qa.md`, the core script, and both `mutation-red-replay/` files). Neither `ci-local.sh` nor `.githooks/pre-push` named the deleted test or fixture (rc=1). **§4 item 9 reproduces on the natural reading:** `grep -c 'scripts/tests/test-'` in `ci-local.sh` = **39** lines — note that is 39 occurrences over **20 unique paths** (63 such files on disk); pre-push `bash scripts/` = **8** |
| ⏹ | **FRESH GRAPH SESSION.** Report the row-1 baselines against the row-2/3/4 post-state. | graph | ✅ **All four deltas match exactly.** `OUT OF BAND` **5 → 2** (survivors `Rule 31`/`Rule 32`); `UNLOADABLE` **4 → 2** (`19b 2s`); fixture dirs **114 → 113**, `undeclared : 0`; `--strays` **exit 0, PASS**, still 894 files carrying the envelope. The unchanged 894 is not a stalled scan: none of the deleted files carried a provenance token (0 each at `main`; control — `validate-provenance-block.sh` carries 22). **No deviation, no stop condition; no ai-dlc session needed.** Rows 2–4 committed as `0acaac210` on branch `chore/retire-duplicate-enforcement-0.214.0`; pre-commit (gitleaks) passed. Working tree back to the row-1 baseline: the same 4 `_bmad-output/` entries, nothing else |
| 5 | Subject 3: excise the stray arm, repoint Check 17 + `pr-class.sh` + 3 tests — §6.5 | graph | ✅ `a697174dc`. **Script 155 → 83: −101 / +29.** Report the removal as **101**, not row 3's 110 and not §6.5's 66 — the 66-line body (90–155) plus 35 header/usage lines that documented ONLY the retired mode; the 29 added are the retirement header and the fail-closed arm. **Bare/explicit invocation now exits 2, not 0** — a missed caller fails closed instead of reporting a silent PASS for a scan that no longer runs. Repointed **11 live sites across 8 files** + 5 fixture prose files; `pr-class.sh:89` **KEPT** (pipeline-infra path allowlist, not an enforcement site; the script still exists). §5e flip: `--strays` rc=**0** PASS, `AI_DLC_KNOWN_SKILLS_EXT=""` control rc=**1**, **exactly 5** findings, **0** outside `scripts/tests/**`. Tests: legit-homes **5/0** (RED under both mutants, **4/1** each, `cmp -s` guarded); pr-class **16/0**; s241-5-ac4 ALL PASS (surviving arm untouched); s239-1-hardening **10/5 = its exact HEAD tally** (2 stray calls repointed, its unrelated `rc=127` staleness left). Four row-1 baselines unmoved: `OUT OF BAND` **2**, `UNLOADABLE 19b 2s`, `manifest ids 41 anchors 41`, **113** fixture dirs / `undeclared : 0` (disk control 113). shellcheck: no file gained a finding, this one lost one. **§6.5 was silent on three couplings, all measured with controls — see the ⏹ row below** |
| ⏹ | **FRESH GRAPH SESSION.** Row 5 is per-item judgement and does not share a context with row 6. | graph | **Row 5 findings the next session must carry. (a) `--strays` does NOT normalize an absolute path** — it matches the homes on the repo-relative path, so an absolute one makes every home miss and reads a legitimate block as a stray. Core's header states it; the retired consumer arm forgave it via `${f#$ROOT/}`. **Both repointed tests passed `"$ROOT/…"`**; every explicit path is now repo-relative. **(b) `scripts/tests/**` is NOT one of core's five schema homes** — this repo contributes it via `extensions/known-skills.json` `party_mode_homes`, which is *why* `AI_DLC_KNOWN_SKILLS_EXT=""` is the vacuity control. §6.5's "identical to the consumer's six" is right about the RESOLVED set (6) but not about the FAIL message, which printed **5** in the control run precisely because the control drops the extension. **(c) A bare `:163` repoint would have WEDGED, two ways.** `guarded-merge.sh:256` word-splits `EXPECTED_VALIDATORS`, so `--strays` cannot live in `pr-class.sh` — it is selected from `PR_CLASS` instead; without that, `run_validator` validates `docs/retro/sprint-${SPRINT_N}.md`, and `SPRINT_N` is **empty** on a non-sprint branch (control: `docs/retro/sprint-.md` absent, `sprint-249.md` present) → rc **9** → infra-DENY of every `provenance-in-non-retro` merge. And `audit-main-since.sh:170/265` gated on the validator's NAME, discriminating only by accident since S249; core's name on both classes silently REACTIVATES its per-block arm — the exact enforcement the S249 carve-out prevents. It failed C1b/C2 on first run (**13/2**) because the ephemeral fixture repo stages **no** schema (control: `grep -n 'schemas\|known-skills'` rc=**1**, 0 lines). Both gates now test `PR_CLASS`; behaviour identical to HEAD, dead arm surfaced not revived → **16/0**. **Row 6 must file (a)–(c), the dead audit arm, and:** `test-pr-class-provenance-in-non-retro.sh` **A6/A7 sit after `exit 0` at :363 and have never run**; this branch derives `PR_CLASS=fix-forward` with `EXPECTED_VALIDATORS=''` — **no validator gates its own merge** — because `scripts/check-{protected-core-paths,mutation-red-anchor}.sh`, deleted in rows 3–4, match no `_PIPELINE_INFRA_RES` entry (and `^scripts/ai-dlc/` is in that list nowhere). **`--strays` envelope count 894 → 893:** fully attributed, not a stalled scan — a per-file diff over every touched file shows `scan-stray-provenance.sh` is the ONLY one whose marker count moved (**2 → 0**). **No stop condition fired; no ai-dlc session needed** |
| 6 | File the 3 REFUTED subjects as ledger entries with falsifiable receipts — §6.6 | graph | ✅ **12 entries filed, `ledger-reverify.sh` rows 53 → 66.** The pre-filing count reproduced §6.6's `[V — 53 rows today]` exactly. 13 new receipt rows (12 entries; `TRUNK-PUSH` carries 2), **all STILL-LIVE**; the pre-existing status mix is unmoved (3 ENTRY-SWALLOWED / 12 HAND-REVIEW / 2 NAMED-UPSTREAM / 3 NEEDS-REVIEW), so nothing filed here landed as a malformed or vacuous receipt. Ids, all `PC-S312-`: **REFUTED (3)** `RETRO-REPLAY-HARNESS-NOT-ABSORBED-BY-DRIVABILITY` (`theirs_lacks … "defect-manifest"` — 0 under `core/` at BOTH refs, control `fixture` = 183 files), `SPRINT-STATUS-CHECK-STORIES-COVERS-ONE-FIELD-OF-FIVE` (`theirs_lacks … "gate_1_model"` — core's 724-line `sprint-status.sh` carries `priority` 0 / `gate_1_model` 0, control `status` = 87), `TRUNK-PUSH-DECLINES-TO-POLICE-THE-TRUNK` (2 receipts: `theirs_has` on core's disclaimer + an `sh` receipt on the consumer's dormant `.pre-commit-config.yaml:28-30` wiring). **RETIREMENT PROBES (3)** `PROTECTED-CORE-PATHS-STAYS-RETIRED`, `MUTATION-RED-ANCHOR-STAYS-RETIRED`, `STRAY-SCAN-ARM-STAYS-RETIRED` — each `verify: sh`, each carrying its control in the same invocation, **each mutation-tested in both directions** (retirement holds → rc=0; duplicate-returned or control-gone-vacuous → rc=1). **FINDINGS (6)** `STRAYS-DOES-NOT-NORMALIZE-AN-ABSOLUTE-PATH`, `S239-1-HARDENING-CALLS-PRE-RELOCATION-PATHS`, `FIXTURE-PROVENANCE-ARM-HAS-NO-LIVE-DRIVER`, `EXPECTED-VALIDATORS-WORD-SPLIT-EXCLUDES-FLAGS`, `PR-CLASS-TEST-A6-A7-ARE-UNREACHABLE`, `FIX-FORWARD-CLASS-GATES-ON-NO-VALIDATOR`. Every entry states that **exit 0 = the finding/retirement still reproduces (STILL-LIVE, silent)**, so a `CLOSE-CANDIDATE` row against a `PC-S312-` id is a regression, not an absorption. **FIVE DEVIATIONS, reported not adjusted to, none a core defect → no ai-dlc session.** (1) §6.6c quotes core as *"It bounds that commit…"*; `core/scripts/validate-audit-anchors.sh:31` actually reads **"It bounds the licensed commit; it does not police the trunk."** (2) §6.6b states a 43-file wiring surface for `generate-sprint-status.py`; measured **247** unscoped / **82** excluding `_bmad-output` / **44** excluding `_bmad-output`+`docs` — **none is 43**, and the 44 is identical at `main` and `18e00ef40`, so not a branch artefact (control: bogus token 0 files, rc=1). (3) Row 5's ⏹ puts the dead `exit 0` at `:363`; the terminating one is **`:377`** (A6 at 379, A7 at 395) and the `^exit 0` at `:232` is **not** dead — it is inside the `STUB_EOF` heredoc. (4) Row 5's finding (a) says core's *header* states the repo-relative rule; it is stated at **`validate-provenance-block.sh:175-176`**, in the comment on the DEFAULT whole-tree branch, and the **explicit-path branch at :181-183 says nothing about absolute paths** — the branch every path-passing caller takes. The reproduction also had to move subjects: `scripts/tests/test-pr-class-provenance-in-non-retro.sh` PASSes **both** ways and is vacuous (its marker is a split literal, so no block parses); the decisive pair is `docs/retro/sprint-249.md` — **relative rc=0 PASS, absolute rc=1 FAIL**, same file, same validator. (5) §6.6d's *"5 of 15 assertions fail `rc=127`"* — 10 passed / 5 failed is exact and all five are the same missing-file class, but only **3** print the literal `rc=127`; a `grep -c 127` on the output returns 3 |
| 7 | Verify, commit, push, PR, merge. **Report the net tally. Then UNFREEZE** — §6.7 | graph | ✅ **MERGED `39f0248ff` on `main` (PR #833, squash, 2026-07-30T22:42:50Z, operator-approved). THE GRAPH FREEZE IS LIFTED — sprint work in graph may resume; §1's freeze row is updated to say so.** `main` == `origin/main` == `39f0248ff`; working tree back to the row-1 baseline (the same 4 pre-existing `_bmad-output/` entries, nothing else). Merge verified **by content, not by `--is-ancestor`** (a squash merge makes that false): all three deleted paths ABSENT at `main`, control `scan-stray-provenance.sh` PRESENT. **POST-MERGE TALLIES, re-measured at `39f0248ff` with controls:** `W5` / `OUT OF BAND` = **2**, rc=0, `0 error(s), 2 warning(s)`, the two survivors being `Rule 31` and `Rule 32` and **no check id at all** (control: bogus token 0 in the same log); `UNLOADABLE` = **2** (`19b 2s`), rc=1, `MISSING none`, `ORPHAN none`, `manifest source: core`, `manifest ids: 41 anchors: 41`, `extension gate_types: none`; fixture dirs **113**, `undeclared : 0`, rc=0 (disk control 113); `--strays` rc=**0** PASS at **893** files with the `AI_DLC_KNOWN_SKILLS_EXT=""` control still rc=**1** and **exactly 5** findings, **5 of 5** under `scripts/tests/**`. **All three retirement probes re-run at merged `main` exit 0** (retirement holds); `ledger-reverify.sh` = **66 rows**, **0** `PC-S312-` rows anything other than STILL-LIVE. **The four §6.7 exit criteria, as measurements:** (1) duplicate enforcement paths **0** for subjects 1, 2 and subject 3's stray arm, by live-path grep with its control, never by citation; (2) net surface **DOWN** — `23 files changed, 599 insertions(+), 679 deletions(-)`, −80 overall and **−469** excluding the ledger filing; (3) **`W5` 5 → 2** and **`UNLOADABLE` 4 → 2**, the survivors dispositioned in writing in the PR body (rules 31/32 want a crosswalk row; `19b`/`2s` want an anchor plus `gate_types:`, which is NOT a precedent read from row 2); (4) each retirement falsifiable through a `ledger-reverify.sh` receipt, all three mutation-tested in both directions. **No `ecs-deploy.sh` owed: 0 files under `server/`, `web/`, `rebalancer/`, `graph-node-src/`.** Full record, including the SIX deviations reported rather than adjusted to, is in PR #833's body. Prior state of this cell: 7a-7e complete, 7f awaiting approval; row 6 committed as `24f7aa9ff`; branch pushed, `pre-push: all gates green` (103 fixtures ok, **0** FAIL), verified by `git ls-remote` = `24f7aa9ff…` on `refs/heads/chore/retire-duplicate-enforcement-0.214.0`, never by a piped exit code. **PR: https://github.com/euron8/fee_accrual_graph/pull/833**, body carries row 1's four baselines, 7a's four deltas, the net tally, the 2/1/3 verdicts and the 12 row-6 ids. **7a, all four with controls:** `OUT OF BAND` **5 → 2** (rules 31/32 only, rc=0, `0 error(s), 2 warning(s)`; control: bogus token 0 in the same log); `UNLOADABLE` **4 → 2** (`19b 2s`, rc=**1** not the wedge 2, `MISSING none`, `ORPHAN none`, `manifest source: core`, `extension gate_types: none`, `manifest ids: 41 anchors: 41`); fixture dirs **114 → 113**, `undeclared : 0`, rc=0 (disk control 113); `--strays` rc=**0** PASS, **893** files carried the envelope, and the `AI_DLC_KNOWN_SKILLS_EXT=""` control still exits **1** with **exactly 5** findings, **all** under `scripts/tests/**`, **0** outside. **7b net tally, reported not targeted: `23 files changed, 599 insertions(+), 679 deletions(-)` = net −80; excluding the +389 ledger filing the code/rulebook change is +210 / −679 = −469.** Every per-item removal matches the brief's order of magnitude: domain file −142/+28, subject 1 −162, subject 2 −72/−87/−62, stray arm −101/+29. **7d pre-existing red, both confirmed to predate the branch:** `server-fixture-manifest` 14 diff lines rc=1 — the branch changes **0** files under `server/` (control: 10 lines under `scripts/`); shellcheck red on `main` — **DEVIATION 6**, §7d's `118 findings / 45 files / 3 error-level` does **not** reproduce, `main` measures **122 / 49 / 3** under ci-local's exact invocation with shellcheck 0.11.0, branch HEAD **120 / 47 / 3**, per-file diff losing exactly two entries (`check-mutation-red-anchor.sh` 1, `scan-stray-provenance.sh` 1) and **no file gaining one**. Same non-reproducing class as the 0.213.0 handoff's own shellcheck figure, so it is not carried forward. **REMAINING (7f): the merge is a human act — CLAUDE.md Deployment Rule (b) puts merge after operator approval, and this brief cannot self-authorize one.** After approval: `gh pr merge --squash`, then tick this row with the merge sha, the `+/−` tally and the post-merge `W5`/`UNLOADABLE` counts, **and record the UNFREEZE explicitly**. **No `scripts/ecs-deploy.sh` run is owed** — the branch touches only pipeline machinery, rulebook extensions, tests and fixtures; **0** files under `server/`, `web/`, `rebalancer/` or `graph-node-src/` |
