...
**`VERSION` IS `0.542.0`, SO BATCH 82 RELEASES AS `0.543.0`.** Re-derive `VERSION` and add one.
Batch 81 merged once, at `732db5a5`, carrying
`PC-S309-STORY-NO-SPRINT-REMEDY-IS-RENAME-ONLY-WHILE-THE-FIXTURE-RATIONALE-CLAIMS-OUT-OF-BAND-DERIVATION`
and `PC-S309-VALIDATE-MANDATORY-RULES-CHECK5-TEST-ONLY-WEB-DIFF-FALSE-FAIL`, both cited verbatim in
the release commit message and each appearing in EXACTLY ONE commit on the range. They batched
because the range is seven core paths with no bootstrapping file among them and neither receipt is
closed by the other's fix — measured in BOTH directions with a control firing in each invocation.

**THE SWEEP AT THE OPEN MATCHED THE BLOCK EXACTLY AND THEN MOVED TWICE.** Open: live 72, cited 53,
unfiled 19, md5 `76f34a6b…`, archive 175, all six controls fired. Of the 19 unfiled, 4 were already
named in `origin/main` release commits, 11 carry `NOT-UPSTREAM` in the adjudication brief, 1 is
refuted, 1 WITHDRAWN by the consumer itself and 1 superseded — leaving the 2026-09-09 filing as the
only available residue. **Close: live 75, cited 55, unfiled 20, md5 `21cbc866…`, archive 175
UNMOVED.** The consumer filed THREE new candidates mid-batch — `ADR-DEFERRED-WORK-HAS-NO-CARRIER`,
`RETRO-CLOSURE-QUOTATION-NOT-VERBATIM-BOUND`, `VALIDATE-MANDATORY-RULES-CHECK5-…` — while its own
commit subject claimed two. **Derive the id set, never the commit message.** DISCHARGED 25 /
in-flight 30 / untouched 20, summing to 75 after subtracting the 4-id overlap; TERMINAL 61;
discharged-but-invisible 1 (`BL-195`, deliberately uncited).

**WHAT SHIPPED, AND BOTH FILED PREMISES WERE REFUTED BY MEASUREMENT.** The `STORY-NO-SPRINT` refusal
claimed a RENAME was its only remedy; the consumer's operator resolved all 23 such files with PURE
DIRECTORY MOVES — 0 insertions, 0 deletions, basename identical — against controls of 36 rows
carrying insertions and 72 renaming a basename in the same parse. Recovery now reads the file's own
`**Sprint:** <N>` header then the basename, reports every one, and scores 19 correct / 0 wrong / 4
refused of 23; consumer-wide 970 moves against 951, 79 refusals against 98, 0 newly refused. **The
`--follow` premise in the brief was also wrong**: at the PRE-migration ref `--follow` and plain
`git log` return the same oldest commit for all 23, because the renames that make them differ are
the ones the migration itself creates. The fix reads no history at all.

**THE ADVERSARY'S BLOCKER MADE THE FIX WORSE THAN THE STATE IT REPLACED.** The first cut's header
grammar took only a bare digit while the consumer spells 46 headers `**Sprint:** S303`. On those the
header read returned empty and FELL THROUGH TO THE BASENAME, placing a file whose header says sprint
270 into `s124/` — its carry-over ITEM number, which is the exact defect the fix exists to prevent,
and worse than the refusal it replaced. The channel had no **"spoke but unparseable"** state. Fixed
both halves: the grammar takes `[Ss]?[0-9]+`, and a `**Sprint:**` line present but unreadable
(`53-54`, `TBD`, the `[sprint ID/name]` placeholder, `0`) REFUSES. **ASK OF ANY PARTIAL READER WHAT
IT DOES WHEN ITS SUBJECT SPOKE AND IT COULD NOT LISTEN.**

**TWO OF SIX NEW ARMS WERE VACUOUS AND ONE ADVERSARY PROVED IT BY BUILDING THE MUTANT THEY CLAIMED
TO KILL.** The `follow-bound` seed sat in an `s121/` slot, so driving the shipping `legacy_story()`
returned `not-leg` against a `LEGACY` control — it never entered the recovery block. A real
`git log --follow` third channel added to the shipping script PASSED the arm. The subject moved to a
legacy path born on another slot and moved later, plus a new `follow-bound-reaches` arm that drives
`legacy_story()` to prove the subject reaches the mechanism. **AN ARM'S CONTROL MUST ESTABLISH THAT
ITS SUBJECT REACHES THE CODE, NOT MERELY THAT THE TREE HAS A PROPERTY.**

**CHECK 5 TOOK THREE ADVERSARY FINDINGS AFTER ITS GATE WAS GREEN.** A `| head -20` truncation
acquitted a 22-file diff whose only source file sorted past position 20; the bare `web/**` glob was
pinned by NO fixture world, so a narrowed pathspec passed all 19 assertions while `web/index.html`
skipped as "no web changes"; and default rename detection collapsed
`git mv Gauge.jsx Gauge.test.jsx` to one test path, acquitting a diff that DELETED a rendered
component. A fourth finding was a VACUOUS GUARD the commit message presented as a fix — removed by
subtraction. **The `.stories.` widening is caught by a SET-LOCK, not another world**: no battery of
worlds can catch a fifth suffix.

**BOTH BUILD HANDS SHIPPED A STALE DIFF AND ONE PUSHED WITH `--no-verify`.** Each worktree was cut
from `aca1f5e3`, so a straight merge would have reverted the other subject entirely plus three
backlog filings. **CHERRY-PICK A WORKTREE HAND'S COMMIT; NEVER MERGE ITS BRANCH.** One hand pushed
its own branch to origin on the strength of a gate run BEFORE the commit — `origin/main` was
untouched and nothing reached the release, but a gate that ran before the tree froze is not a gate
on what was pushed.

**THREE ENTRIES FILED AND DELIBERATELY NOT FIXED**, live 79 → 82 against a ceiling of 100.
**`BL-214`** — `story_normalize` reads a carry-over ITEM number as a sprint: 165 of 1045 story files
disagree with their own headers, ALL ONE DIRECTION, mass at gaps 44–50, and the licence at
`artifact-path-grammar.md:191` rests on a control that CANNOT DISCRIMINATE (an item number falls
inside the sprint range too). **`BL-215`** — the ADR-carrier filing's headline is refuted
(`requirements.md:50-56` mandates the `docs/adr/` grep one step later); no enforcer is constructible
because the predicate is intent. **`BL-216`** — the closure-quotation filing's own remedy is a FALSE
NEGATIVE ON ITS MOTIVATING CASE, the id on line 198 and the ellipsis on 199. Both new `sh` receipts
were proven BIDIRECTIONAL: exit 1 here, exit 0 against a simulated correct fix, each with a `cmp -s`
control asserting the mutation applied. **The first `BL-214` mutation returned exit 3 — the
migrator's "nothing to migrate" — a receipt satisfied by DELETING ITS OWN SUBJECT.**

**THE GAP IS TWO, PENDING 2, NOT BOOTSTRAPPING, NOT WIDE, NOT REQUIRED.** Consumer installed
`0.540.0`, distribution `0.542.0`. Both shipped ids resolve to `0.542.0` and are invisible to the
consumer until it pulls. The range is seven core paths, **0 mode-only changes** against a control of
7 changed paths, and no `preclassify.sh`/`apply.sh`/`ledger-reverify.sh`/update-skill path in it.
Backlog receipt histogram at close: 72 STILL-LIVE, 8 HAND-REVIEW, 2 NEEDS-REVIEW, **0
CLOSE-CANDIDATE** — no incidental closes.

**THE SWEEP FOR BATCH 82.** Live 75, cited 55, unfiled 20. Two of the three mid-batch filings remain
available and both are adjudicated NOT-READY with the reason recorded in `BL-215` and `BL-216` —
read those entries before re-scoping either. **`BL-137` IS THE READIEST NO-`PC` ENTRY AND ITS
SEPARABILITY IS ALREADY MEASURED** in both directions; `BL-153` is batchable with the caution that
its receipt accepts either remedy form, and `BL-133` SHIPS ALONE (3 of its 5 instances are in
bootstrapping files). Run the sweep anyway — the consumer has now filed mid-batch in four of the
last five batches.
...
**`VERSION` IS `0.540.0`, SO BATCH 80 RELEASES AS `0.541.0`.** Re-derive `VERSION` and add one.
Batch 79 merged twice: `v0.539.0` (PR #692, `BL-086`) and `v0.540.0` (PR #693, `BL-213`). They
did NOT batch because `BL-086` touches the update skill — `self-update-gate.sh` and its
`SKILL.md`, both bootstrapping, derived by running the shipping `machinery_paths()` — and a
bootstrapping fix ships alone; `BL-213`'s slice is `validate-stub-audit.sh` alone. Rotated live
81 -> 80, archive 131 -> 133. **Receipt histogram over entries: exactly the subject at exit 0
before each rotation, 67 at exit 1 and 1 at exit 9 before and after** — no incidental closes.

**THE SWEEP WAS EMPTY AT THE OPEN, AND THE CONSUMER FILED DURING THE BATCH.** Open: live 71,
cited 53, unfiled 18, md5 `c8bf4477…` UNMOVED from batch 78's close, archive 174, all six
controls fired, impossible-id date control empty, every one of the 18 adjudicated. **The
one-liner ARRIVED FROM `ai-dlc-25`**, so this session took `BL-086` on its own ranking. Then the
ledger's md5 moved to `3b33db78…` mid-batch: the consumer filed
`PC-S309-STUB-AUDIT-PROSE-MARKER-FIRES-ON-NEGATION-AND-TEST-DOUBLE-REFERENCE`, UNCOMMITTED in its
working tree — its `-S` date read EMPTY exactly like the impossible-id control, and only reading
the working file separated a filing from a grammar failure. Its receipt reproduced on the
distribution (consumer polarity exit 0, genuine-stub control fires), so it outranked the no-`PC`
set and shipped as `v0.540.0`. Close: live **72**, cited **54**, unfiled 18 (the same 18),
archive 174, DISCHARGED 26 / in-flight 28 / untouched 18, overlap 4, TERMINAL **60**,
discharged-but-invisible 1 (`PC-S308-GATE-METRICS-CHECK2-STALE-VERDICT-READ-ORDER`, `BL-195`'s
deliberately uncited subject).

**WHAT SHIPPED. `BL-086`** — every arm in the self-update gate was a differential and a push
failure predating the pull sat outside the difference by construction. The gate now runs the
pre-push hook GIT WOULD RUN (`git rev-parse --git-path hooks/pre-push`, so `core.hooksPath` and
the `.git/hooks/` shim both count, a hook git would skip is skipped), on the tree as the operator
left it, fed the ref line step 2's push sends from a FILE, and defers on script `pre-push` when
it exits non-zero; the record carries the hook's output as `# probe:` lines and a SAFE-STOP row
saying the split buys nothing. Neither of the entry's proposed remedies shipped: `git push
--dry-run` contacts the remote, and step 1's preflight pushes nothing on an in-sync branch.
**`BL-213`** — two carve-outs on Check 16's comment-portion prose gate: strict-adjacency
negation, and test path AND mock vocabulary within ten lines. Consumer findings 69 -> 57, zero
new on either tree.

**THE FIRST ADVERSARY'S BLOCKER, ON A GATE-GREEN BRANCH WITH A RELEASE COMMIT ON IT.** The gate
opened its verdict record IN PLACE under `_bmad-output/` before the probe; that path is untracked
and unignored on the reference consumer, so its hook's read-set skip saw an untracked path no
fixture reads and ran all 179 fixtures on every probe — 287s/257s/247s where the bare hook took
30s — and the commit's claimed 28s warm cost was unreachable by construction. The record is now
assembled under `$TMP` and moved in at exit; re-measured 37s. **And the run after that read 257s
again**, because the moved-in record was still untracked when the next probe fired: the warm
figure holds only on a committed tree, and the header says so. Two latent DEFECTs shipped in the
same correction: the probe passed the literal `origin` and, with no such remote, a NAME where git
passes the URL; and its local ref named a branch that did not exist, which a hook resolving each
pushed ref refused. The correction's bare `mv` then REPLACED a same-second record silently where
the old in-place append had left a visible double trailer — caught by the fixture's own
nested-write mutant surviving on the release-2 push, and fixed by yielding to an existing name.

**THE SECOND ADVERSARY'S TWO DEFECTS, BOTH SHIPPED IN THE RELEASE.** Eight prefixes from the
validator's own `PHASE_ABSENCE` list (`not implemented`, `not wired`, `no-op`, …) satisfied the
negation grammar, so `# not implemented stub` was acquitted where the shipped gate had fired and
the header's own must-fire case was held only by its colon; the negation phrase is now refused
when it carries deferral vocabulary. And four differently-wrong validators passed every fixture
arm AND the receipt because no seed sat at a boundary — the intervening-word bound widened, the
mock window at 5 and at 100, the test path widened. V31–V34 sit at those boundaries and the
receipt carries the same worlds. **A first cut of the bound seed sat six words out and its mutant
survived: a boundary seed sits ONE past the boundary.** The scoping hand's own implementation
also carried a defect the builder found by mutant — a hand-spelled word boundary let the
`prose-marker-unbounded` mutant survive.

**HARNESS HAZARDS MEASURED THIS BATCH.** `git push` exited 141 with every gate phase green FIVE
times; the ref never moved until a re-push on the verified content key. Both adversaries' first
sends "returned success" and never landed; both resent under 12000 characters. A `cherry-pick`
of three commits in one command applied nothing and reported nothing; one at a time landed all
three. A validator run over 2000 consumer files from the Bash tool exceeds the 600s ceiling and
must be backgrounded.

**THE GAP IS FOUR, PENDING 0, BOOTSTRAPPING, NOT WIDE, NOT REQUIRED.** Consumer installed
`0.536.0`, distribution `0.540.0`. Zero mode-only changes across 42 raw rows. Bootstrapping paths
in the range: the update skill's `SKILL.md`, `self-update-gate.sh`, `self-update-fixtures.sh`.
The installed `validate-layer-entries.sh` is byte-identical to the distribution's, so that
differential cannot discriminate and is not reported as a null. The two `PC-` tokens in the
range's messages are bare prefixes, not ids; PENDING by full slug is 0. Consumer porcelain 5 at
the open, 12 at the close, all under `_bmad-output/` and `docs/escalations/`; zero paths under
this repo's write surface moved. **The next pull's step 2 runs the OLD gate (no push probe, no
record) against the NEW runner — the `NOT-REQUIRED` tolerance case — and the probe protects the
pull AFTER the one that delivers it.**

**THE SWEEP FOR BATCH 80.** Live 72, cited 54, unfiled 18, residue ZERO. Run it anyway — the
consumer filed mid-batch twice in two batches. Then **`BL-195`**, `BL-087`, `BL-088` and the rest
of the live 80 — none carries a `PC-` id, so any PC-backed candidate the sweep turns up outranks
them. `BL-086` and `BL-213` are ARCHIVED (`LANDED (v0.539.0)`, `LANDED (v0.540.0)`), so do not
re-scope them.

### BATCH 78 SHIPPED AS `v0.538.0`, ONE RELEASE CARRYING TWO NO-`PC` ENTRIES — THE SWEEP WAS EMPTY A THIRD TIME, `BL-155` WAS TAKEN ON THIS SESSION'S OWN RANKING BECAUSE THE ONE-LINER ARRIVED FROM A PEER, THE BRIEFING PROBE FOUND `BL-212` IN THE SAME GATE, AND **FOUR DESIGNS WERE BUILT AND REFUTED BEFORE THE ONE THAT SHIPPED — ONE OF THEM PASSED THE FULL GATE GREEN AND WOULD HAVE REFUSED EVERY LEGITIMATE SELF-UPDATE.** THE GAP IS TWO, PENDING 0, BOOTSTRAPPING, NOT WIDE, NOT REQUIRED.

This block replaces the batch-77 record below it. Every figure was re-derived on the release
branch after the merge, against the working tree with controls in the same invocation; re-derive
them again rather than reading them.

**`VERSION` IS `0.538.0`, SO BATCH 79 RELEASES AS `0.539.0`.** Re-derive `VERSION` and add one.
Batch 78 merged once (PR #690, `0.538.0`, `BL-155` + `BL-212`). They shipped as ONE release
because both live in `self-update-gate.sh` — the same file, the same arm family — and neither
receipt is closed by the other's fix (scored both directions). The release touches THREE
bootstrapping files (`SKILL.md`, `self-update-gate.sh`, `self-update-fixtures.sh` of the update
skill), derived by running the shipping `machinery_paths()` globs over the range: 3 of the
7 changed paths. Rotated live 83 -> 81, archive 129 -> 131. **Receipt histogram over entries:
exactly the two subjects at exit 0 before rotation, 67 at exit 1 and 1 at exit 9 before and
after** — no incidental closes.

**THE SWEEP WAS EMPTY.** Live 71, cited 53, unfiled **18**, md5 `c8bf4477…` UNMOVED from batch
77's close, archive 174, all six controls fired, impossible-id date control empty. Every one of the
18 re-derives as adjudicated exactly as batch 75 left them. DISCHARGED 25 / in-flight 28 /
untouched 18, overlap 4, TERMINAL **60**, discharged-but-invisible **1**
(`PC-S308-GATE-METRICS-CHECK2-STALE-VERDICT-READ-ORDER`, `BL-195`'s deliberately uncited
subject). **The one-liner ARRIVED FROM `ai-dlc-b0`**, so per action 1 this session took the
readiest no-`PC` entry without asking and named the choice in its first ping.

**WHAT SHIPPED. `BL-155`** — step 2's autonomous self-update left no approval artifact. The gate
now records its verdict, its rows byte-identical to stdout, and one three-column `# input:` line
per consumer file the verdict read; the runner refuses without a recorded OK for its own resolved
range whose inputs still hash as recorded or as `theirs`. The cycle stays autonomous. **`BL-212`**
— the gate returned OK on an unreadable range because the failed diff read as empty; both refs
are peeled to a commit at entry and a non-git DIST refused, one UNDECIDED row naming the ref.

**FOUR REFUTATIONS, EACH MEASURED, EACH NOW AN ARM.** (1) A record keyed on base+theirs alone
accepts a post-write re-run: pre-write 2 DEFER, post-write 4 OK, reverted 2 DEFER. (2) Binding
every input to its record-time digest REFUSES THE LEGITIMATE FLOW — step 2 writes the slice and the
stamp between gate and runner; 6 of 9 recorded inputs read MOVED on the seed. **That cut passed the
full gate green (195 ok / 0 FAIL) and had a release commit on it** before the adversary's second
pass; the fixtures' join part wrote nothing between the two programs, so it could not see it.
(3) The same design still accepted a late gate run. (4) A plain `theirs` acceptance left the
digest column decorative post-write: a forged record naming the derived set with ALL-ZERO digests
passed. Shipped: theirs-acceptance conjoined with "the recorded digest is a blob the path carried
at base or in the range"; strict equality for the fallback-hook row; gate and runner both refuse
PRE-WRITTEN; required set derived from the hook; stamp not an input; tolerance keyed on the gate
at BASE composing the record filename, not on directory emptiness — that token was respelled
TWICE after the fourth refutation: the directory variable over-fired on a declaring gate, and
the write site's exact text UNDER-fired on a reflowed one, which is the acquitting direction.

**THREE FALSE ZEROS OF THIS SESSION'S OWN, CAUGHT BY HANDS.** The already-at-theirs arm's FP
claim ("0 of 11") was a VACUOUS zero — the arm's population on the consumer's live range is 0,
and against two stale bases it fires 4 of 4 and 3 of 3 (a refusal, the right direction). The
tolerance token I first proposed (`self-update-gate-${`) scored 0 on BOTH sides — it could not
spell the write site. And the contract's receipt sketch "runner exits 2 against the gate seed"
would have passed unfixed: that seed's dist has no `core/fixtures`, so the wrong-repo guard
already exits 2 there.

**THE GAP IS TWO, PENDING 0, BOOTSTRAPPING, NOT WIDE, NOT REQUIRED.** Consumer installed
`0.536.0`, distribution `0.538.0`. Zero mode-only changes. The installed `validate-layer-entries.sh`
is byte-identical to the distribution's, so that differential cannot discriminate and is not
reported as a null. **The next pull's self-update will run the OLD gate (records nothing) and
install the NEW runner — the exact delivery case the `NOT-REQUIRED` tolerance exists for; the
runner reads the gate at `base` and proceeds once.** Consumer porcelain 12 at the sweep, 2 at
the close (its own pipeline committed in between), all under `_bmad-output/`; ledger md5 unmoved;
zero paths under this repo's write surface moved.

**THE SWEEP FOR BATCH 79.** Live 71, cited 53, unfiled 18, residue ZERO. Run it anyway. Then
**`BL-195`** and the rest of the live 81 — none carries a `PC-` id, so any PC-backed candidate the
sweep turns up outranks them. `BL-155` and `BL-212` are ARCHIVED (`LANDED (v0.538.0)`), so do not
re-scope them. `BL-086` (the gate never asks whether the consumer can push at all) is adjacent to
this batch's subject and unchanged by it; the record now carries a scope line saying so.
...
**`VERSION` IS `0.536.0`, SO BATCH 77 RELEASES AS `0.537.0`.** Re-derive `VERSION` and add one.
Batch 76 merged once (PR #687, `0.536.0`, `BL-113`). It shipped ALONE because
`ledger-reverify.sh` and `SKILL.md` are in the machinery slice — derived by running the shipping
`machinery_paths()` globs over the range, 2 of the 8 changed paths. Rotated live 85 -> 84,
archive 125 -> 126. **Receipt histogram 71 STILL-LIVE-by-exit before and after** — one exit 0
before rotation and it was the subject; no incidental closes.

**THE SWEEP WAS EMPTY.** Live 73, cited 55, unfiled **18**, md5 `9459fa46…` UNMOVED from batch
75's close, archive 172, all six controls fired. Every one of the 18 dates 2026-09-07 or earlier
and re-derives as adjudicated exactly as batch 75 left them: 4 named in `origin/main` release
commits, 11 `NOT-UPSTREAM`, 1 refuted, 2 WITHDRAWN. DISCHARGED 27 / in-flight 28 / untouched 18,
overlap 4, TERMINAL 58, discharged-but-invisible **1** — and that one is
`PC-S308-GATE-METRICS-CHECK2-STALE-VERDICT-READ-ORDER`, batch 71's `BL-195` subject, which
shipped no fix and is deliberately uncited; the row is correct, not a gap. **The one-liner
ARRIVED FROM `ai-dlc-07`**, so per action 1 this session took the readiest no-`PC` entry without
asking and named the choice in its first ping.

**WHAT SHIPPED. `BL-113`** — both reverify engines extract a `verify: sh` receipt as ONE line, so
a receipt an author wrapped across two arrives cut inside its quote. The entry filed the
distribution engine's symptom: `eval` dies at exit 2, which reads STILL-LIVE forever with no
hint. **Driving the consumer engine on a scratch world found the worse half**: its `*)` arm reads
any non-zero exit as "no longer reproduces", and `receipt_absent_subjects` cannot downgrade it
because every path the fragment names still exists — so a receipt that never ran read
CLOSE-CANDIDATE beside a one-line control. Each engine now runs `bash -n` over a brace-wrapped
copy of the receipt whose closing `}` sits on a SECOND line, and refuses a non-parsing receipt as
`NEEDS-REVIEW` under `unresolved:` naming it MALFORMED. On the consumer side that wrapped copy is
the exact string it then runs.

**THE ADVERSARIAL HAND FOUND TWO DEFECTS IN THE COMMITTED FIX, AND BOTH WERE IN THE HALF I HAD
NOT DRIVEN.** The distribution engine's first cut parsed the BARE receipt. A trailing backslash —
the canonical way to break a command across lines — parses clean as a fragment on bash 3.2 and the
fragment EXITS 0, so the engine CLOSED a live entry on half a receipt; an open heredoc parses
clean too and read STILL-LIVE. The consumer engine caught both, because its wrapper's second line
turns each into a syntax error; the two engines disagreed about the same receipt. Fixed by
wrapping on the distribution side too, FP 0 of 234, and both shapes are now seeded in both
fixtures. **The second defect was in my receipt's near-miss**: `false # comment` exits 1 and a
syntax error exits 2, and both read STILL-LIVE, so a guard that parsed one string and RAN another
passed the receipt and the distribution fixture. The near-miss now exits 0 and must CLOSE. **A
near-miss that lands in the same bucket as the offender discriminates nothing.**

**THE OWED FP MEASUREMENT WAS TAKEN AND IS EMPTY: 0 of 234** `sh` receipts (consumer live 38,
backlog 72, archive 124) under the wrapped form; controls: a receipt cut inside a quote, a trailing
backslash and an open heredoc each exit 2.

**THE ENTRY'S OWN RECEIPT WAS A TOKEN GREP AND A ONE-LINE COMMENT CLOSED IT.** Built on the
pre-fix engine: a comment carrying `bash -n -c` and `MALFORMED` returned exit 0. Rewritten at close
to drive the engine on a seeded cut-quote receipt, an exit-0 near-miss and the backslash shape;
rejects the pre-fix tree, the comment-only non-fix, a guard that refuses any receipt carrying `#`,
and the bare-parse first cut.

**A MUTANT'S FIRST CUT SWALLOWED THE LINE AFTER ITS ANCHOR.** The wrong-string mutant's `skip`
logic consumed the guard line as well as the wrapper's closing line, so the control row vanished
and the arm reported "the mutant broke the closer". Re-anchored on the assignment's own two lines.
And the distribution fixture's mutant copy needed a `VERSION` marker and `lib.sh` beside it —
the engine walks up for its root, so a bare copy in `$WORK` reported INPUT-UNRESOLVED with no rows,
which scored as the same "broke the engine" finding.

**PUSH EXIT 141 WITH THE GATE GREEN, AGAIN.** Every phase PASS, `pre-push: all gates green`, ref
NOT on origin against a control. The retry landed. Confirm `git ls-remote` before opening a PR.

**THE GAP IS THREE, PENDING 2, NOT WIDE, NOT REQUIRED.** Consumer installed `0.533.0`,
distribution `0.536.0`; all three releases touch `core/skills/ai-dlc-update/**`, so the
consumer's installed engine runs the pull that delivers them. Zero mode-only changes in the
range. Differential on the consumer's real ledger, read-only: installed engine vs this one,
121 rows byte-identical, `cmp -s` control confirms the binaries differ — the pull changes
nothing observable today, and its limit is that the guard fires only on a receipt shape the
consumer has not yet written. The consumer's porcelain read 58 at the sweep and 3 at the close;
its own sessions committed, and zero paths under this repo's write surface moved.

**THE SWEEP FOR BATCH 77.** Live 73, cited 55, unfiled 18, residue ZERO. Run it anyway. Then
**`BL-155`**, `BL-188`, `BL-189`, `BL-195` and the rest of the live 84 — none carries a `PC-`
id, so any PC-backed candidate the sweep turns up outranks them. `BL-122` and `BL-148`, which
earlier blocks listed here, are ARCHIVED (both `LANDED (v0.523.0)`), so do not re-scope them.

### BATCH 75 SHIPPED AS `v0.534.0` AND `v0.535.0`, TWO RELEASES CARRYING ONE ENTRY EACH — THE SWEEP THAT EVERY PRIOR BLOCK SAID TO EXPECT EMPTY RETURNED TWO NEW `PC-` CANDIDATES, BOTH REAL, AND THE OPERATOR TOOK BOTH. **THE FIRST FIX'S FIRST CUT WAS WRONG IN THE DIRECTION THAT CLEARS A HARD BLOCK, AN ADVERSARIAL HAND MEASURED IT ON SIXTY COMMITS OF REAL HISTORY, AND THE HAND'S OWN PROPOSED NARROWING WOULD HAVE REFUSED THE CONSUMER'S MOTIVATING CASE.** THE CONSUMER PULLED TO `0.533.0` BEFORE THE BATCH OPENED, SO THE GAP WAS ZERO AND IS NOW TWO.

This block replaces the batch-74 record below it. Every figure was re-derived after the merges at
`327f1d78` and `505c3dc3`, against the working tree with controls in the same invocation;
re-derive them again rather than reading them.

**`VERSION` IS `0.535.0`, SO BATCH 76 RELEASES AS `0.536.0`.** Re-derive `VERSION` and add one.
Batch 75 merged twice: `327f1d78` (PR #684, `0.534.0`, `BL-209`) and `505c3dc3` (PR #685,
`0.535.0`, `BL-210`). **The split was MECHANICAL** — both subjects sit under
`core/skills/ai-dlc-update/**`, the machinery slice, so action 2's ships-alone rule bound each.
Rotated live 92 -> 85 (the first release also rotated `BL-202` through `BL-208`, which the s309
program had annotated LANDED without the verified-sha form the rotator keys on), archive
116 -> 125. **Receipt histogram 74 STILL-LIVE / 9 HAND-REVIEW / 2 NEEDS-REVIEW before and after
both rotations** — no incidental closes.

**THE SWEEP WAS NOT EMPTY.** Live 73, cited 53, unfiled **20**, md5 `9459fa46…`, archive 172,
all six controls fired. Two of the 20 were NEW, both committed by the consumer:
`PC-S343-ADJUDICATION-ROW-WITHHOLDS-THE-ONE-FIELD-ITS-RECORD-REQUIRES` (filed 2026-09-07 on the
`0.526.0 -> 0.530.0` pull) and
`PC-S309-READOPT-STAMP-REFUSES-A-CLEAN-MERGE-WHEN-A-DELETED-CORE-LINE-IS-A-SUFFIX-OF-ITS-REFLOWED-REPLACEMENT`
(filed 2026-09-08 on the `0.530.0 -> 0.533.0` pull). The other 18 re-derive as adjudicated exactly
as batch 74 left them. **The one-liner was TYPED BY THE OPERATOR**, so both were reported with a
marked recommendation and the operator chose both, READOPT first.

**WHAT SHIPPED. `BL-209`** — `readopt-override.sh` computed the superseded set as a whole-line
difference between the base and theirs sections and tested each survivor by substring against the
body. When upstream re-flows a paragraph a base line survives at theirs inside a longer line,
scores as deleted, and every faithful adoption contains it, so `--stamp readopt` refused exactly
the state it certifies; the consumer had stamped `reaffirm` for a readopt and its register carries
the wrong outcome name. Reproduced on a scratch copy of the consumer's Check 20 override at
`eb49b783 -> a798e215`. **`BL-210`** — `layer-drift.sh`'s blocking row printed the digest and
withheld the `clause` id the register schema requires; two clauses produce textually identical
rows, and SKILL.md's one example says `LC-E4`. The map is derived from the contract at THEIRS
beside the code set; the listing appends the clause as column 6 because five readers across three
fixtures take the digest from field 4.

**THE LESSON OF THIS BATCH: THE FIRST CUT OF `BL-209` ACQUITTED ON BARE CONTAINMENT, AND THAT
CLEARS A HARD BLOCK ON A BODY TEACHING A RULE CORE NEGATED.** The adversarial hand drove it over
the last 60 non-merge commits touching the shipped rule text: 46 of 209 deleted lines survive by
containment, 22 of them INSIDE a single theirs line where upstream had qualified or negated the
line in place. A body carrying only the old line passed `--check` and landed its stamp. **The
hand's proposed narrowing — require the containment to cross a line join — would have refused the
consumer's own motivating line, which is in the single-line group.** What separates a re-flow from
an edit is the BODY: a faithful adoption carries the theirs text that now holds the base line's
words, and a body teaching the old rule does not. The predicate now acquits only when
`carried_in_context` finds the body carrying that run. Re-verified by the same hand: 35 of the 46
score correctly in BOTH directions, the other 11 sit under an h1 anchor the shared resolver
cannot match and are refused as undecidable on both scripts.

**MY RECEIPT ACCEPTED THE WRONG FIRST CUT, TWICE.** The first receipt passed the filing's own
suggested fix (substring against single theirs lines) until re-seeded on a base line theirs SPLIT
across two lines. The re-seeded receipt then passed the bare-containment cut until seeded with a
negated line as well. **Build the non-fixes and score the receipt against each before reading its
zero.** Four are now rejected: origin/main, the filing's suggestion, a whole-adoption clear, and
bare containment.

**TWO MUTANTS SURVIVED FOR REASONS THAT HAD NOTHING TO DO WITH THE PREDICATE.** Arm J's re-flow
mutant survived because the arm's own successful `--stamp readopt` had advanced `base_sha` to
theirs, so the mutant compared a degenerate range; the arm now resets `base_sha` and asserts the
range is not degenerate before scoring. The negation mutant survived because the seed lowercased
the letter after `Never`, so the base line was not contained at all and every implementation
refused it; the seed is now asserted as a negation-in-place before any verdict is read. **A
surviving mutant is a claim about the SEED before it is a claim about the arm.**

**`BL-210`'s BUILD SURFACED A SECOND DEFECT.** The listing's whole-row `sort -u` was a subject
dedupe only while every column was a property of the subject; one entry keyed by two clauses in
one pass printed twice and the count line called rows subjects. The dedupe now keys on entry,
target and digest. Its adversary found two figures wrong in the new COMMENTS — a dual-keyed count
of three where the drive gives two, and a register total attributed to a population the listing
cannot derive — both corrected on the branch before merge. **A figure in a comment is a claim
about a population; name the population beside it.**

**THE CONSUMER'S REGISTER CARRIES NINE BAD CLAUSE VALUES AND NOTHING CAN SEE THEM** — 2 status
names where an id belongs, 2 `LC-A1` (the blocking status itself), 5 clause-vs-fired-status
mismatches, out of 441. `BL-210` makes the row print the right value; it does not add a detector
that joins a recorded clause back to the status that fired. That detector is owed and is not
filed.

**THE GAP IS TWO, PENDING 2, NOT WIDE, NOT REQUIRED.** Consumer installed `0.533.0`, distribution
`0.535.0`. Both subjects are `core/skills/ai-dlc-update/**`, so BOTH releases carry a
BOOTSTRAPPING change and the consumer's installed engine runs the pull that delivers them. The
consumer's porcelain moved 35 -> 58 across the batch, all under `_bmad-output/` and zero paths
under this repo's write surface.

**THE SWEEP FOR BATCH 76.** Live 73, cited 55, unfiled **18**; DISCHARGED 27 / in-flight 28 /
untouched 18, summing to 73 after subtracting the 4-id overlap; TERMINAL **58**;
discharged-but-invisible 0. **The available residue is ZERO** — all 18 unfiled re-derive as
adjudicated (4 named in `origin/main` release commits, 11 `NOT-UPSTREAM`, 1 refuted, 2 WITHDRAWN).
Run the sweep anyway: this batch is the fourth in a row where a block said to expect it empty and
it was not. Then **`BL-113`** (ships ALONE, bootstrapping; its owed false-positive measurement is
now TAKEN — `bash -n` over all 72 live `sh` receipts fails none, control exit 2; re-derive the count, it fell from 79 as entries rotated), then `BL-155`,
`BL-122`, `BL-148` and the rest of the live 85.

**ONE THING IS OWED TO THE CONSUMER BRIEF.** The `0.533.0` register note on
`steps__gate-validation__check-20.md` records a READOPT under `reaffirm`; once the consumer pulls
`0.534.0` the operator can re-stamp it `readopt` and the note should say the refusal was a
distribution defect, fixed. And the nine bad register clauses above are the consumer's to correct
once `0.535.0` prints the right value beside each row.

### BATCH 74 SHIPPED AS `v0.527.0` AND `v0.528.0`, TWO RELEASES CARRYING THREE ENTRIES — THE OPERATOR TOOK ALL THREE AVAILABLE CANDIDATES WHEN ASKED FOR ONE. **EVERY ADVERSARIAL HAND FOUND SOMETHING, AND ONE OF THEM FOUND A BLOCKER IN A FIX ALREADY COMMITTED: AN ARM THAT COULD NOT FIRE ON THE CASE THAT MOTIVATED IT.** A SECOND FOUND A **WRITER/READER WEDGE** A READER-ONLY FIX WOULD HAVE WIDENED, AND A THIRD MEASURED THE OBVIOUS FIX FOR `PC-S342` AT A **100% FALSE-POSITIVE RATE**. THE CONSUMER PULLED TO `0.526.0` BEFORE THE BATCH OPENED, SO THE GAP WAS ZERO AND IS NOW TWO.

This block replaces the batch-73 record below it. Every figure was re-derived after the merges at
`4ccba0ec` and `d0affb32`, against the working tree with controls in the same invocation;
re-derive them again rather than reading them.

**`VERSION` IS `0.528.0`, SO BATCH 75 RELEASES AS `0.529.0`.** Re-derive `VERSION` and add one.
Batch 74 merged twice: `4ccba0ec` (PR #671, `0.527.0`, `BL-199` + `BL-200`) and `d0affb32`
(PR #672, `0.528.0`, `BL-201` alone). **The split was MECHANICAL, not a scope reduction** —
`layer-drift.sh` is in the machinery slice, derived by running the shipping `machinery_paths()`
rather than by hand, so action 2's ships-alone rule bound it. Rotated live 87 -> 85, archive
113 -> 116. **Receipt histogram 72-at-1 before, 72-at-1 after both rotations** — the three closes
each showed as an exit 0 before rotating and none afterwards, so no incidental closes.

**THE SWEEP FOUND FIVE AVAILABLE, THREE OF WHICH WERE NEW THAT DAY.** Live 73, archive 170, md5
`a72adb6e…`, all six controls fired. Two of the five turned out to be batch 72's withdrawn/
superseded PRE-PUSH pair — **read a candidate's own status line before scoring it as available**;
their titles read like live defects and their bodies say WITHDRAWN. The genuinely new one was
`PC-S342`, filed during the consumer's own `0.525.0 -> 0.526.0` apply.

**WHAT SHIPPED. `BL-199`** — `validate-provenance-block.sh` tested `value in forbidden`, an EXACT
match, so a DECORATED placeholder passed. Driven on the consumer's live artifact:
`toolu_PLACEHOLDER_LEAD_TO_FILL` rc=0 PASS, bare `toolu_PLACEHOLDER` rc=1. The fix is a per-field
`forbidden_match: prefix_ci` on `tool_use_id`; `mode` stays exact because its forbidden value is
an enum member and `check_value` returns on an enum miss before the forbidden test — a hand built
the widened variant and got identical cells, so widening it would be VACUOUS. Unknown match mode
is a hard FAILURE, not a fallback. **`BL-200`** — Checks 29 and 33 presuppose artifacts
`discovery.md` authors while the carry-over variant gates before discovery; both now report
`NOT-YET-AUTHORED`. **`BL-201`** — a `layer-drift.sh` row prescribes an entry edit AND a verdict
keyed on that entry's working-tree blob, so recording first spends the record.

**THE BLOCKER, AND IT IS THE LESSON OF THIS BATCH: I SHIPPED A CHECK THAT COULD NOT FIRE, IN THE
RELEASE THAT FIXES ONE.** `BL-200`'s first cut required the sprint SLOT to be absent. Correct for
Check 29 (`_bmad-output/specs/s<N>/`, which no opening step writes) and FALSE for Check 33:
`carry-over-evaluation.md:23` and `:147` write into `planning-artifacts/s<N>/` BEFORE its own §7
gate. Measured at the two real gate shas — s307 (`1f35f5637`, the reproducing case) held 4 files
with `locked-requirements.md` ABSENT; s308 (`a4304cbfc`) held 7 with it present. The arm
self-excluded on the reproducing case and routed it to the real-absence FAIL. **Two checks that
read as a symmetric pair were not one.** Fixed at `498d85d4` before the merge.

**A READER-ONLY FIX WOULD HAVE SHIPPED A WEDGE, AND THE WRITER HALF PREDATES THIS RELEASE.**
`stamp-story-provenance.sh` validated the id against the charset pattern alone — its own comment
admitted it "does NOT catch a well-formed invention such as `toolu_PLACEHOLDER`" — so the writer
stamped onto every story BOTH values the reader refuses. Fixing only the reader widens that to the
decorated form. The writer now resolves `forbidden` and `forbidden_match` off the same schema
field. **Ask of every reader fix what the WRITER does with the same value.**

**MY RECEIPT ACCEPTED ITS OWN NEGATION, WHICH IS WORSE THAN THE FALSE-NEGATIVE I HAD STATED.** A
body reading *"This check never reports `NOT-YET-AUTHORED`; an absent artifact is always a FAIL"*
satisfied `BL-200`'s receipt, because a token grep cannot see the POLARITY of the sentence carrying
it. Reproduced with `cmp -s` controls. It now requires the DIRECTIVE phrase plus the scope heading
and rejects the pre-fix tree, the negation decoy and a bare-token decoy. Shipped as a correction
inside `0.528.0`.

**THE NARROWING IS THE CHECK. `PC-S342`'s OBVIOUS FIX HAS A 100% FP RATE.** "A verdict exists under
some other digest" fires on ordinary cross-pull expiry: driven against the consumer's real register
with the 12 current-key records withheld, **6 rows fired and all six were false**, every entry clean
in git. Narrowed on a dirty working tree: 0, and 1 with one entry dirtied. **The non-git guard is
load-bearing** — `git diff --quiet` exits 128 outside a repo, so a bare conditional reads it as
dirty; measured on a de-gitted copy, 6 false notes without it and 0 with it.

**THE FILING NAMED THE WRONG ROW AS ITS SHARPEST CASE.** It picked `EXTENSION-TITLE-MATCHES-CORE`;
driving `layer-drift.sh --adjudicated-codes` returns `OVERRIDE-SUPERSEDED`, `EXTENSION-HOOK-DRIFT`,
`EXTENSION-ANCHOR-DRIFT`, `EXTENSION-RETIRE-CANDIDATE` — its pick is ABSENT, so it is WARN and
blocks nothing. The rows that BLOCK are the multi-anchor `OVERRIDE-SUPERSEDED` arms it missed.
**Drive `--adjudicated-codes` rather than reading a level out of the contract by hand.**

**A CONTROL THAT WAS ONE PROPERTY SHORT LET A MUTANT PASS THE WHOLE FIXTURE.** `BL-201`'s first
control cleared the REGISTER, which exercises the register-empty path — so a mutant with the
dirty-entry guard removed passed 36 ok / 0 bad. The discriminating state is prior verdict PRESENT
and entry CLEAN. Three mutants now die on exactly one arm each, and the fixture's consumer had to
become a git repo because a plain directory cannot express "has uncommitted edits".

**FOUR OF MY OWN ERRORS WERE CAUGHT BY MECHANISMS OR HANDS, NEVER BY READING.** A `grep -q` fed
from a pipe read NOT-FOUND on matching input. A wrong-fix scoring used a non-discriminating input
— the artifact's own id failed every implementation, so the column I was varying decided nothing.
My schema prose broke `taught-schema`'s V4b mutant, whose anti-vacuity was a whole-file
`grep toolu_PLACEHOLDER` that my own explanation then satisfied; it now asks the PARSED schema
whether the FIELD carries the key. And the first `adj_spent_note` read `.recorded_at` where the
schema requires **`recorded_utc`**, with a `// "date unknown"` fallback beside it that would have
made every note print "date unknown" and read as working.

**TWO FIGURES I SHIPPED IN SCHEMA PROSE WERE WRONG AND A HAND CORRECTED BOTH.** "730 distinct
values across both trees" was a THREE-DIRECTORY count that re-derives as 731 today, and "exact
catches 0" was wrong — exact catches **1**, a test fixture under `graph/tests/fixtures/` outside
the directories I scanned. The conclusion (empty FP set) survived both, re-verified over the honest
**870**-value whole-tree population. **Name the population beside any such figure; it moves.**

**THE GAP WAS ZERO AT THE TOP OF THIS BATCH FOR THE THIRD TIME.** The consumer pulled to `0.526.0`
before the sweep ran. **These two releases reopen it at TWO**, `PENDING` 3 (`PC-S309` ×2,
`PC-S342`). Not wide, not owed yet — re-derive it. The consumer's porcelain moved 9 -> 20 mid-batch;
verified by mtime and content as `graph-40`'s own live sprint-309 work under `_bmad-output/`, with
zero paths touched under this repo's write surface.

**THE SWEEP FOR BATCH 75.** Live 73, cited 55, unfiled **18**; DISCHARGED 27 / in-flight 28 /
untouched 18, summing to 73 after subtracting the 4-id overlap; TERMINAL **56**;
discharged-but-invisible 1 (batch 71's `BL-195` subject, deliberately uncited — the row is correct,
not a gap). **The available residue is ZERO.** All 18 unfiled re-derive as adjudicated: 4 named in
`origin/main` release commits, 11 `NOT-UPSTREAM` in the adjudication brief, 1 refuted at
`182817fb`, plus the two PRE-PUSH entries the consumer WITHDREW itself. **Do not re-scope onto any
of them.** Run the sweep anyway — the consumer filed three candidates in one day during batch 73 —
but expect it empty and rank the no-`PC` set without waiting.

Then **`BL-113`** (bootstrapping — ships ALONE; a two-line `verify: sh` is truncated by the engine
and mis-scores silently, its FP set unmeasured and the first thing it owes), then **`BL-155`**,
**`BL-122`**, **`BL-148`** and the rest of the live 85.

**ONE THING IS OWED TO THE CONSUMER BRIEF, AND IT IS NOT A DEFECT IN THE FIX.** The consumer's own
receipt for `PC-S309-CHECK29-33-…` is
`theirs_has .claude/skills/ai-dlc/steps/gate-validation.md "### 29. Spec kernel integrity"` — a
heading this fix never touches. Measured 1 before and 1 after, against an impossible-heading
control of 0. It was satisfied before the fix existed, so a `CLOSE-CANDIDATE` row from it is
evidence about the heading and nothing else. **Tell them to replace it.**

**AND ONE THING WAS NOT MEASURED THOUGH IT WAS AVAILABLE**: `apply.sh` mid-apply with a spent
verdict. `core/fixtures/apply-drift-after-write` drives it and `apply.sh` has prior form here
(`PC-S331`). Assign it before the next release touching that path.
...
**`VERSION` IS `0.520.0`, SO BATCH 69 RELEASES AS `0.521.0`.** Re-derive `VERSION` and add one.
Batch 68 merged once: `69618a55` (PR #662, squash, `v0.520.0`) closing `BL-193` and citing
`PC-S340-AUDIT-RULE-FILES-DRIFT-FINDINGS-IN-CORE-PROSE-ARE-NOT-CONSUMER-FIXABLE` verbatim in the
release commit message — the id resolves to exactly ONE commit at `VERSION` 0.520.0. **Control note:
`PC-S999-NEVER` is NOT a valid impossible-id control on the message channel — it returns 2, because
this plan's own prose about that control has been committed. Use a token no commit carries; the
corrected control returned 0.** Rotated (live 89 → 88, archive 104 → 105), `--check` PASS with the
sha resolving before `--apply`. **Receipt histogram before and after: 76 all exit 1 before, and
exactly ONE exit 0 after, identified BY NAME as `BL-193`'s own — zero incidental closes.** The other
apparent zero is the `## Receipts` legend, prose about the grammar and not an entry; that is the
known trap in this extraction. No correction release. **The sixteenth batch opened by a peer's
handoff** (action 9's message from `ai-dlc-18`), so it scoped itself from its own ranking and stated
the choice in its first ping.

**ELEVEN OF THE THIRTEEN AVAILABLE UNFILED CANDIDATES WERE ALREADY ADJUDICATED IN THIS REPO, AND NO
BATCH HAD EVER JOINED THAT VERDICT TO THIS FILE.** `docs/reviews/graph-ledger-adjudication-brief.md`
carries a table headed *"D — LIVE, consumer-local"* whose rows are verdict `NOT-UPSTREAM`, with the
same verdict in `final-disposition.tsv`, `phase1-verdicts.tsv` and `adjudicable-entries.tsv`. Derived
both ways, controls passing: of the 13 available, **11 are rows in that brief and 2 are absent from
all four files** — the two `PC-S340-*` entries, which were filed 2026-08-31, AFTER the corpus pin,
which is why no adjudication covers them. **So the standing corpus this file has called "the S312
cluster nobody has read" HAS been read; the answer was recorded and never joined back.** Do not
re-scope onto those eleven. The brief's own words: *"There is no upstream work and none is coming."*

**THE SUBJECT: `PC-S340-AUDIT-RULE-FILES-DRIFT-FINDINGS-IN-CORE-PROSE-ARE-NOT-CONSUMER-FIXABLE`,
CLOSED AS `BL-193`.** `steps/retro.md` Step 4 told the lead to disposition every rule-file audit
finding, and its decision tree ends in editing a rule file — but Rule 27 forbids a consumer editing
core and `apply` restores any core file it writes, so on a consumer the instruction had no compliant
remedy.

**THE FILING'S SPLIT HAD EXPIRED, AND THE CORRECTION WIDENS THE CLAIM RATHER THAN NARROWING IT.** The
filing reports 33 findings across fourteen files, two of them the consumer's own to fix. Driving the
consumer's INSTALLED audit against its real tree: **32 findings across 14 files, and `--is-core`
returns 0 for ALL 14** — control in the same invocation, `docs/coding-conventions.md` returns 1.
Neither named consumer-owned file draws a finding today, so the consumer can fix **none** of the 32
where the filing implies two. `verification-discipline.md` warns that narrowing is not the safe
direction; this correction ran the other way.

**WHY IT WENT UNSEEN UPSTREAM FOR SO LONG, WHICH IS THE REUSABLE HALF.** Both pre-push hooks run the
audit `--fail-on=deterministic`, so only tier 1 gates — and narrative drift is **tier 2**. This repo
pushes green carrying **62** findings while the consumer's retro invokes the script BARE, defaults to
`--fail-on=any`, and exits 1 every run. **35 of the 61 narrative findings fire on the token
`measured:`**, attributed by testing each finding against every alternative in the class regex — and
that token is what `.claude/rules/resident-context.md:34` REQUIRES to stay. That is why the remedy
ROUTES the core half rather than proposing to strip it.

**SHIPPED SHAPE: ownership resolved once and reported at `emit()`, the single seam every class
already routes through.** Each finding prints `[core]`/`[local]`/`[unknown]`, each verdict line
carries the split, and a new `--fail-on=local` gates the half the reader can act on. `retro.md` uses
that threshold and routes `[core]` findings to the push-candidate ledger via `upstream-routing.md`,
which prescribes the SAME `--is-core` call.

**THE FIX'S OWN PROBE CAUGHT TWO DEFECTS IN THE FIX, BOTH IN THE SUBJECT.** With the resolver
...
unresolved now counts as LOCAL and fails. And the first cut asked the resolver ONCE PER FILE, taking
the audit from **200ms to 3700ms** across three interleaved pairs timed from inside the repo — a cost
the suite POLE's path pays. Reshaped to fetch the glob set once via `--list` and match in-process:
**390ms**, of which ~175ms is that single unavoidable resolver call, measured alone.

**A SECOND MATCHER WAS INTRODUCED DELIBERATELY AND SCORED AGAINST THE SHIPPING ONE.** It loads
`core-paths.sh --list` as DATA, so the manifest parser is not copied. Differential over all 112
corpus files: **112 agree, 0 disagree**, with both verdicts present in the corpus so the null is
readable. `fnmatch` was refused because `*` crosses `/` there.

**SIX FIXTURE ARMS IN `retro-audit-scans`, FOUR WORLDS ONE PROPERTY APART**, each scored against a
mutant: unresolved-acquits dies at the fail-closed arm alone, everything-core at the local arm alone,
everything-local and a dropped owner tag at the tag arms. Unmutated whole-tree control clean. **The
first mutant score was a BROKEN MEASUREMENT and the control is what exposed it** — `run.sh` calls
`seed.sh` itself and `seed.sh` re-resolves `$AUDIT`, so an env override is overwritten and the mutant
never runs, reading as a survived mutant at 0 FAILs. Score fixture mutants with a whole-tree copy.

**GAP TWO (`0.518.0` installed, `0.520.0` shipped) — PENDING 2, AND THE PULL IS REQUIRED, NOT MERELY
OWED.** Action 7's second test was run on the REAL subject with a `cmp -s` control proving the two
binaries differ (28573 vs 35134 bytes): on the same consumer tree, **installed exits 1 and shipped
exits 0**, because the installed audit has no ownership concept and fails that consumer's retro over
32 findings it cannot fix. That is a live red gate today, not a latent divergence — which is what
makes this REQUIRED rather than owed. **A measurement trap to avoid repeating: the shipped audit
resolves `core-paths.sh` as a SIBLING, so a copy run from `/tmp` reads `unresolved 32` and exits 1,
which reads as agreement and is a broken measurement.** Copy the resolver beside it.
**The bootstrapping hazard IS live in the range** — `emit-report.sh` and `hard-blockers.sh` moved at
...
**`VERSION` IS `0.518.0`, SO BATCH 67 RELEASES AS `0.519.0`.** Re-derive `VERSION` and add one.
Batch 66 merged once: `11ebb342` (PR #659, squash, `v0.518.0`) closing `BL-190`. Rotated (live
89 → 88, archive 102 → 103), `--check` PASS before `--apply`, resolving against the merge sha.
**Receipt histogram after the rotation: 76 receipts, all exit 1 — ZERO incidental closes.** No
correction release. **The fourteenth batch opened by a peer's handoff** (action 9's message from
`ai-dlc-52`), so it scoped itself from its own ranking and stated the choice in its first ping.
**No `PC-` id was cited**, so `named_absorbed()` has nothing to join and the consumer's ledger
cannot move on this release.

**BL-190: EVERY GIT CALL IN `validate-claude-rules.sh` NOW ANSWERS ABOUT THE REPOSITORY IT WAS
POINTED AT.** Git exports `GIT_DIR` ABSOLUTE to any hook running from a linked worktree, and
nothing in that program pinned a repository. **The WRITE:** A1's probe builds a `mktemp` repo by
upward DISCOVERY, so the inherited environment sent its `git add -A -f` at the caller's index —
measured on a clone with a linked worktree, **757 tracked paths to 3**, control unset 757 to 757,
eight arms then failing closed and every one reading as a corpus defect. **The READ, which is
silent and worse:** `a1_offenders()` and `glob_matches()` enumerate the corpus with a bare
`git ls-files`, so A1 printed `ok` over a corpus it never opened — measured against a tree carrying
a real tracked `.claude/settings.json` and a victim clean under `.claude/`, clean environment
CONVICTS and inherited environment ACQUITS.

**THE FIRST CUT OF THAT FIX SCRUBBED ONLY THE PROBE SUBSHELL AND WAS WORSE THAN THE DEFECT.** It
closed the write and left the read redirected, turning a loud clobber into a silent false green;
an adversarial hand caught it after the branch was gate-green. The shipped scrub is one line at the
TOP of the program, before any git call. **Three rejected alternatives, each BUILT rather than
argued:** moving the hook's scrub above the validator dispatch breaks nothing above it and still
reads 757 → 3 when the validator is invoked directly; `--git-dir=` is refuted by construction
because `git init -q .` itself honours an exported `GIT_DIR`, so no `.git` is created and A1's
self-probe goes DEAD; and `unset GIT_DIR` alone is defeated by `GIT_INDEX_FILE`, which git exports
to a pre-commit hook even from a PRIMARY checkout.

**THREE FIXTURE ARMS, EACH KILLING A BUILD THE OTHER TWO ACCEPT — AND THE THIRD EXISTS BECAUSE OF
A SEED DEFECT THIS BATCH ALMOST SHIPPED.** A fully reverted fix dies at `m12` (victim `9 -> 3`); a
subshell-only scrub passes `m12` and dies at `m13`; a scrub narrowed to `unset GIT_DIR` passes both
and dies at `m12b`. The entry NAMED `GIT_INDEX_FILE` + `GIT_WORK_TREE` as the separating input and
then drove neither — both arms set `GIT_DIR`, the one variable that build handles. **The assertions
were correct and the SEED was one property short**, which is the shape that reads as thorough
coverage.

**AND THE ARM POINTED AT CLOBBERING WAS ITSELF CLOBBERING WHILE REPORTING INTACT.** `git init` under
an exported `GIT_DIR` **silently succeeds without creating a `.git`**, so a victim seeded that way
is not a repository and every reading of it describes the OUTER repo. Measured on `m12`'s first
version: outer `17 -> 9`, both readings taken from that outer repo, which the seed had just
overwritten to exactly 9 — `v_after -eq v_before` held BY CONSTRUCTION and the arm printed
`intact at 9` over the wreckage. A count guard cannot catch it. Both arms now assert
`[ -d "$victim/.git" ]`, driven both ways.

**FILED AND NOT FIXED — BOTH ARE REAL WORK.** `BL-191` (the class: **45 files run `git init`, 42
scrub nothing**; through the hook the exposure is 1 of 14 and `run_fixtures` is protected, but run
DIRECTLY — how a hand debugs a fixture — **8 of 8 fixtures clobbered a 757-entry index and 6 did it
at exit 0 with zero FAILs**; its remedy is a `core/fixtures/lib/preamble.sh` seam plus a binding
that fails the push when a `run.sh` does not source it, touching 40 shipped files and their ship
declarations, and its detector's FP set measures **41 flagged of 45** so it ships REPORT-ONLY with a
downward ratchet). `BL-189` amended, not closed — see below.

**`BL-189`'s MECHANISM WAS REPRODUCED BY COMMITTING ITS DEFECT.** An unescaped backtick pair in a
fixture's diagnostic string — `` `git init` `` written as PROSE inside a double-quoted string — was
executed by the shell against this repo. No damage (`core.bare` false, index 757, `HEAD` unmoved),
but it materialised a `bare = false` line at `.git/config` line 8, appended after `hooksPath`. That
POSITION is exactly what `BL-189` reads as its tell and said was unreproduced: seeded with the bare
line stripped and a `hooksPath` key added, a re-init writes `bare` at line 8 and `hooksPath` at 7,
while a fresh clone writes it at line 4. **So the position is a signature of a re-init on a live
config; it still does not explain the value `true`, since a re-init writes `false`.** **NO LINT IS
PROPOSED and the entry says why**: the scan returns **224 executable-line hits**, dominated by python
heredoc bodies where nothing expands, so it is a ceiling of unknown depth and shipping it would be
the unmeasured lint `CLAUDE.md` forbids.

**THE LEDGER DID NOT MOVE THIS BATCH.** md5 `7b363a17…` at open and at close, unchanged. **The
consumer did not pull** — its stamp still reads `0.516.0` / `be47fd24`. **Live 67, cited 49, unfiled
18**, archive 168, partition control 0. Goal partition **22 DISCHARGED / 27 in flight / 18
untouched**, overlap 3, discharged-but-invisible 0, TERMINAL **54**. No write to the consumer by this
session or its three hands; a `graph-46` session was live in the peer list mid-batch and was neither
messaged nor written to, and the consumer's five dirty paths are all its own `_bmad-output/` state.

**GAP TWO (`0.516.0` installed, `0.518.0` shipped) — PENDING 2 (`BL-185`, `BL-186` at `0.517.0`,
both PC-backed), AND THE PULL IS OWED, NOT REQUIRED.** Batch 66 cited no `PC-` id, so it added
nothing to PENDING. A pull is operator-initiated; readiness is not authorization. Re-derive the gap
before reading this figure.

**THE SWEEP FOR BATCH 67, AND THE STRONGEST CANDIDATE IS ALREADY SCOPED AND MEASURED.**
`PC-S308-EMIT-REPORT-RUNS-LAYER-DRIFT-AND-UNREGISTERED-DRIFT-TWICE` was fully scoped this batch and
DELIBERATELY NOT STARTED, because `emit-report.sh` is machinery: it matches the
`core/skills/ai-dlc-update/**` glob in `reconcile/setup-sites.md`, `apply.sh:219` gates its own
writes on `emit-report.sh --verify`, and `self-update-gate.sh` on the live range returns
**`SELF-UPDATE-DEFER` twice plus `SELF-UPDATE-SAFE-STOP`** — no split lands the engine first, so
step 5 renders on the installed unfixed engine. **It SHIPS ALONE.** Its premise HOLDS and is not
refuted: `hard-blockers.sh:84` and `emit-report.sh:340` both pass layer-drift `DIST BASE THEIRS
CONSUMER`, and `:97`/`:333` both pass unregistered-drift `DIST BASE CONSUMER THEIRS`; the `UD_BASE`
split at `:74-78` moves only under `--post-apply`, which `emit-report.sh` never passes. Cost
re-measured here: **~20s of a 57s render**, five interleaved reps with the order reversed, spread
±1s.

**THREE THINGS MAKE THAT DE-DUPLICATION UNSAFE, ALL MEASURED, AND A BATCH-67 SESSION MUST READ THEM
BEFORE WRITING THE FIX.** (1) `hard-blockers.sh` **SWALLOWS a refusing detector** — stub
`layer-drift.sh` to `exit 2` and the wrapper exits 0 printing `0 HARD blockers.`; the duplication is
currently what MASKS that, because `emit-report.sh`'s own `ld_rc` renders `DETECTOR-REFUSED` beside
the false clean line. (2) **Fix shape (a) breaks `I105`** — its two predicates at
`validate-enforcement-map.sh:8774-8775` go `invoked` 1 → 0 with `exempt` 0, neither state — and makes
**four arms of `reconcile-emit-report` vacuous**, including the `E10`/`E11` pair whose stated purpose
is that neither alone moves a verdict. (3) The rendered **BYTES must not move** (`%-32s %s` padding,
two-space joins) or every approved report fails `--verify`, which `apply.sh:219` gates writes on.
**Shape (b) — a rows mode on the wrapper — is the recommendation**: it keeps the base split
single-sourced, leaves `I105` alone, and fails LOUDLY in that fixture rather than going quiet.

**AMONG THE NO-`PC` RESIDUE**, `BL-191` is the readiest and is this batch's own filing. `BL-187`
(Rule 21's stated mechanism does not exist), `BL-113` (ships alone, bootstrapping), `BL-122` and
`BL-148` rank after it. The 17 older unfiled ids are enumerated in action 1b, none filed since
2026-08-31.

### BATCH 65 SHIPPED AS `v0.517.0`, ONE RELEASE CLOSING THE TWO `PC-S308-*` CANDIDATES THE CONSUMER FILED ON 2026-09-06, WITH THREE MORE ENTRIES FILED AND NOT FIXED. THE CONSUMER PULLED TO `0.516.0` MID-BATCH, SO THE GAP IS ONE AND PENDING IS TWO, AND THE PULL IS OWED, NOT REQUIRED. THE SWEEP IS NOT EMPTY: ONE NEW `PC-S308-*` CANDIDATE ARRIVED WITH THAT PULL AND UNFILED IS 18.

This block replaces the batch-64 record below it. Every figure here was re-derived on 2026-09-06
after the merge, against the working tree with the controls in the same invocation; re-derive
them again rather than reading them.

**`VERSION` IS `0.517.0`, SO BATCH 66 RELEASES AS `0.518.0`.** Re-derive `VERSION` and add one.
Batch 65 merged once: `db078cbe` (PR #658, squash, `v0.517.0`) closing `BL-185` and `BL-186`.
Both rotated (live 89 → 87, archive 100 → 102), `--check` PASS before `--apply`, both resolving
against the merge sha. **Receipt histogram after the rotation: 75 receipts, all exit 1 — ZERO
incidental closes.** No correction release. **The thirteenth batch opened by a peer's handoff**
(action 9's message from `ai-dlc-01`), so it scoped itself from its own ranking and stated the
choice in its first ping.

**BL-185: RULE 25(a) STATES THE HEADER A MOVED BLOCK CARRIES, AND THE ROTATOR REFUSES AT ITS CUT
FLOOR INSTEAD OF AFFIRMING.** `PC-S308-RULE-25A-HISTORY-MOVE-HAS-NO-DEFINED-HEADER-FORMAT`. The
`## ` is load-bearing — `rotate-snapshot-archive.sh` cuts at the Nth-from-last `^## ` and never
interprets a heading. **The second half was taken on the OPERATOR'S instruction mid-batch**, after
the adversary measured that the reachable failure is a silent exit 0 no rulebook text reaches: one
surviving heading took the `N_CUT <= KEEP_ENTRIES` branch and exited 0 while the file grew
forever, 122514 bytes against 8821 over eight rounds, both arms printing `10 entr(ies) present`.
A size threshold was measured and REJECTED (defect case 4.40, five healthy files higher). FP set
of the shipped discriminator: 1 of 109. Fixture assertions 12/13/14 plus MUTATION 2.

**BL-186: EVERY SHARED APPEND-ONLY ARTIFACT DECLARES WHERE ITS ENTRY FORMAT LIVES.**
`PC-S308-WRITE-FORMAT-STEERING-APPLIED-AD-HOC-NOT-UNIVERSALLY`. **The filed remedy was REFUTED and
deliberately not built**: a duty to LOCATE a format is discharged by looking, and a competently
written vacuous locate-duty scored CLOSE-CANDIDATE against a prose receipt. The shipped duty is
that a format EXISTS and is DECLARED. Population joined from `pipeline-state-paths.json`
(`transient: false`, 20 entries) under **I95**, never hand-listed; FP set zero on the failing
tier, report-only for absence because 15 of 20 carry no declared format. Delivery verified by
running the real `install.sh` into a tree with `_bmad/` present — an installer refusal for a
missing BMAD precondition reads exactly like a packaging defect and is not one.

**BOTH FILED RECEIPTS WERE UNSATISFIABLE IN THREE INDEPENDENT WAYS**, and this is the batch's
reusable finding. They named `core/SKILL.md`, which resolves nowhere here (`/SKILL.md$` matches 3
tracked paths, so the basename fallback refuses on ambiguity); they used `theirs_has` where an
ABSENCE needs `theirs_lacks`; and their regex anchors are matched by `grep -qF` at
`ledger-reverify.sh:611` — literal, deliberately. **A correct fix scores STILL-LIVE under literal
semantics**, proved by construction. `BL-185`'s replacement then failed twice more: it counted a
TOKEN (prose FORBIDDING the format closed it, as did an HTML comment), and its own successor
carried a VACUOUS CONJUNCT — it built a probe history from a HARDCODED heading rather than the
form it extracted, so the rotator arm could never fail. The shipped receipt is two anchored greps
and no range, scored across nine worlds.

**FILED AND NOT FIXED — THESE ARE REAL WORK, NOT BOOKKEEPING.** `BL-187` (Rule 21 says the gate
FAILS on a missing step-token citation; **zero** programs read the token against a control of 9
validators reading the gate log, and citation practice decayed to 0 across five consecutive
consumer sprints unreported). `BL-188` (no gate bounds `SKILL.md`'s size or its narrative;
`audit-rule-files.sh:19` names narrative as its own job and scored the offending passage 0 —
**filed with NO lint proposed**, because verbosity-is-scar-tissue governs that file and any check
keyed on length flags the prose the rule requires). `BL-189` (an argument-less `git init --bare`
under an exported `GIT_DIR` writes `core.bare=true` into the real repository, and git exports
`GIT_DIR` ABSOLUTE to any hook running from a linked worktree — reproduced on the eleventh
attempt; the writer is still unidentified and the entry says so).

**A DEFECT THE `BL-186` HAND FOUND AND NOBODY HAS FILED: `scripts/validate-claude-rules.sh:99-101`
runs `git init` + `git add -A -f` in a `mktemp` dir, and from a linked worktree `GIT_DIR` is
absolute, so the probe's `git add` writes the REAL index — collapsing it from 757 files to 3 and
failing every corpus-derived arm closed.** It cost that hand two blocked pushes and one clobbered
index. **It makes every worktree-based hand's push unlandable and it is the same `GIT_DIR` root
cause as `BL-189`.** File it and fix it; it is the readiest item in this file.

**THE LEDGER MOVED THIS BATCH, BY THE CONSUMER'S OWN PULL.** md5 `b1e78692…` at open,
`7b363a17…` at close. **The consumer pulled `0.515.0 → 0.516.0` mid-batch** (its PR #1024,
`5002539e7`), rotating the two candidates this batch closed and filing one new one,
`PC-S308-EMIT-REPORT-RUNS-LAYER-DRIFT-AND-UNREGISTERED-DRIFT-TWICE`. No write to the consumer by
this session or its five hands (reads, `cat`, copies to `mktemp`; one rotator dry-run driven
against a `/tmp` COPY, never the source). **Live 67, cited 49, unfiled 18**, archive 168,
partition control 0. Goal partition **22 DISCHARGED / 27 in flight / 18 untouched**, overlap 3,
discharged-but-invisible 0, TERMINAL **54**.

**GAP ONE (`0.516.0` installed, `0.517.0` shipped) — PENDING 2 (`BL-185`, `BL-186` at `0.517.0`,
both PC-backed), AND THE PULL IS OWED, NOT REQUIRED.** A pull is operator-initiated; readiness is
not authorization. Re-derive the gap before reading this figure — batch 50 opened on a resume
block that said gap ONE when the consumer had already pulled it to ZERO.

**TELL THE CONSUMER, IN THE NEXT BRIEF,** that from `0.517.0` Rule 25(a) prescribes
`## [MOVED <ISO-8601 timestamp> from <source basename> — <trigger>]` for every history/archive
move, and that its own `pipeline-snapshot-history.md` carries 11 unheaded markers nested inside
one 467-line entry — **after its next rotation that file lands on exactly `--keep-entries`
headings and the rotator will REFUSE rather than affirm**, which is the fix working and not a
regression; the remedy is to head those blocks. Also that `validate-write-format-steering.sh`
reports (does not fail) on 15 of 20 durable artifacts carrying no declared format, and that the
batch-61 through batch-64 brief items below still stand.

**THE SWEEP FOR BATCH 66.** One new candidate,
`PC-S308-EMIT-REPORT-RUNS-LAYER-DRIFT-AND-UNREGISTERED-DRIFT-TWICE`, arrived with the consumer's
pull and outranks the no-`PC` residue on provenance. Read its status line in the consumer's ledger
and run its receipt raw before scoping it. **The unfiled `validate-claude-rules.sh` `GIT_DIR`
defect above is the strongest no-`PC` candidate and it BLOCKS other hands' work**, which is a
consequence argument the ranking should weigh. Among the live no-`PC` entries, `BL-187` (filed
this batch, and its subject is a rule whose stated mechanism does not exist), `BL-113` (the
reverify engine truncates a two-line `verify: sh` receipt; its reader is a BOOTSTRAPPING file, so
it ships alone), `BL-122` and `BL-148` rank next. The 17 older unfiled ids are enumerated in
action 1b, none filed since 2026-08-31.
...
after the third merge, against the working tree with the controls in the same invocation;
re-derive them again rather than reading them.

**`VERSION` IS `0.515.0`, SO BATCH 64 RELEASES AS `0.516.0`.** Re-derive `VERSION` and add one.
Batch 63 merged three times: `56a104cd` (PR #653, squash, `v0.514.0`) closing `BL-171`,
`24b957e4` (PR #654, squash, `v0.515.0`) closing `BL-176`, and the docs commit carrying this
block; both entries rotated and `BL-184` filed (archive 95 → 97, live 88 → 87). Neither id
carries a `PC-`, so `named_absorbed()` has nothing to join. No correction release. **The
eleventh batch opened by a peer's handoff** (action 9's message from `ai-dlc-64`), so it scoped
itself from its own ranking and stated the choice in its first ping: the sweep was empty and the
block below ranked `BL-171` and `BL-176` first. **They were built in parallel by two hands and
did NOT batch into one release**: the merged branch was green at every gate phase when BL-176's
adversary returned two BLOCKERs, so `BL-171` shipped alone from the pre-merge commit and
`BL-176` followed as its own release once both were fixed and re-measured. A green gate on a
merged branch is not a reason to merge while a hand is out; this batch is the measurement.

**BL-171: THE GATE NOW VERIFIES THE CITATION THE GUARD ALREADY VERIFIED.**
`validate-gate-adjudication.sh` takes `--transcript` / `--transcript-dir`, verifies each
in-force suppression's quote with `validate-steering-budget.sh --cite` through the guard's own
helpers (four byte-identical sites under I92 and I103 now), fails closed with `no-transcript`,
reads the verifier's exit in three tiers so node off PATH is reported as tooling rather than
forgery, and widens a lone transcript to its directory as the guard does. Check 26's call site
passes both flags, MANDATORY. Its adversary's BLOCKER: every seeded corpus held ONE file, so a
verifier scanning the first member satisfied the receipt and every case while reading NOMATCH
on the consumer's one genuine in-force citation; the seed's quote sits in the second file now
and m17 scores it. Fixture S18–S23, mutants m14–m18, 19 in the battery.

**BL-176: A VERDICT BINDS TO THE DISPATCH THAT PRODUCED IT, BY ORDER IN HOOK-WRITTEN
TIMESTAMPS.** The filed name join is refuted by its own subject (the round-nonce file names an
agent that IS in the spawn ledger, and 102 legitimate verdicts are not). The guard's arm 2
records who wrote each verdict path when the harness attributes it to a dispatched agent; arm
7a and the validator's `dispatch_binding()` bind by that row or by a `gate-adjudicator`
dispatch within 900s after the nonce, a hard constant in both readers. An unbound verdict may
convict but never acquit. The adversary's two BLOCKERs, both fixed pre-merge: with nothing
bound the guard fell through to ALLOW silently (it convicts on the newest failing pass now);
and the spawn ledger sat outside the guarded roots so a denied lead could Write the dispatch
row that binds its own forgery (guarded now). Also fixed: the window was overridable from the
environment (10^9 passed the receipt and all three fixtures), and the validator fixture's
one-row ledger made every "binds" arm pass by EXEMPTION — a one-row ledger makes its own row
the epoch. Stated limit, in the arm's header: it catches a nonce minted in its own future, not
forgery as such; a lead that mints at now, dispatches any adjudicator (B2's role is read from
the lead's own prompt) and writes the verdict itself is indistinguishable from a real pass.

**THE LEDGER MOVED THIS BATCH, BY THE CONSUMER'S PULL AND NOT BY ANY CITATION HERE.** md5
`417c783e…` at open, `2c129779…` at close; the consumer pulled `0.507.0 → 0.513.0` while the
batch ran (its checkout went from `9e6466cad` on a carry-over branch, through an
`ai-dlc-update/self-update-0.508.0` branch, to `49f72d473` on `ai-dlc/layer-debt/retro-owed-20260906`,
porcelain 4 throughout), and that pull rotated SEVEN candidates: five cited ids moved
DISCHARGED → TERMINAL and the two already-in-core `RETRO` siblings from batch 59 left the
unfiled set — the consumer's own rotation, as batch 59 said it would be. No write to the
consumer by this session or its four hands. **Live 65, cited 47, unfiled 18**, archive 167,
partition control 0. Goal partition **20 DISCHARGED / 27 in flight / 18 untouched**, overlap
3, discharged-but-invisible 0, TERMINAL **54** (was 49).

**GAP TWO (`0.513.0` installed at `070b6442`, `0.515.0` shipped) — PENDING 2 (`BL-171` at
`0.514.0`, `BL-176` at `0.515.0`, no `PC-` id among them), AND THE PULL IS OWED, NOT
REQUIRED.** Batches 59 and 60's evidence is delivered: the installed `0.513.0` carries both
fixes. This batch's own second test is a null on the consumer's real files, read-only: its one
in-force suppression verifies against its corpus and its latest FAIL verdict blocks identically
under the installed and the fixed reader; its live pass is `bound-dispatch`, both validators
exit 0 on it, and the forged `planning-20260902T160000Z` blocks as the control. What the null
does not cover: the installed validator never parses past its two positionals, so the new
Check 26 flags are INERT on it and a forged suppression passes at exit 0 with no warning, while
the new validator run from the old call site blocks every suppressed FAIL with `no-transcript`
— the step file and the validator land in one pull. Range `070b6442..origin/main`: 13 core
rows, 0 mode-only, 0 bootstrapping files; the INSTALLED `0.513.0` gate runs the delivering
pull. A pull is operator-initiated; readiness is not authorization.
...
**TELL THE CONSUMER, IN THE NEXT BRIEF,** that from `0.514.0` Check 26 requires
`--transcript-dir` (and `--transcript`) on `validate-gate-adjudication.sh`, that the old call
site against the new validator blocks every suppressed FAIL with `no-transcript`, that 2 of its
17 historical suppression citations are paraphrases that NOMATCH its corpus (both S305,
"Fresh SUPPRESSED, gate only" variants; neither in force) and will read as a regression to
whoever re-cites one; that from `0.515.0` the remediation guard writes
`_bmad-output/gate-adjudication/.verdict-writes.jsonl` (durable, under the gate directory,
committed), a verdict whose nonce no dispatch follows within 900s is refused by Check 26 with
the remedy "mint the nonce at entry, THEN dispatch", nine of its historical verdicts are that
class, and a gate-adjudicator that writes its verdict through Bash rather than Write is bound
by the dispatch window alone; and that the batch-61 and batch-62 brief items below still stand
(batches 59 and 60's are delivered).

**THE SWEEP FOR BATCH 64 IS EMPTY, AND THE 18 UNFILED IDS ARE THE STANDING CORPUS.** Check the
ledger's md5 first — the consumer files mid-batch and pulls mid-batch, and it did both shapes
of the latter this batch. Of the 18, one awaits the operator's rejection (above) and the other
17 are enumerated in action 1b and the batch-39 paragraph of action 1, none filed since
2026-08-31. No PC-backed candidate is real work today. What ranks first, by consequence, now
that `BL-171` and `BL-176` are rotated: `BL-163` and `BL-161` (batch 50/51 residue, NOTE tier,
read their entries — `BL-161` carries a driving receipt that exits 1 today), then `BL-184` (the
genuine-operator predicate's two limits, owner `validate-steering-budget.sh`, four readers move
together — measure its false-positive set on the consumer's 17 historical suppressions first),
then `BL-159` (a consumer `.gitignore` item, brief material rather than core work). None is
bootstrapping on its face; derive each subject's reader set before deciding whether two may
batch.
...
re-derived AGAIN at the fresh-resume check, which is when the filing below appeared; re-derive
them again rather than reading them.

**THE CONSUMER FILED AFTER THE MERGE, THE FILING IS VERIFIED HERE, AND IT IS BATCH 59'S
SUBJECT — TAKE IT.** `PC-S308-SUPPRESSION-LIFETIME-RESOLVES-GATE-METRICS-FROM-CWD-BEFORE-PROJECT-ROOT`
entered the ledger on 2026-09-05 as an UNCOMMITTED edit in the consumer's tree (the ledger
reads ` M` in its porcelain, so the sweep's `-S` date is EMPTY for it until its session
commits; md5 `7f097cf7…` → `417c783e…`, live 71 → 72, unfiled 23 → 24). The claim:
`core/scripts/validate-suppression-lifetime.sh` resolves its default `--gate-metrics` file by
trying three CWD-relative candidates BEFORE the one under `AI_DLC_ROOT` (the `for cand in`
loop in section 2 of that script), and the remediation guard's arm 7b — batch 55's `BL-169`,
`0.505.0` — passes `--gate-metrics` only when `AI_DLC_GATE_METRICS` is set, so a suppression's
lifetime is counted against whichever project the caller is STANDING IN. Verified on this
tree before this block was written: `core/fixtures/gate-remediation-deny/run.sh` reads PASS
from this repo's root (which has no `gate-metrics.jsonl`) and **18 assertion(s) FAILED** from a
scratch directory carrying a copy of the consumer's 348-line metrics file — the consumer's
pre-push has failed that fixture from its repo root since it pulled `0.505.0`, and it pushed
the `0.507.0` reconcile `--no-verify` on the record. The distribution's own suite cannot see
this, because its root carries no metrics file. Subject files: that validator and
`core/hooks/ai-dlc-gate-remediation-guard.sh` — neither bootstrapping, so it may batch under
action 2. The filing's remedy is right as far as it goes (resolve under `AI_DLC_PROJECT_ROOT`
first, fall back to CWD only when no root was given; have the hook pass `--gate-metrics`
explicitly, since it knows `PROJECT_DIR`); derive the reader set of that candidate loop before
trusting it, and seed the fixture from a cwd that carries a metrics file, which is the world
the consumer's suite runs in and this repo's never has. Its receipt is `verify: manual` by its
own note (a reorder keeps every token), so write a driving receipt.

**`VERSION` IS `0.508.0`, SO BATCH 59 RELEASES AS `0.509.0`.** Re-derive `VERSION` and add one.
Batch 58 merged at `d6cf1a97` (PR #641, squash; on the branch the release commit came last
after seven fix, test and docs commits, none naming an id), closing
`PC-S337-RETIRED-TOKENS-CANNOT-SAY-IT-SCANNED-NOTHING` as `BL-177`, rotated (archive 88 → 89,
live 88 → 89 → 88 after one filing and one rotation). `named_absorbed()` lists exactly one
release commit for the id at `0.508.0`; the impossible-id control for that channel is NOT
`PC-S999-NEVER`, which docs commits quote. No correction release. **The sixth batch opened by
a peer's handoff** (action 9's message from `ai-dlc-03`), so it scoped itself from its own
ranking and stated the choice in its first ping: the sweep was empty at open (ledger md5
unmoved, 77 / 53 / 24, newest filing 2026-08-31), and the top-ranked standing candidate's
subject is the update skill's `reconcile/retired-tokens.sh`, a bootstrapping file, so it
shipped ALONE under action 2.

**THE CONSUMER PULLED MID-BATCH, AND THE RESUME BLOCK SAID GAP THREE.** Its stamp reads
`0.507.0` / `528ca84b` on all four fields (commit `2fb0ae7d7` at 20:11, 42 files, while this
batch's hands were out), which rotated SIX cited ids into its archive — the batch-55 pair
(`PC-S308-GATE-REMEDIATION-GUARD-BLIND-TO-SUPPRESSED`, `…RULE-11…`), batch 56's
`…LEDGER-REVERIFY-READS-ESCAPED-BACKTICKS…` and the three batch-57 `PC-S305-*` — and filed
nothing (live 77 → 71, archive 154 → 160, md5 `773146d7…` → `7f097cf7…`, unfiled 24 → 23 by
this batch's citation alone). **Check the delivery gap BEFORE reading this block's figure for
it** — the seventh time it has closed in this program. The consumer's porcelain read 10 at
open, then 1, 2 and 4, every move a commit or a pipeline-state write by its own live
sprint-308 session (`8b25f9475` at 19:27 carried exactly the ten); no write to the consumer by
this session or its four hands. Its HEAD is `2fb0ae7d7` on
`ai-dlc/carry-over/pool-pnl-backlog-triage`; peer `graph-b9` was busy and was not messaged.

**THE FIRST CUT PASSED EVERY GREEN CHANNEL AND THE FOUR HANDS FOUND SEVEN HOLES, ALL BEFORE
THE MERGE.** The consumer-side hand drove the first cut on a real range that listed 8
`CLASSIFY` files and opened 7 and read a NOTE naming only the 7 — the vacuity class the
change exists to close, one level in; the NOTE now prints `M of N`. The scope hand found the
refusal listing only exotic causes (a docs-only range yields zero preclassify rows too, and
the fixture's own p8 drives exactly that case). The receipt adversary hardcoded the retiring
count and fabricated the listed count and both channels stayed green, because every world had
exactly one file retiring a token; a third core file whose token survives gives a world with
nothing retiring. The fixture adversary inserted ONE line — say nothing unless something was
listed — and every arm stayed green, because the deleted-file worlds had displaced the
incident's own all-at-theirs shape where nothing is listed; that world is back, asserted as
`listed 0`. The same hand found `mkmut`'s `exit 2` inside `$( )` ending the subshell only, so
a moved anchor scored `0000` as a REGRESSION rather than BROKEN (proven on a copy with one
anchor misspelled). Fixing it exposed the next one: a predicate returning early on a mutant
left its last world on disk and the next predicate read `2 of 2` where nobody had seeded it,
so every seed now defines the whole consumer state. **And the fixture's pre-existing
comment-strip mutant had been passing for the wrong reason since it was written**: a lone
copy of the detector has no `preclassify.sh` beside it, reads zero rows on every input, and
its silence scored as a kill the strip never earned — the `tool-hazards.md` "copied validator
needs its siblings" hazard, inside a fixture. Every mutant is now a copy of the whole
reconcile directory with the sibling asserted present, and the strip mutant carries a stderr
control saying it opened one file. Every hand's payload truncated at ~16000 characters; each
tail was recovered by asking for one named section under a stated size. The fixture went from
under a second to ~31s solo (recorded loaded cost 6 before this batch; the pole is
`layer-reference-resolution` at ~250) and passed in both layouts, from `/tmp`, and under
`env -i`.

**THE CONSUMER-SIDE DIFFERENTIAL IS A NULL ON STDOUT AND DIVERGES ON STDERR, WHICH IS THE
WHOLE FIX.** On the consumer's real range at the time (`6fc38a46..e3f96795`, on a scratch
clone) both scripts print 0 rows byte-identically (`cmp -s` control: the scripts differ, 0 vs
3 `>&2`); the installed copy prints nothing on stderr and the fixed one prints `listed 0
CLASSIFY file(s) and opened NONE`, because 34 of 39 paths are `UPSTREAM-ONLY` and none is
`CLASSIFY` — a different bucket from the filed incident's `ALREADY-AT-THEIRS`, same NOTE. Both
program callers (`apply.sh:504`, `emit-report.sh:243`) discard stderr, so nothing the pull
GENERATES changes; the NOTE reaches the operator running step 3a-ii by hand. The 0.504.0 →
0.507.0 pull did not change this file, so the reading holds against the installed `0.507.0`.
**So the pull is OWED (PENDING 1) and NOT REQUIRED on this batch's evidence**, and the
batch-55 and batch-57 reasons it WAS required are discharged by the consumer's own pull. A
pull is operator-initiated; readiness is not authorization.

**THE LEDGER MOVED TWICE: ON THE CONSUMER'S PULL, AND ON ITS FILING AFTER THE MERGE.** md5
`417c783e…` at the fresh-resume check (`7f097cf7…` between). **Live 72, cited 48, unfiled 24**,
archive 160, partition control 0. Goal partition **21 DISCHARGED / 27 in flight / 24
untouched**, overlap 3, discharged-but-invisible 0, TERMINAL **49** (43 + six rotated). Gap ONE (`0.507.0` installed, `0.508.0` shipped), PENDING 1 (`BL-177` at `0.508.0`,
one `PC-` id). Say WIDE at five. The range `528ca84b..origin/main` carries bootstrapping
files (`reconcile/retired-tokens.sh` and the update skill's `SKILL.md`); re-derive core paths
and mode-only changes against the STAMP commit, not against `6fc38a46`.

**THE TOP-RANKED CANDIDATE STILL AWAITS THE OPERATOR'S REJECTION.**
`PC-S340-CHECK-26-READS-A-PARTIAL-RE-VERIFY-VERDICT-FILE-AS-UNADJUDICATED` is live at the
consumer (1 in `live.txt`), recommended for rejection in the batch-57 block below and in the
`0.507.0` CHANGELOG's "Not taken" section, deliberately not named in any release commit. Ask
the operator to carry it into a graph session; until then it stays live and unfiled.

**TELL THE CONSUMER, IN THE NEXT BRIEF,** that `0.508.0` makes `reconcile/retired-tokens.sh`
say on stderr when it opened no core file, and that its next pull's hand run at step 3a-ii
will print `listed 0 CLASSIFY file(s) and opened NONE` because the range lists no `CLASSIFY`
file; that a run which opened files and matched nothing prints `M of N CLASSIFY file(s)
opened, R carrying …`, and a range on which `preclassify.sh` lists nothing at all is refused
rather than read as clean; that rows, exit code and the per-path form are unchanged, so its
reconcile report and worklist do not differ by a byte; that its own receipt for the entry
(`theirs_lacks … ">&2"`) is a whole-file substring test a comment would satisfy, and it is
satisfied for the right reason; that `PC-S303-POSTCOMPACT-…` still awaits its re-disposition
(remediated at `9cbb77f`); and that the batch-57 brief items below still stand except the
pull-required ones, which its own pull discharged.

**THE SWEEP FOR BATCH 59 HOLDS ONE NEW FILING, ABOVE, AND IT RANKS FIRST; IF A PEER HANDED
YOU THIS PLAN, TAKE IT WITHOUT ASKING, AND BATCH A STANDING ONE BESIDE IT ONLY WHERE ACTION 2
ALLOWS.** Check the ledger's md5 first — the consumer files mid-batch, pulls mid-batch, and
filed once after this batch's merge. Of the 24 unfiled ids, 23 are the standing corpus
enumerated in action 1b and the batch-39 paragraph of action 1, none filed since 2026-08-31,
and the batch-57 block's per-status reading of them below still holds. What ranks after the
new filing, by consequence:
`PC-S305-UNION-GATE-UNPASSABLE-ON-ANY-PULL-THAT-HAD-A-BLOCKER` (SKILL.md step 7 —
bootstrapping, alone; re-derive whether the union gate's clause (1) still precedes the
adjudication loop before believing the filing),
`PC-S335-NO-DETECTOR-REACHES-A-RETIRED-STATUS-TOKEN-…` (a new report-only detector in
reconcile — bootstrapping, alone; its receipt is `verify: manual` by its own note), and the
three `PC-S337-RETRO-*` extension drains (prose additions to `retro.md`, `verify: manual`,
batchable). `BL-159`, `BL-161`, `BL-163`, `BL-171`, `BL-176` (no `PC-` id) rank below any
PC-backed one; `BL-171` and `BL-176` are the two with a consumer consequence today.

### BATCH 57 SHIPPED AS `v0.507.0` — THE BLOCK ABOVE REPLACES THIS ONE. TAKE THE STATE FROM THERE.

This block replaces the batch-56 record below it. Every figure here was re-derived on 2026-09-05
after the merge, against the working tree with the controls in the same invocation; re-derive
them again rather than reading them.

**`VERSION` IS `0.507.0`, SO BATCH 58 RELEASES AS `0.508.0`.** Re-derive `VERSION` and add one.
Batch 57 merged at `3520fbb4` (PR #639, squash; on the branch the release commit came last
after eight fix, test and docs commits, none naming an id), closing THREE candidates in one
release under action 2 — `PC-S305-RETRO-BRANCH-CUT-FROM-STALE-PARENT-NOT-MAIN` as `BL-173`,
`PC-S305-NARRATIVE-DRIFT-FALSE-POSITIVE-USED-TO` as `BL-174` and
`PC-S305-GATE-VALIDATION-PREFER-IT-SOFT-LANGUAGE` as `BL-175` — all rotated (archive 85 → 88,
live 87 → 91 → 88 after four filings and three rotations; `BL-176` filed and not fixed).
`named_absorbed()` lists exactly one release commit for each of the three ids at `0.507.0`; the
impossible-id control for that channel is NOT `PC-S999-NEVER`, which docs commits quote. No
correction release. **The fifth batch opened by a peer's handoff** (action 9's message from
`ai-dlc-f2`), so it scoped itself from its own ranking and stated the choice in its first ping:
the sweep was empty at open, the top-ranked standing candidate was refuted as by-design (below),
and the three `PC-S305-*` siblings — none touching a bootstrapping file — were batched.

**THE CONSUMER FILED MID-BATCH, AND THE FILING IS A DEFECT THIS DISTRIBUTION FIXED TWO
RELEASES AGO.** `PC-S308-GATE-REMEDIATION-GUARD-BLIND-TO-SUPPRESSED` entered the ledger on
2026-09-05 (md5 `8c904ef9…` → `773146d7…`, live 76 → 77, the consumer's HEAD `fd2dadbec` →
`3293d29a9`, one commit closing a sprint-308 gate under a `SUPPRESSED` entry): the remediation
guard locks the lead out on a raw FAIL that Check 26 has already passed under an in-force
suppression. That is `BL-169`, arm 7b of the guard, shipped at `v0.505.0` — the consumer runs
`0.504.0` and is paying a remediator dispatch per suppressed gate until it pulls. The archived
`BL-169` entry now cites the id, so the partition reads it DISCHARGED; no release commit names
it, because the fix's release is `0.505.0` and a later commit naming it would mis-attribute.
**This makes the pull REQUIRED on this batch's own evidence**, not merely on batch 55's: the
consumer is working around a shipped fix today. A pull is operator-initiated; readiness is not
authorization. Report it and stop there.

**THE TOP-RANKED CANDIDATE IS RECOMMENDED FOR REJECTION AND THE OPERATOR MUST CARRY IT.**
`PC-S340-CHECK-26-READS-A-PARTIAL-RE-VERIFY-VERDICT-FILE-AS-UNADJUDICATED` asks for a script
that merges a prior pass's verdicts with a targeted re-verify's. `team-roles/gate-adjudicator.md`
requires the verdict's check-id set to equal the derived escalated set exactly, and
`gate-validation.md`'s escalation preamble says a re-dispatch re-derives EVERY check under a
fresh nonce and never carries forward, cites or reconciles against a superseded dispatch. The
consumer's two-check re-verify and its hand-written merged verdict file (a round nonce no
dispatch generated) are the acts that rule forbids, and the merge script would institutionalise
them. The adversarial hand tried to overturn it and upheld it, narrowed to those two acts; the
two upstream gaps it exposed are closed (the dispatch procedure now says a re-dispatch is a whole
new dispatch) and filed (`BL-176`: nothing binds a verdict file to the dispatch that wrote it,
and the guard picks the live verdict by lexical stem order). **An adjudication is delivered by a
person**: the id is deliberately NOT named in the release commit, so the ledger join reads no
absorption; the CHANGELOG's "Not taken" section carries the reasoning for the brief. Ask the
operator to carry the rejection into a graph session; until then it stays live and unfiled.

**THE FIRING SET OF THE NEW CHECK IS WIDER THAN THE DEFECT, MEASURED, AND THE FILING'S TITLE
NAMED ONE INSTANCE OF A WIDER CLASS.** The census hand split the consumer's sixteen
behind-at-cut retro branches with two independent discriminators (a commit in the branch's
range touching a path outside the retro write-set; a commit GitHub associates with another pull
request), controlled on the incident and on a known-clean PR: twelve had inherited another
pull request's history, and the leftover ref was a carry-over or `ai-dlc-update` reconcile
branch more often than the sprint's feature branch; four were trunk cuts main moved past. My
own first discriminator — "is the first commit's parent an ancestor of origin/main" — was TRUE
for all sixteen and for every PR in the population, and would have shipped as a measurement.

**THE ADVERSARY'S FINDINGS COST ONE FIX COMMIT AND NO CORRECTION RELEASE, BECAUSE THE HAND
DELIVERED BEFORE THE MERGE.** A stale local `origin/main` read the exact defect world as 0 (the
squash's branch point is an ancestor of the leftover ref by construction), so Check 7 now fetches
the ref itself where a remote named `origin` exists and states on its line whether it could — a
fetch that lived only in step prose was the omission class the check exists to catch. The
`used to` article window crossed a sentence boundary and a blanked backticked subject fell
through to the previous line; both repaired and driven on sixteen constructed cases, and the
census hand's independent labels agree with the shipped predicate on every line of both corpora.
The "one command" cost claimed for the benign class was false for a trunk that moved on a file
the retro touched, so the failure names re-cutting first. The step's `--transcript-dir` mandate
had drifted tighter than the two validators' own usage text, which now says the gate requires
it. Every hand's payload truncated at ~16000 characters; each tail was recovered by asking for
one named section under a stated size. The first push exited 141 with every gate green and the
ref NOT on origin (`ls-remote` 0 against a control of 1); the second push landed it.

**THE CONSUMER-SIDE DIFFERENTIAL DIVERGES ON THE CONSUMER'S REAL TREE TODAY.** The shipped
validator, driven read-only from the consumer root on its current carry-over branch, reads
`CHECK 7: FAIL — HEAD is 1 commit(s) behind origin/main` where the installed `0.504.0` copy has
no Check 7 (`cmp -s` control: the copies differ); the shipped audit reports 33 findings where the
installed one reports 36, the three being the purpose-shaped `used to` sites. That branch is not
a retro branch, so nothing runs the validator there in anger. The consumer's porcelain read 6 at
open and 10 at close — its sprint-308 pipeline is LIVE and writing `_bmad-output/` state — with
no write to the consumer by this session or its five hands.

**THE LEDGER MOVED ONCE, BY THE CONSUMER.** md5 `773146d7…` at close. **Live 77, cited 53,
unfiled 24** (three ids moved from unfiled to cited by the batch's citations; the one filed
mid-batch entered and left unfiled via the archived `BL-169`), archive 154, partition control
0. Goal partition **26 DISCHARGED / 27 in flight / 24 untouched**, overlap 3,
discharged-but-invisible 0 — the mid-batch filing is named only by the docs commit that cited
it, with its `0.505.0` attribution in the message, never by a release commit — TERMINAL 43.
Gap THREE (`0.504.0` installed, `0.507.0`
shipped), PENDING 7 (`BL-169`, `BL-170` at `0.505.0`; `BL-172` at `0.506.0`; `BL-173`, `BL-174`,
`BL-175` at `0.507.0`; the new filing rides on `BL-169`; six `PC-` ids among them). Say WIDE at
five. The range `6fc38a46..origin/main` still carries the batch-56 bootstrapping files
(`ledger-reverify.sh`, the update skill); this batch added none — 37 core paths at the release
commit, 0 mode-only changes against a control of 4 mode changes that are adds.

**TELL THE CONSUMER, IN THE NEXT BRIEF,** that its 2026-09-05 filing is `BL-169`, shipped at
`0.505.0`, and the remediator workaround ends with the pull; that `0.507.0` adds Check 7 to
`validate-mandatory-rules.sh` (a retro branch behind `origin/main` fails Step 5c; the check
fetches `origin main` itself and says on its line whether it could; the fix is to cut the retro
branch from `main` at `origin/main`, which `retro.md` Step 1 now prescribes, and sixteen of its
own past retro branches were cut behind); that its `validate-retro-compliance.yml` and any local
copy of the skip-accounting fixture expect the summary to say 7 checks; that the audit's
`used to` false positives in `coding-conventions.md` and its gate-validation extension clear on
the pull; that `--transcript-dir` is stated as MANDATORY at both gate sites; that the dispatch
procedure now says a re-dispatch is a whole new dispatch and its hand-merged
`planning-20260902T160000Z` verdict file is the shape `BL-176` names; and that every earlier
brief item in the batch-56 and batch-55 blocks below still stands.

**THE SWEEP IS EMPTY OF UNFIXED NEW FILINGS FOR BATCH 58 — RANK THE STANDING CORPUS, AND IF A
PEER HANDED YOU THIS PLAN, TAKE THE TOP ONE(S) WITHOUT ASKING.** Check the ledger's md5 first —
the consumer files mid-batch and did so twice in two batches. The 24 unfiled ids are the
standing corpus enumerated in action 1b and the batch-39 paragraph of action 1; batch 57 read
every status line: the three `PC-S307-*` are discharged-unfiled; the nine `PC-S312-*` are
consumer-local retirement probes; the two `PC-S297-*` ask core to adopt consumer scripts;
`PC-S303-POSTCOMPACT-…` records in its own status line that it was remediated at `9cbb77f` (on
`origin/main` since `0.374.0`) and awaits the consumer's re-disposition — carry that with the
rejection above; `PC-S340-AUDIT-RULE-FILES-DRIFT-…` is a per-file judgement, not one subject;
`PC-S340-RETRO-AUDIT-SCANS-FIXTURE-FAILS-ONCE-…` is intermittency. What is left ranks, by
consequence: `PC-S337-RETIRED-TOKENS-CANNOT-SAY-IT-SCANNED-NOTHING` (a silent vacuous scan on a
pull, one stderr NOTE, mechanical receipt; its subject is the update skill's reconcile, so it
ships ALONE under action 2), `PC-S305-UNION-GATE-UNPASSABLE-ON-ANY-PULL-THAT-HAD-A-BLOCKER`
(SKILL.md step 7 — bootstrapping, alone; re-derive whether the union gate's clause (1) still
precedes the adjudication loop before believing the filing), `PC-S335-NO-DETECTOR-REACHES-A-
RETIRED-STATUS-TOKEN-…` (a new report-only detector in reconcile), and the three
`PC-S337-RETRO-*` extension drains (prose additions to `retro.md`, `verify: manual`, batchable).
`BL-159`, `BL-161`, `BL-163`, `BL-171`, `BL-176` (no `PC-` id) rank below any PC-backed one;
`BL-171` and `BL-176` are the two with a consumer consequence today.

### BATCH 56 SHIPPED AS `v0.506.0` — THE BLOCK ABOVE REPLACES THIS ONE. TAKE THE STATE FROM THERE.
...
**`VERSION` IS `0.505.0`, SO BATCH 56 RELEASES AS `0.506.0`.** Re-derive `VERSION` and add one.
Batch 55 merged at `288d0e53` (PR #635, squash; on the branch the release commit came last after
seventeen fix, test and docs commits, none naming an id), closing
`PC-S308-RULE-11-AMBIGUITY-QUESTIONS-HAVE-NO-MANDATED-PRESENTATION-TOOL` as `BL-170` and the
no-`PC` `BL-169`, both rotated (archive 82 → 84, live 89 → 87 after two filings and two
rotations; `BL-171` filed, not fixed). `named_absorbed()` lists exactly one release commit for the
id at `0.505.0`; the impossible-id control for that channel is NOT `PC-S999-NEVER`, which two docs
commits quote. No correction release. **The third batch opened by a peer's handoff** (action 9's
message from `ai-dlc-1b`), so it scoped itself from its own ranking and stated the choice in its
first ping: RULE-11 as the only PC-backed unfiled candidate, `BL-169` batched with it as the one
no-`PC` entry with a measured consumer consequence and a one-call remedy.

**THE CENSUS REFUTED THE FILING'S SUGGESTED SITE BEFORE ANYTHING WAS BUILT, AND THE ADVERSARIES
THEN FOUND THREE HOLES, EACH IN A DIFFERENT DIRECTION.** The read-only census over the consumer's
247 transcripts (3660 turn-ends, control 138 turns carrying the tool): the predicate fires 97
times, 84 genuine questions, and the incident's burial turn ended with the pause flag DOWN on the
Stop hook's default block path, whose reason told the lead to create the flag — so an arm sited
only in the pause-flag branch, which is what the filing's shape produces, cannot fire on its own
motivating case; 32 of 97 fires and 10 of 53 real decisions sit there. The predicate is now read
in both branches. Pause points (b) and (c) end the turn with a prose question by design (7 of 65
flag-up fires), so the flag-up branch warns with a neutral `systemMessage` and never blocks.
Then: the RULE-11 adversary timed the first cut at sixteen seconds per Stop on the consumer's
largest transcript (a whitespace `gsub` over every record, paid before Check 1 on every exit),
found a wrong fix counting ANY tool use that passed fixture and receipt and would acquit 65 of 96
fires, and found the hook's own header still naming a Rule 11(a) HARD_BLOCK as a flag case; the
BL-169 adversary drove a lead-authored `SUPPRESSED` entry through the guard end to end (arm 6
lets the lead write `pending.md`; nothing verified the quote) and found the cache key blind to a
same-size rewrite inside one second. All closed before the merge; the guard now verifies each
covering quote with `--cite` and Check 26's half is `BL-171`. Every hand delivered before the
merge; four payloads truncated, each recovered by asking for the tail by name. The census hand's
turn definition was independently reproduced by the adversary (3601 / 96 against 3660 / 97).

**THE CONSUMER PULLED MID-BATCH, AND THE RESUME BLOCK SAID GAP TWO.** Its stamp reads `0.504.0` /
`6fc38a46` on all four fields (PR #1015 at 15:39, while this batch's hands were out), which
rotated three of the four batch-53/54 ids into its archive and left
`PC-S308-VALIDATE-ADVERSARIAL-CONVERGENCE-SCOPE-GREW-MISFIRES-ON-PASS-1` LIVE — the pull did not
rotate it, so it reads DISCHARGED here and cited there. The same pull filed
`PC-S308-LEDGER-REVERIFY-READS-ESCAPED-BACKTICKS-LITERALLY`. **Check the delivery gap BEFORE
reading this block's figure for it** — the sixth time the gap has closed in this program and the
fifth time it reopened on the very next release. The consumer's porcelain read 4 at open and 6 at
close, every path a pipeline-state file written by its own session; no write to the consumer by
this session. Its HEAD is `fd2dadbec` on `ai-dlc/carry-over/pool-pnl-backlog-triage`; peer
`graph-0f` was busy throughout and was not messaged.

**THE CONSUMER SIDE IS MEASURED ON REAL FILES, READ-ONLY, ON SCRATCH COPIES.** On its live pass
by the guard's own nonce pick (`implementation-20260905T172547Z`, catalog `core`, one FAIL, `16`,
covered by `[S308-GATE3-STORY-1]`) with a `cmp -s` control that the two guards differ: installed
denies the lead's edit, fixed allows, against a known-positive pass where both deny. That
differential was taken before the citation check landed; the entry's quote was then verified
separately, `validate-steering-budget.sh --cite "Suppress Check 16 (Recommended)"` over the
consumer's 249 transcripts → `MATCH`, control phrase `NOMATCH`. The two readings have not been
taken in one invocation on the final guard; the first thing batch 56 measures is that.

**THE LEDGER MOVED ONCE, ON THE CONSUMER'S PULL.** md5 `3724a5b3…` at open, `8c904ef9…` after the
pull and at close. **Live 78 → 76, cited 50 → 48, unfiled 28 → 28** (three rotated out of cited,
RULE-11 in by citation, one new filing in unfiled), archive 151 → 154, partition control 0. Goal
partition **21 DISCHARGED / 27 in flight / 28 untouched**, overlap 3, discharged-but-invisible 0,
TERMINAL **42**. Gap ONE (`0.504.0` installed, `0.505.0` shipped), PENDING 2 (`BL-169`, `BL-170`
at `0.505.0`; one `PC-` id between them). Say WIDE at five. The second test diverges today on the
consumer's live pass (installed deny, fixed allow, above), and its HEAD commit records a handoff
with the lead still locked out — the pull is REQUIRED, not merely owed. A pull is
operator-initiated; readiness is not authorization.

**TELL THE CONSUMER, IN THE NEXT BRIEF,** that `0.505.0` lifts its remediation lock-out on the
live pass once pulled, provided the suppression's operator quote verifies against the transcript
corpus its `transcript_path` sits in (it does today); that a Rule 11(a) ambiguity question is now
put with `AskUserQuestion` and sets NO pause flag, while PVC and retro prompts still set the flag
and end the turn; that a session opening under a persisted pause flag whose greeting ends in a
question mark will get the neutral pointer; that Check 26 still accepts a `SUPPRESSED` entry
whose quote nothing verifies (`BL-171`); that the `--in-force` row is now six fields; that
`_bmad-output/.gate-remediation-in-force` is a new transient path its `.gitignore` will need
beside `BL-159`'s two; that its `QUICKSTART.md` still says three pause points; and that every
earlier brief item in the batch-54 block below still stands.

**ONE NEW `PC-S308-*` CANDIDATE IS UNFILED FOR BATCH 56, THEN THE STANDING CORPUS — RANK THEM, AND
IF A PEER HANDED YOU THIS PLAN, TAKE THE TOP ONE(S) WITHOUT ASKING.**
`PC-S308-LEDGER-REVERIFY-READS-ESCAPED-BACKTICKS-LITERALLY` (2026-09-05, filed on the consumer's
`0.502.0 → 0.504.0` pull, `verify: manual`): a `theirs_has` receipt whose quoted substring carries
markdown-escaped backticks is matched with the backslashes in it, so `BL-168`'s own candidate read
NEEDS-REVIEW "absent at BOTH refs" while the unescaped substring was present at base and gone at
theirs — a CLOSE-CANDIDATE reported as vacuous, in the direction that keeps an absorbed entry open
forever. The filing offers two readings (grammar says backticks are literal and authors must not
escape; or the reader unescapes) and asks upstream to choose. Its status line was read by this
session; its receipt is manual. **Its subject is `ledger-reverify.sh`, a BOOTSTRAPPING file, so it
ships ALONE under action 2** and the consumer's INSTALLED reader runs the pull that delivers the
fix. The other 27 unfiled ids are the standing corpus enumerated in action 1b and the batch-39
paragraph of action 1, none filed since 2026-08-31. `BL-159`, `BL-161`, `BL-163`, `BL-171` (no
`PC-` id) rank below the PC-backed one; `BL-171` is the one with a consumer consequence today.

### BATCH 54 SHIPPED AS `v0.504.0` — THE BLOCK ABOVE REPLACES THIS ONE. TAKE THE STATE FROM THERE.

This block was the batch-55 resume record. It replaced the batch-53 record below it. Every figure here was re-derived on 2026-09-05
after the merge, against the working tree with the controls in the same invocation; re-derive
them again rather than reading them.
...
**`VERSION` IS `0.497.0`, SO BATCH 49 RELEASES AS `0.498.0`.** Re-derive `VERSION` and add one.
Batch 48 merged at `fb2fd0ee` (PR #614, release commit `a6101eae` followed by two correction
commits on the same branch before the merge), closing
`PC-S308-POST-COMPACT-RECOVERY-PROTOCOL-SKIPS-ROUTE-MD` as `BL-157` and rotating it (archive
72 → 73, live 85 → 84). `named_absorbed()` resolves the id to exactly one commit at `0.497.0`,
impossible-id control 0. No correction release: the adversarial hand reported BEFORE the merge
for the third consecutive batch, and all three of its defects were fixed on the branch. The
operator chose this subject from the three the sweep reported.

**THE DEFECT WAS A PROSE ORDERING, AND ITS POPULATION WAS NOT THE ONE ITS TITLE NAMES.**
`SKILL.md`'s recovery protocol sits at line 26 and the unconditional router Read sits under
`## INITIALIZATION` roughly 1,800 lines below, so a session started with `/ai-dlc resume` read the
protocol as its resume procedure, proceeded to the next pipeline action, never read
`steps/route.md`, skipped Step 0a, and was denied its first write by Check 2z. Measured on the
consumer's transcripts before building: 10 of 41 `/ai-dlc resume` sessions since 2026-09-03 denied
before any router Read, all 10 having read the snapshot first; 0 of 51 sessions carrying a
compaction summary denied after it (the adversary's independent scan: 0 of 58). The protocol now
opens by stating its scope and sending an un-routed session to the router;
`validate-reattach-budget.sh` gained an arm requiring the installed router path inside the
section outside an HTML comment; `postcompact-rulebook-recovery` carries three mutants and a
bracketed-path control. The recover hook's injected block is untouched.

**THE ADVERSARIAL HAND'S THREE DEFECTS, ALL REAL, ALL FIXED PRE-MERGE.** The comment guard was
`sed 's/<!--.*-->//'` and 18 of the 20 HTML comments in `SKILL.md` span lines, so a path parked
in the house-style comment passed — **a comment-stripping guard must match the comment form the
FILE actually uses**; it is now an awk state machine, and the greedy `.*` that ate a live path
between two comments went with it. The hook-authority sentence ("this section adds nothing")
revoked the new paragraph for a post-compaction lead; narrowed to the recovery steps below it.
The paragraph's condition keyed on transcript state a compacted lead cannot evaluate, and a lead
resolving the doubt by re-routing would be denied by `recover-gate.sh` and paused by Step 0a; it
now keys on the session's INPUT and tells a compaction-recovering lead not to re-route. The hand
also found mutant 7b's `cmp -s` guard could not fire for its stated reason (INITIALIZATION's copy
of the path kept the file differing); it now counts inside the section.

**TWO TOOL FACTS, ONE OF THEM A REPEAT.** A heredoc inside `bash -c '…'` whose body carried
backticks mangled the release commit message to five lines — the SECOND time this file records
that hazard, so it is now in `tool-hazards.md` rather than only here; write the body with the Write
tool and pass `-F`. `scripts/render-postcompact-digest.sh` without `--write` exits 0 and writes
nothing, which read as "re-rendered" and cost one gate run; the gate's digest check caught it.

**THE CONSUMER PULLED NOTHING THIS BATCH, AND THE GAP IS NOW WIDE.** Stamp `0.492.0` /
`2f1ee8bc` against `VERSION` `0.497.0`: gap FIVE, PENDING 4 (`PC-S309` at `0.493.0`, corrected
`0.494.0`; `PC-S337` at `0.495.0`; `PC-S308-AI-DLC-ACKNOWLEDGE-…` at `0.496.0`;
`PC-S308-POST-COMPACT-…` at `0.497.0`). FIVE is the line this file calls wide: more paths
adjudicated in one session and a bigger blast radius. `0.497.0` changes `SKILL.md`, a validator,
a fixture and the digest — no bootstrapping file — so the floor for the next brief stays
`0.494.0`. The second test is banked: the consumer's installed `SKILL.md` is byte-identical to the
pre-fix distribution copy and its installed `validate-reattach-budget.sh` passes it, while the
shipped validator fails it on the router arm (`cmp -s` control: the two validators differ). A
pull is operator-initiated; readiness is not authorization.

**THE LEDGER MOVED ONCE AND IS COMMITTED.** md5 `ea30c200…` → `0dd318bd…`, **live 78, cited 48,
unfiled 30**, archive 145, partition control 0: the consumer committed the POST-COMPACT filing
(`c2ddf7554`) and filed `PC-S308-HANDOFF-PROCEDURE-5-STEP-NOT-FOLLOWED` (`6d645a346`, 2026-09-04).
Goal partition **21 DISCHARGED / 27 in flight / 30 untouched**, overlap 3, discharged-but-invisible
0, TERMINAL **33**. The consumer sat on `pool-pnl-backlog-triage` with porcelain 7–8, graph
sessions `graph-f1` and `graph-c4` seen and neither contacted, no write to `/Users/n8/git/graph` by
this session.

**BATCH 49 IS SCOPED, BY RULING, ON `PC-S308-HANDOFF-PROCEDURE-5-STEP-NOT-FOLLOWED`, WITH THE
OPERATOR'S CONTEXT THAT THE HANDOFF MISS "HAPPENS OFTEN, IT'S JUST NEVER BEEN FILED BEFORE".** Two
read-only hands did the preliminary work during batch 48's close; their findings are HYPOTHESES to
re-derive, and action 1 says which first.

- **Population (census hand).** 120 handoff commits across all consumer refs (control: 8409
  commits), but the transcript corpus spans only 2026-08-04 → 2026-09-04, so 39 are scorable and
  every rate below is over ≤ 39. Step 3 (`git push`) skipped 3 of 39. Step 4's
  `touch _bmad-output/.driver/handoff` skipped **20 of 39** — the most-skipped step by six times,
  and the one the filing ranks third. Step 5's `rm -f .handoff-in-progress` skipped 4 of 17 (the
  clause entered `handoff.md` at `v0.447.0` on 2026-08-30; earlier rows are out of scope). Step-4
  skips ran 7 of 7 in the first half of August, 12 of 17 in the second, 1 of 15 in September, and
  the turn coincides with the operator starting to ask — 11 "did the handoff steps run" prompts
  across 397 transcripts, all on or after 2026-08-30 — not with any mechanism: no hook reads
  `.driver/handoff`. The scoring credits a step run within 25 minutes of the commit, so
  operator-prompted remediation counts as run and the unprompted rate is worse than every figure.
- **Mechanism (mechanism hand).** `core/hooks/ai-dlc-continue.sh` Check 0, a `Stop` hook, already
  blocks the stop on three arms — step 1's teammate sweep (`:316-349`), step 3's push
  (`:369-383`, `@{u}..HEAD`), step 4's resume block (`:307-310`) — and the consumer's copy is
  byte-identical to core. **No arm exists for step 4's touch or step 5's marker clear**, and the
  marker is a TRIGGER (key 1 of `ai_dlc_handoff_pending`, `ai-dlc-handoff-pending.sh:87`), not an
  assertion: a stop with the marker still on disk is allowed once the three arms pass.
- **THE INCIDENT NEVER ARMED THE GUARD, AND THAT IS THE HALF THE FILING DOES NOT NAME.** At the
  2026-09-04 06:52 handoff the log records `ALLOWED_BY_PAUSE` and no guard row. Arming needs the
  pause flag plus one of three keys: the entry marker (written by `ai-dlc-handoff-entry.sh` on a
  READ of `handoff.md` — the lead never read it), an operator handoff request in the log (the lead
  self-initiated), or the snapshot key (did not hold). A self-initiated handoff that skips the step
  file is invisible to the guard by construction. The fix has two halves — the arming predicate and
  the missing arms — and shipping the arms alone leaves this incident unreached.
- **Four adjacent defects, none of them the subject.** `PUSH_OK` counts COMMITS, so a skipped step
  2 reads `ahead 0` and passes (consumer today: ahead 0, 7 dirty files; `handoff.md:64-65` names
  the trap in prose). `_bmad-output/.handoff-in-progress` is TRACKED in the consumer although
  `core/schemas/pipeline-state-paths.json:168-172` declares it ignored — 2 of the schema's 14
  transient patterns are missing from the consumer's `.gitignore`, and `sync-transient-ignore.sh:95`
  is the producer that should have written them. `_gate-procedures.md:681-687`'s auto-handoff twin
  never clears the marker. Check 0's on-disk trigger sits behind a readable-transcript test
  (`:288`), and the guard self-releases after `MAX_RAPID_BLOCKS` = 3 (a delay, never a wedge; the
  consumer's log shows a `(3/3)` exhaustion).
- **This is the THIRD filing of one end state.** `PC-S307-HANDOFF-PUSH-IS-A-BARE-GIT-PUSH-…`
  (`BL-120`, `v0.434.0`) and `PC-S307-POSTCOMPACT-RECOVERY-NEVER-ROUTES-TO-HANDOFF-MD` are both
  archived on both sides; `BL-125` (`v0.438.0`) fixed the resume line; each release removed one
  route to the same end state and the state returned by another. None covers steps 4 or 5.
- **Siting and fixture.** The mechanism hand recommends a fourth arm inside Check 0 — the seam
  every handoff already reaches — asserting the marker is gone and `.driver/handoff` present once
  the other three arms are green, with `ai-dlc-driver-signal.sh` rejected as a site (its header
  says it decides nothing) and a `handoff.md` step rejected as the mechanism that already failed.
  `core/fixtures/handoff-completion-assertion/` owns the seam (19 mutants; `run.sh:153` warns not
  to seed the marker by touching it). The candidate's receipt, `theirs_has … "git push -u origin
  HEAD"`, is satisfied at both refs and reads STILL-LIVE through any fix.
- **Two verified `PC-S308-*` siblings remain available**: the indented-fence blind spot and the
  SCOPE_GREW pass-1 misfire, both reproduced at batch 47 and 48 and unchanged.

### BATCH 47 SHIPPED AS `v0.496.0` — THE BLOCK ABOVE REPLACES THIS ONE. TAKE THE STATE FROM THERE.

This block was the batch-48 resume record. Every figure here was re-derived on 2026-09-04
after the merge, against the working tree with the controls in the same invocation; re-derive
them again rather than reading them.
...
**`VERSION` IS `0.495.0`, SO BATCH 47 RELEASES AS `0.496.0`.** Re-derive `VERSION` and add one.
Batch 46 merged at `fec5c5de` (PR #608, release commit `ff368078`), closing
`PC-S337-ROTATE-ACCEPTANCE-TEST-FALSE-FAILS-WHEN-A-PREFIX-CROSSES-THE-ONE-VS-MANY-THRESHOLD` as
`BL-154` and rotating it (archive 69 → 71, live 86 → 84 — the second rotation is `BL-131`,
NARROWED on an operator ruling against `v0.488.0`, with its step-2 autonomous self-update path
filed as `BL-155`). `named_absorbed()` resolves the id to exactly one commit at `0.495.0`. No
correction release: both adversarial hands reported BEFORE the merge, for the first time in this
program, and the fixture hand's blocker — two existing arms in `ledger-rotate/run.sh` asserted the
DEFECT as a requirement (bytes must differ, count must rise `2 -> 3`) and went red on the fix —
was fixed in the release commit rather than one later.

**THE DEFECT WAS A TRANSIENT, AND THE MEASUREMENT THAT HAD REJECTED THE FIX WAS TAKEN AT REST.**
`BL-068` (`v0.377.0`) built the same counter widening, counted zero verdict flips on the consumer's
ledger AS IT STOOD, and rejected it. A ledger at rest has no annotated-but-unrotated entry, so the
population could not contain the state the defect lives in. The consumer's diff was taken ACROSS a
rotation. **And this session's own census to reopen it was wrong in the other direction**: it
predicted six flips at full-drain size and the shipping program returned an identical row set —
a full drain annotates every member of every prefix, so no row survives to flip. The
discriminating input is a PARTIAL drain over a two-member prefix, which is what the consumer's
filing was. Drive the shipping program; a census is a second implementation. Current exposure on
the reference consumer at rest is nil (0 of 21 prefixes have exactly two corpus members; control 4
of 21 with one), which is a corpus property and not a reason to have left it.

**`theirs_has A B` IS A DISJUNCTION OF TWO CLOSE CONDITIONS, NOT A CONJUNCTION.** `all_present()`
is an AND over substrings, so the row closes the moment ANY one is absent. The consumer's receipt
note reasoned "both must be present for the entry to be live" and concluded the opposite. Both of
its substrings sat at column 6 of wrapped lines in `SKILL.md`, and 12 of 61 reflows closed it with
the defect verbatim, while a legitimate fix keeping both clauses read STILL-LIVE. It will read
`CLOSE-CANDIDATE` on the next pull for a correct reason — the second substring is gone — and the
consumer should still re-anchor it; `BL-154`'s receipt drives the shipping reverify and rotator and
is the shape to offer.

**THE CONSUMER PULLED NOTHING THIS BATCH.** Stamp `0.492.0` / `2f1ee8bc` on all four fields
against `VERSION` `0.495.0`: gap THREE, PENDING 2 (`PC-S309` at `0.493.0`, corrected `0.494.0`;
`PC-S337` at `0.495.0`). `0.493.0`/`0.494.0` change `apply.sh`, a bootstrapping file, so the
floor for the next brief is `0.494.0`; `0.495.0` changes `ledger-reverify.sh` and
`ledger-rotate.sh`, which step 8 runs AFTER the machinery slice lands, so the fix protects the
pull that delivers it. The second test is banked for this batch: the consumer's installed
`ledger-reverify.sh` is byte-identical to the pre-fix distribution copy (`cmp -s`, control: the
fixed copy differs), and the differential on a copy of its real ledger at rest is 116 rows either
side, 0 row-set changes, 3 displayed counts moved — the null's limit is that the transient is
unobservable at rest, stated rather than hidden. A pull is operator-initiated; readiness is not
authorization.

**THE LEDGER MOVED THREE TIMES DURING THIS BATCH AND NONE OF THEM WAS THIS SESSION.** md5
`30b64caf…` → `5d1fd6ac…` → `d8902b6c…`, porcelain 4 → 8 → 2 → 5, a graph session
(`graph-96`) live throughout, this session issuing no write to `/Users/n8/git/graph` (every probe
copied the ledger out to `mktemp`). **Live 73 → 76, cited 45 → 46, unfiled 28 → 30**, archive 145.
Three new candidates, all filed 2026-09-03 by sprint-308 sessions:
`PC-S308-VALIDATE-ARTIFACT-DERIVATIONS-INDENTED-FENCE-BLIND-SPOT`,
`PC-S308-AI-DLC-ACKNOWLEDGE-ROUTE-DENIED-SUBAGENT-CANNOT-CLEAR`, and
`PC-S308-VALIDATE-ADVERSARIAL-CONVERGENCE-SCOPE-GREW-MISFIRES-ON-PASS-1` (the third landed after
the merge and has not been read). The first two carry `theirs_has` receipts naming CONSUMER-shaped
paths (`core/scripts/ai-dlc/…`, `core/.claude/hooks/…`) that resolve to nothing in this tree — the
real files are `core/scripts/validate-artifact-derivations.sh` and
`core/hooks/ai-dlc-acknowledge.sh` — so both are vacuous at both refs and will read
`NEEDS-REVIEW`. The indented-fence defect reproduces here in one command with a control: a
```` ```derived ```` fence indented under a list item reports `0 block(s)` and `OK`, the same content
unindented reports `1 block(s)` — this repo's signature class. Goal partition **19 DISCHARGED /
27 in flight / 30 untouched**, TERMINAL **33**. The operator was asked mid-batch whether to switch
to either of the first two and chose to finish `PC-S337`; that answer was about batch 46's scope
and is NOT a ruling on batch 47's. **Report the three and ask.**

**`PC-S999-NEVER` IS NOT AN IMPOSSIBLE ID ON THE COMMIT-MESSAGE CHANNEL.** `git log -F --grep`
returns 2 for it on `origin/main`, because docs commits quote the derive block. Action 1 already
says so; this batch tripped it anyway. Use an id no commit has ever quoted.

**TWO SMALL TOOL FACTS.** A markdown section with backticks cannot be written through a heredoc
inside `bash -c '…'` — the outer quoting breaks and nothing is written; use the Write tool and
splice. `git cherry-pick` has no `-q`.

### BATCH 45 SHIPPED AS `v0.493.0`, CORRECTED BY `v0.494.0` — THE BLOCK ABOVE REPLACES THIS ONE. TAKE THE STATE FROM THERE.

This block was the batch-46 resume record. Every figure here was re-derived on 2026-09-03
after the merge, against the working tree with the controls in the same invocation; re-derive
them again rather than reading them.
...
1. **CHECK `ListAgents` FIRST, RUN THE SWEEP (action 1b below), RANK THE UNFILED CANDIDATES,
   ...
   **BATCH 81's TWO SUBJECTS ARE SHIPPED — do not re-scope either of them.**
   `PC-S309-STORY-NO-SPRINT-REMEDY-IS-RENAME-ONLY-WHILE-THE-FIXTURE-RATIONALE-CLAIMS-OUT-OF-BAND-DERIVATION`
   and `PC-S309-VALIDATE-MANDATORY-RULES-CHECK5-TEST-ONLY-WEB-DIFF-FALSE-FAIL` shipped as ONE
   release, `v0.542.0` at `732db5a5`, both cited verbatim in the release commit message. Neither
   has a `BL-` entry to rotate — both came straight from the consumer's ledger.

   **TWO OF THE THREE MID-BATCH FILINGS REMAIN AVAILABLE AND BOTH ARE ADJUDICATED NOT-READY.**
   `ADR-DEFERRED-WORK-HAS-NO-CARRIER-INTO-BACKLOG` is refuted at its headline and has no
   constructible enforcer (`BL-215`); `RETRO-CLOSURE-QUOTATION-NOT-VERBATIM-BOUND` is real but its
   filed remedy is a false negative on its own motivating case, and choosing between the two
   ...
   **THE READIEST NO-`PC` ENTRY IS `BL-137`, AND ITS SEPARABILITY IS ALREADY MEASURED** in both
   ...
   HEAD rather than the 4 it records, 3 of them in bootstrapping files, and its receipt is a
   corpus-wide count that cannot close without touching them. `BL-091` writes `docs/backlog.md`
   across 44 headings and therefore CONTENDS with the batch's own filings and rotation — do not
   batch it with anything that files an entry.
   ...
   **BATCH 79's TWO SUBJECTS ARE SHIPPED AND ROTATED — do not re-scope either of them.** `BL-086`
   ...
   `PC-S309-STUB-AUDIT-PROSE-MARKER-FIRES-ON-NEGATION-AND-TEST-DOUBLE-REFERENCE` verbatim; both
   rotated. **Every gate-green branch this batch carried a finding an adversary had to
   surface** — read the batch-79 resume block before trusting a warm-cost figure or a receipt
   that no built wrong fix has been scored against. The readiest no-`PC` entries now are
   `BL-195`, `BL-087`, `BL-088`; run the sweep first, a PC-backed candidate outranks all three.

   **BATCH 78's TWO SUBJECTS ARE SHIPPED AND ROTATED — do not re-scope either of them.** `BL-155`
   and `BL-212` shipped as ONE release, `v0.538.0` (PR #690), neither carrying a `PC-` id, both
   rotated. They batched because both live in `self-update-gate.sh` and neither receipt is
   closed by the other's fix; the release touches three bootstrapping files, so it shipped with
   nothing else. **The first integrated cut passed the full gate green and would have refused
   every legitimate self-update** — read the batch-78 resume block's four refutations before
   writing any join keyed on "unchanged since"; the discriminator is WHICH input moved and
   TOWARD WHAT. The readiest no-`PC` entries now are `BL-195` and `BL-086`; run the sweep first,
   a PC-backed candidate outranks both.
   ...
   **BATCH 76's SUBJECT IS SHIPPED AND ROTATED — do not re-scope it.** `BL-113` shipped ALONE as
   ...
   is taken (0 of 234) and its receipt was rewritten at close because a comment closed the
   original. **`BL-122` and `BL-148` are ARCHIVED** (`LANDED (v0.523.0)`), so the ranking that
   earlier paragraphs below carry — "then `BL-155`, `BL-122`, `BL-148`" — names two entries the
   sweep cannot find live; the readiest no-`PC` entries now are `BL-155`, `BL-188`, `BL-189`,
   `BL-195`. Run the sweep first; a PC-backed candidate outranks all of them.

   **BATCH 75's TWO SUBJECTS ARE SHIPPED AND ROTATED — do not re-scope either of them.**
   `PC-S309-READOPT-STAMP-REFUSES-A-CLEAN-MERGE-WHEN-A-DELETED-CORE-LINE-IS-A-SUFFIX-OF-ITS-REFLOWED-REPLACEMENT`
   (`BL-209`) shipped ALONE as `v0.534.0` at `327f1d78` (PR #684) and
   `PC-S343-ADJUDICATION-ROW-WITHHOLDS-THE-ONE-FIELD-ITS-RECORD-REQUIRES` (`BL-210`) shipped ALONE
   as `v0.535.0` at `505c3dc3` (PR #685), both machinery-slice, both cited verbatim in their
   release commit messages, both rotated. **THE AVAILABLE PC-BACKED RESIDUE IS ZERO** — all 18
   unfiled re-derive as adjudicated. Run the sweep anyway; `BL-113` is the readiest no-`PC` entry
   and ships ALONE, and its owed FP measurement is taken and empty.

   **BUILD THE NON-FIXES AND SCORE THE RECEIPT AGAINST EACH BEFORE READING ITS ZERO.** Batch 75's
   receipt accepted the filing's own suggested fix and then the bare-containment first cut, each
   caught only by building the non-fix and running the receipt on it.

   **A SURVIVING MUTANT IS A CLAIM ABOUT THE SEED BEFORE IT IS A CLAIM ABOUT THE ARM.** Two of
   batch 75's mutants survived on a degenerate range and a mis-cased seed respectively; both arms
   now assert their seed's discriminating property before any verdict is read.

   **BATCH 74's THREE SUBJECTS ARE SHIPPED AND ROTATED — do not re-scope any of them.**
   `PC-S309-PROVENANCE-FORBIDDEN-LIST-EXACT-MATCH-MISSES-DECORATED-PLACEHOLDERS` (`BL-199`) and
   `PC-S309-CHECK29-33-NO-SCOPE-CLAUSE-FOR-CARRY-OVER-OPENING-GATE` (`BL-200`) shipped as
   `v0.527.0` at `4ccba0ec` (PR #671); `PC-S342-ADJUDICATION-ROW-PRESCRIBES-AN-ENTRY-EDIT-THAT-SPENDS-ITS-OWN-VERDICT`
   (`BL-201`) shipped ALONE as `v0.528.0` at `d0affb32` (PR #672), because `layer-drift.sh` is in
   the machinery slice — derived by RUNNING the shipping `machinery_paths()`, never by hand.
   All three are cited verbatim in their release commit messages and all three are rotated.
   **THE AVAILABLE PC-BACKED RESIDUE IS ZERO**: all 18 unfiled re-derive as adjudicated — 4 named
   in `origin/main` release commits, 11 `NOT-UPSTREAM` in the adjudication brief, 1 refuted at
   `182817fb`, and 2 WITHDRAWN by the consumer itself. Run the sweep anyway, because the consumer
   filed three candidates in one day during batch 73; expect it empty and rank the no-`PC` set
   without waiting. `BL-113` is the readiest and ships ALONE.

   **READ A CANDIDATE'S OWN STATUS LINE BEFORE SCORING IT AS AVAILABLE.** Two of batch 74's five
   apparently-available candidates were a WITHDRAWN/superseded pair whose titles read like live
   defects and whose bodies say WITHDRAWN in their first sentence.

   **ASK OF EVERY READER FIX WHAT THE WRITER DOES WITH THE SAME VALUE.** `BL-199` corrected a
   validator that refused a forged id; the STAMPER that writes those ids checked only the charset
   pattern and had been writing the refused value onto every story all along. Fixing the reader
   alone would have widened a wedge rather than closing anything.

   **DRIVE `layer-drift.sh --adjudicated-codes` RATHER THAN READING A LEVEL OUT OF THE CONTRACT.**
   `PC-S342`'s filing named a WARN-level row as its sharpest case; the rows that actually BLOCK
   were ones it never mentioned.
   ...
   **ELEVEN WERE ALREADY ADJUDICATED `NOT-UPSTREAM`** in
   ...
   work and none is coming."* Batch 68 derived that join, with controls, and it had never been made
   before — the S312/S297 cluster this file has called unread since batch 1 WAS read; the answer was
   ...
   **BATCH 68's SUBJECT IS SHIPPED AND ROTATED — do not re-scope it.**
   ...
   `v0.520.0` at `69618a55` (PR #662) closing `BL-193`, cited verbatim in the release commit
   message, rotated. Its consumer-owned half stays that consumer's carry-over and is not upstream
   work. **The filing's own split had EXPIRED, and the correction WIDENED the defect** — it claims
   two consumer-fixable files; today all 14 finding files are core-owned and the consumer can fix
   none. Re-derive a filing's split before building to it.
   ...
   **BATCH 66's SUBJECT IS SHIPPED AND ROTATED — do not re-scope it.** `BL-190` closed the
   ...
   CLASS is live as `BL-191`** — 45 files run `git init`, 42 scrub nothing, 8 of 8 fixtures
   clobber when run directly and 6 do it at exit 0 — and it is the strongest no-`PC` alternative.
   Batch 66 is SHIPPED as ONE release — `v0.518.0` at `11ebb342` (PR #659) closing `BL-190`, no
   `PC-` id cited, rotated, `BL-191` filed and `BL-189` amended, no correction release.
   Batch 65 is SHIPPED as ONE release — `v0.517.0` at `db078cbe` (PR #658) closing `BL-185` and
   `BL-186`, BOTH PC-backed, both rotated, `BL-187`/`BL-188`/`BL-189` filed and not fixed, no
   correction release; the two batched because neither receipt was closed by the other's fix,
   scored in both directions across two spellings of each fix. Batch 64 is SHIPPED as ONE release —
   ...
   review landed after the merged branch was green. Batch 62 is SHIPPED as TWO releases — `v0.512.0` at
   ...
   `PC-S305-RETRO-BRANCH-CUT-FROM-STALE-PARENT-NOT-MAIN` closed as `BL-173`,
   `PC-S305-NARRATIVE-DRIFT-FALSE-POSITIVE-USED-TO` as `BL-174` and
   `PC-S305-GATE-VALIDATION-PREFER-IT-SOFT-LANGUAGE` as `BL-175`, all rotated, `BL-176` filed
   and not fixed, no correction release; batch 56 is SHIPPED — `v0.506.0` at `3e59c856`
   ...
   **`BL-131` IS DECIDED AND ROTATED; `BL-155` IS WHAT REMAINS.** The operator ruled at batch 46:
   narrow it and file step 2 separately. `BL-131` closed against `v0.488.0` and is archived;
   `BL-155` carries step 2's autonomous self-update — `core/skills/ai-dlc-update/**` written onto
   a consumer and auto-merged with no approval artifact in the path — with its receipt limits
   stated (prose-keyed, closable by rewording, replace with one that drives the gate once a gate
   exists). It carries no `PC-` id and ranks below any PC-backed subject.
   ...
   **THE GAP IS THREE AND PENDING IS 7** — the consumer's stamp reads `0.504.0` / `6fc38a46` on
   all four fields against a distribution `VERSION` of `0.507.0`; `BL-169` and `BL-170` at
   `0.505.0`, `BL-172` at `0.506.0` and `BL-173`, `BL-174`, `BL-175` at `0.507.0` are the six
   discharged entries it cannot see, five `PC-` ids among them, and the consumer's own
   2026-09-05 filing rides on `BL-169` — it is paying a remediator dispatch per suppressed gate
   for a fix shipped two releases ago, which is what makes the pull REQUIRED on this batch's
   evidence. The consumer pulled `0.502.0 → 0.504.0` on its own WHILE batch 55 was
