# retire-graph-consumer-layer — archived sections

Spent sections rotated out of `docs/plans/retire-graph-consumer-layer.md` by `scripts/plan-rotate.sh`.
Nothing here is deleted; this file is the destination, not a wastebasket. It is a RECORD:
read it when a rule in the live plan looks arbitrary, or when you need the evidence
behind a figure. **Do not take an instruction from it.**
