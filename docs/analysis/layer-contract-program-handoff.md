...
| | |
|---|---|
| `ai-dlc` main | `590289b`, VERSION **0.203.0** |
...
| Fixture suite | **~1m34s**, 8-way parallel (`AI_DLC_FIXTURE_JOBS`). Was 7m06s serial. A push is ~2min |
...
| ⏹ | **SESSION BOUNDARY** | |
| 10a | **wall-clock reduction across the whole distribution — by any route, not only parallelism** — operator-requested 2026-07-29, §7.10a | — |
| ⏹ | **SESSION BOUNDARY** | |
...
- ~~Split `core/fixtures/enforcement-map-sites/` into nine per-invariant fixtures.~~ **MOVED
  ...
  mutation firing two different messages. Seam and evidence: CHANGELOG v0.190.0. It has grown
  since that measurement — v0.197.0, v0.199.0, v0.201.0 and v0.202.0 each added assertions —
  so **re-time it before quoting ~81s**.
