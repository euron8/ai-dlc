...
## BL-319 — a fix story folded into a sprint after its architecture step never reaches an architect

**DEFECT.** Filed by the consumer as `PC-S313-FOLDIN-NEVER-RETAKES-ARCHITECTURE-ASSESSMENT`.

**WHAT HAPPENED IN S313.** The architect seat's retro finding W-1 (MAJOR) is recorded at
graph `_bmad-output/party-mode-transcripts/s313/retro.md:109`. The brief for this batch cited it at
`_bmad-output/retro-artifacts/s313/party/architect.md`, which no graph ref has ever carried
(0 commits touch that path, against 1 for `retro-artifacts/s313/next-inputs.md` in the same
query). The sprint's `architecture-assessment.md` validated CAP-1 through CAP-4 and concluded that
none of them touched `rebalancer/`. A bug was then folded in through `bug-investigation.md` at
2026-09-23T21:54:56Z, as story-3-1, whose frontmatter carries `capital_path: true` at line 11
and whose fix edits `rebalancer/execution.py`. The only disposition that capability ever got is
the lead-written `- **No-AD:** CAP-5` at graph
`_bmad-output/planning-artifacts/architecture/architecture-graph-2026-09-10/ARCHITECTURE-SPINE.md:258`,
under the heading at line 256, "Capability dispositions added by the 2026-09-23 mid-step fold-in". Its reason
cites the stale assessment as the sprint's architecture gate. No architect was dispatched over the
folded story before merge. The first architect-seat review of it was the sprint-review party.

**WHAT SURVIVED OF THE FILING, AND WHAT DID NOT.** The defect survived: nothing in the fold path
sends a folded story to an architect. Two parts of the filing did not.

- Its named artifact `architecture-assessment.md` is a consumer-named file. `git grep -c
  'architecture-assessment.md' -- core` matches 0 files, against a control of 7 files for
  `architecture-impact.md`. Core owns no such artifact, so no core step can be told to re-take it.
- Its proposed remedy, "a re-take or an explicit recorded waiver", was refuted by the contract
  adversary. The S313 waiver already existed: it is the lead-written No-AD above. A remedy that
  accepts a waiver acquits the sprint that motivated it.

**THE FIX (0.644.0).** When the folded story's sprint runs a `pipeline_variant` whose `route.md`
sequence includes `architecture`, `bug-investigation.md` §4 now dispatches ONE `architect` after
the one-shot and its remediation. The architect runs `/bmad-review-adversarial-general` on the fix
story itself, scoped to architecture fidelity, and dispositions the capability from what that
review found, either as an AD extension or as a `No-AD` line. It then stamps the residue
`s<N>/fold-architecture-<slug>.md`. That residue carries one `SKILL_INVOCATION_PROVENANCE v1` block
whose fields §4 lists in full: `skill: bmad-review-adversarial-general`, `invoked_at`,
`tool_use_id` (the Agent dispatch that spawned the architect), `mode: subagent`,
`lead_role: bug-investigation.md`, `artifact:` (the story, spelled as the one-shot spells it), and
`findings_critical`, `findings_major` and `findings_minor`. Because the architect really runs the
skill, the block's `skill:` line is true. `core/team-roles/architect.md` carries the instruction to
run the skill and emit the block, and cites the schema rather than restating it. When
`validate-provenance-block.sh` sees a fold residue that cites the native convergence skill, it now
says the residue is wrong instead of telling the reader to repoint the pin.

**THE FOLD GATE IS ONE COMMAND.** Check 17's "Fold architecture gate (bug-investigation)" bullet
runs only `scripts/ai-dlc/validate-spawn-ledger.sh --fold-architect <residue> <one-shot> --ledger
_bmad-output/spawn-ledger.jsonl --route .claude/skills/ai-dlc/steps/route.md`, exit 0 required. The
script runs the residue's shape check itself, by running its sibling `validate-provenance-block.sh
<residue> --require-skill bmad-review-adversarial-general`, and it does so only when the fold is
owed and the ledger join has passed. `NOT-OWED` and `SKIP-PRE-ADOPTION` never read the residue.
The implementation-gate bullet and `implementation.md` cite that one command.
`bug-investigation.md` §4's direct-fold paragraph now carries no command of its own (round
v3.4). It cites Check 17's "Bug-fix story readiness gate (bug-investigation)" and "Fold
architecture gate (bug-investigation)" bullets by name, so a residue command re-added there is
caught by the fixture's DF arm and by the receipt, where before nothing read that paragraph.

**A BUG-VARIANT SPRINT IS NOT-OWED AT EVERY GATE (round v3.3).** In the previous round the bullet
ran `validate-provenance-block.sh <residue>` as a separate first command. Every variant writes the
per-bug one-shot, so a plain bug-variant fix story is a declared folded story, and that first
command failed for want of a residue the bug variant never writes. It failed at the
bug-investigation gate and at every implementation gate, on correct work. With the shape check
moved behind the owed decision, a `bug` sprint prints `NOT-OWED` and exits 0. The fixture now
extracts the implementation-gate command set from `gate-validation.md` and executes it in a `bug`
world, and the receipt below does the same.

**WHAT THE JOIN DECIDES.** `--fold-architect` joins the residue's `tool_use_id` to a spawn-ledger
row with role `architect`, in the one-shot's sprint, and cited by no other fold residue in that
sprint. The ordering anchor is the `ts` of the `adversary` ledger row that the one-shot's own
`tool_use_id` joins, and the architect row's `ts` must be at or after it. `invoked_at` decides
nothing, because the lead writes it. The one-shot's `tool_use_id` is itself a line the lead can
rewrite, so the anchor is only as fixed as that line. This round adds three clauses on top of the
join.

- **The story-stamp id equality check is new.** The story named by the one-shot's `artifact:`
  must carry a provenance block citing the one-shot's skill, whose `tool_use_id` equals the
  one-shot's. `stamp-story-provenance.sh` copies that id verbatim when it stamps the story. A story
  with no such block exits 1 with "story not stamped". A one-shot whose id differs from the stamp
  exits 1 with "one-shot re-pointed after the story was stamped". This catches an edit to the
  one-shot alone, made without touching the story. It does not fix the id: a re-stamp, or a
  hand-written block carrying the new id, restores agreement (residual 4).
- **The role message is new.** A one-shot id that the ledger records on a row whose role is not
  `adversary` exits 1 with "one-shot id not in the ledger as an adversary dispatch", naming the
  role. It does so in a fully adopted sprint and in a partially adopted one, where it never reads
  as SKIP.
- **The variant cross-check is new.** The bullet passes no `--variant`, which stays as an explicit
  override for tests. When `_bmad-output/implementation-artifacts/sprint-status.yaml` (or
  `--sprint-status`) carries a top-level `variant:`, and it disagrees with a variant taken from
  `--variant` or the snapshot, the script exits 2 with "variant disagreement". An absent file or
  an absent field means no cross-check. The snapshot reader now skips fenced code blocks and HTML
  comments, so a quoted `pipeline_variant: bug` above the real line no longer wins.

**THE VARIANT RESOLUTION ORDER (round v3.4).** The variant resolves in this order: (1)
`--variant`; (2) the `pipeline_variant:` line of `_bmad-output/pipeline-snapshot.md`; (3) the
top-level `variant:` of `sprint-status.yaml`; (4) none of the three, and the fold is OWED. A
snapshot that exists but yields no `pipeline_variant:` line now falls through to (3) instead of
exiting 2. Graph's snapshot history spells the variant `- **Variant:** …`, `**Pipeline variant:**`
and `- variant:` in revisions that carry no parseable line, and there the exit 2 wedged a correct
gate before `NOT-OWED` was decided. The cross-check applies only when both (1) or (2) and (3)
yield a value. An unknown value from whichever source won still exits 2.

With no snapshot and no `--variant` the fold is OWED. An unknown variant exits 2 and never reads as
`NOT-OWED`. A variant with no architecture step prints `NOT-OWED`, and a sprint whose ledger rows
carry no `tool_use_id` prints `SKIP-PRE-ADOPTION`. Both exit 0 and neither prints `PASS`. The
mode also requires the residue's `artifact:` to name the one-shot's story by full path.

**THE LEGACY ONE-SHOT IS REACHED THROUGH THE GATE.** Both fold bullets, the fold architecture gate
and the implementation-gate bullet, now also run the same `--fold-architect` command on a legacy
`s<N>/bug-fix-oneshot.md` whenever one is present in the slot, once per gate and whether or not any
story is folded, with residue `s<N>/fold-architecture.md`. The script decides the legacy name
against the variant: a legacy one-shot in a variant that runs an architecture step exits 1 with
"legacy one-shot name in an architecture variant", and in the `bug` variant it is `NOT-OWED`. The
legacy name still declares no folded story to any other arm of Check 17.

The gate arms run where the story's bytes are still the ones the stamp hashed. For a direct fold
(`route.md` Step 4), both halves of the bug-fix arm, including the cross-check, and the fold
architecture gate run at the end of §4, before any dev dispatch. At an implementation gate, the
separate "Implementation gate, declared folded bug-fix stories (implementation)" bullet runs the
story's shape check `validate-provenance-block.sh <story-file> --require-skill
bmad-review-adversarial-general` and the fold architecture gate, and never the cross-check. The dev
writes Scope Verification, the Dev Agent Record and `Status:` into the story before gate1, so the
cross-check would fail every correctly executed folded story there.

**S313 IS CAUGHT THROUGH THE DOCUMENTED COMMAND.** Its one-shot is the legacy
`s313/bug-fix-oneshot.md`, and its sprint's variant is `carry-over`, which runs an architecture
step. The fold bullet's command was EXTRACTED from `gate-validation.md` at `120fff13` (the one
backticked `--fold-architect` command in the bullet, with HTML comments stripped), and the legacy
residue path `s<N>/fold-architecture.md` was extracted from the same bullet. Both placeholders were
substituted for `s313`, and the command was run from inside a scratch world holding copies of
graph's spawn ledger (1761 rows, 106 of them S313), its pipeline snapshot
(`- pipeline_variant: carry-over`), its `sprint-status.yaml` (top-level `variant: carry-over`), the
legacy S313 one-shot, and graph's `route.md`. That `route.md` differs from the tip's elsewhere, and
its variant table is byte-identical to the tip's. Graph was at `8f7f5f2da`, with the ledger and
`sprint-status.yaml` modified in its working tree, and the copies were taken from the working tree.
The command exits 1 and prints `FAIL: legacy one-shot name in an architecture variant — rename to
bug-fix-oneshot-<slug>.md`. It was reached through the documented command, not a hand invocation.
With the scratch snapshot edited to `bug` and graph's `sprint-status.yaml` kept, the same command
exits 2 with "variant disagreement: snapshot says bug, sprint-status says carry-over". With that
`sprint-status.yaml` also removed, it prints `NOT-OWED` and exits 0, which is the control. The
legacy name was not the consumer's deviation. The per-bug name `bug-fix-oneshot-<slug>.md` entered
`bug-investigation.md` at 0.628.0. At the fold (2026-09-23T21:54:56Z), graph's committed
`.claude/.ai-dlc-version` read 0.618.0, set at `d048f3cb4`; the next change to it is `0bcb35c82`
on 2026-09-25, to 0.641.0. The install therefore predates 0.628.0.

**SIX RESIDUALS, ALL KNOWN AND NONE CLOSED.**

1. **The lead can write the spine text.** The lead holds Write on the spine and on the residue, so
   nothing proves that the No-AD TEXT came from the architect. The gate proves that an architect
   was DISPATCHED after the fold, and that its id is cited once.
2. **An unrelated later architect dispatch satisfies the join.** Re-measured at `120fff13` on a
   scratch copy of graph's S313 data with the one-shot renamed to
   `bug-fix-oneshot-rebalancer-burn-block-fix.md`. Graph's story carries no
   `bmad-review-adversarial-general` stamp, so the tip first exits 1 with "story not stamped". With
   a stamp carrying the one-shot's id appended to the scratch story, a residue citing
   `toolu_1790231838987_1`, the graph ledger's architect row at 2026-09-24T06:37:19Z in sprint 313,
   exits 0 and prints `PASS`. The one-shot's own ledger row is at 2026-09-23T22:59:43Z. That
   dispatch was not about the fold.
3. **The negated-prose bullet is not mechanically catchable.** "Do NOT run" in place of "Run"
   leaves the command backticked and runnable, so every extraction still finds it. It closes on
   the structural trace, under the batch-147 ruling that no behavioural replay closes a prompt or
   step-file fix.
4. **The one-shot's `tool_use_id` can be re-pointed at any time; a re-stamp, or a hand-written
   block carrying the new id, restores agreement.** Re-pointing the one-shot and then re-running
   the shipped `stamp-story-provenance.sh --terminal … --profile bug-story-provenance` makes the
   fold gate, the story's shape check and the cross-check all exit 0. A hand-appended or fenced
   block carrying the id also satisfies the stamp read. The stamp is the sanctioned writer, so no
   mechanical close exists without a new record the lead cannot rewrite. What the equality check
   does buy: an edit to the one-shot alone, without touching the story, is caught.
5. **The variant is lead-written in both records.** The pipeline snapshot and `sprint-status.yaml`
   are both lead-written. The cross-check raises the cost of a false variant from one line to two;
   it does not remove it. Disagreement between them is now caught.
6. **A Bash-appended id-less ledger row buys SKIP-PRE-ADOPTION.** One ledger row with no
   `tool_use_id` turns a fully adopted sprint into a partially adopted one, where a one-shot id
   that joins no row prints `SKIP-PRE-ADOPTION` and exits 0. The remediation guard covers Edit,
   Write and MultiEdit only, and only while a gate FAIL is live. Measured in the third
   tip-adversary round: a fully adopted S313 with the one-shot's id absent from the ledger exits 1, and the
   same world with one id-less S313 row appended exits 0 with `SKIP-PRE-ADOPTION`.

**THE RECEIPT DRIVES THE EXTRACTED COMMANDS.** It extracts Check 17's span from `gate-validation.md`
with HTML comments stripped, finds the fold bullet and the implementation-gate bullet by header
(each exactly once), and takes the one backticked `--fold-architect` command from the fold bullet
and every backticked `scripts/ai-dlc/` command from both bullets. Each case is a `mktemp` world
holding a pipeline snapshot, the ledger, `route.md`, the schema, a stamped story and the planning
slot, with `validate-spawn-ledger.sh` and `validate-provenance-block.sh` installed side by side
under `scripts/ai-dlc/`. The extracted command runs from inside the world with its placeholders
substituted, and no case types the command by hand. The ledger carries an epoch row, an early
adversary row, an early architect row, the one-shot's adversary row, a later architect row, a later
adversary row, an id-less sprint, a cross-sprint architect row, and a partially adopted sprint
whose one id-bearing earlier row is an architect.

- The passing world runs the whole extracted command set, worst exit 0, and the fold command
  prints `PASS` on its first line.
- **B-1:** a `bug` world with no residue runs the whole extracted command set with worst exit 0,
  and the fold command prints `NOT-OWED`. This is the input that discriminates the previous
  round's separate residue command.
- Exit 1 for a missing residue, for no snapshot at all, for a residue without `lead_role`, for an
  `artifact:` of `other-dir/p.md`, for an adversary id, an early architect id, a cross-sprint
  architect id, two residues citing one id, and a one-shot backdated before the early architect.
  A one-shot backdated before the epoch with a later architect prints `PASS`. The id-less sprint
  prints `SKIP-PRE-ADOPTION` with no `PASS` line, and an unknown variant exits 2.
- **D-a:** a one-shot re-pointed at the early adversary row, over a story stamped with the original
  id, exits 1 with "one-shot re-pointed after the story was stamped". An unstamped story exits 1
  with "story not stamped".
- **D-b:** a one-shot id on an architect row exits 1 with "as role architect", in the fully adopted
  sprint and in the partially adopted one.
- **D-c:** a `bug` snapshot against a `sprint-status.yaml` carrying `variant: carry-over` exits 2
  with "variant disagreement", and an agreeing pair prints `PASS`. A snapshot with a fenced and a
  commented `pipeline_variant: bug` above the real `carry-over` line exits 1 on the missing residue,
  so the decoy is not read.
- **D-d:** a legacy `bug-fix-oneshot.md` run through the extracted fold command, with the legacy
  residue path extracted from the same bullet, exits 1 with the legacy message in a `carry-over`
  world and prints `NOT-OWED` in a `bug` world.
- **DEFECT 3 (round v3.4):** a snapshot carrying only `- **Variant:** bug` beside a
  `sprint-status.yaml` carrying `variant: bug` prints `NOT-OWED` and exits 0 through the extracted
  fold command. With that `sprint-status.yaml` removed it exits 1, because the fold is owed.
- **DEFECT 2 (round v3.4):** `bug-investigation.md` §4's direct-fold paragraph is extracted by
  section, and exactly one must exist. Zero of its backticked `.sh` commands may invoke
  `validate-spawn-ledger.sh`, or `validate-provenance-block.sh` on a residue, and it must cite the
  "Fold architecture gate (bug-investigation)" bullet by name.
- Last, the residue path Check 17 names must equal the path `bug-investigation.md` names, and
  `gate-slice.sh --type implementation` must plan check 17 exactly once.

Every tree was a fresh `git archive` extraction. Each regression was built by `python3` in its own
extraction of `120fff13`, with an exact-once anchor asserted before the edit and the result
compared by `cmp` against the unmutated tip, and each one parsed under `bash -n`. The script's
self-probe carries its own copies of the D-a, D-b and shape cases, so each regression was scored
twice: with the self-probe left in, where the regression can be caught by the probe before any
case runs, and with the self-probe disabled, which isolates what the receipt's own cases catch.
Scored, raw exits:

| tree | exit | first failing case |
|---|---|---|
| tip `120fff13` | 0 | |
| base `7b32fb1a` | 1 | fold bullet absent |
| prior tip `3bc7ab11` | 1 | B-1 `bug` world, extracted command set |
| adversary-role clause dropped | 1 | self-probe; with the probe off, D-b |
| residue shape checked before the owed decision | 1 | B-1 `bug` world, either way |
| story-stamp equality skipped | 1 | self-probe; with the probe off, D-a re-point |
| sprint-status cross-check dropped | 1 | D-c disagreement, either way |
| tip with the self-probe disabled only (control) | 0 | |
| second correct spelling (fold command's flags reordered) | 0 | |

Round v3.4 re-scored the receipt with the two cases above added, the same way, each tree a fresh
`git archive` extraction with the working tree's `core/` laid over the tip:

| tree | exit | first failing case |
|---|---|---|
| tip (round v3.4) | 0 | |
| `781d564f`, the round this one repaired | 1 | DEFECT 3 unparseable snapshot |
| the tip adversary's direct-fold regression (a residue command and the fold command re-typed in the paragraph) | 1 | direct-fold command |
| unparseable snapshot exits 2 again | 1 | passing-world command set (the self-probe refuses every call); with the probe off, DEFECT 3 unparseable snapshot |
| tip with the self-probe disabled only (control) | 0 | |
| second correct spelling (the paragraph's citation reworded) | 0 | |

**What else satisfies it.** A fix that keeps the join but moves the residue path consistently in
both files passes, and that is intended. A bullet whose backticked command is correct but whose
surrounding prose tells the reader not to run it passes, as the third residual states. A
direct-fold paragraph that re-types a command the receipt's grammar does not name, or spells a
command outside backticks, also passes.

verify: sh R="$(pwd)"; V="$R/core/scripts/validate-spawn-ledger.sh"; VPB="$R/core/scripts/validate-provenance-block.sh"; SCH="$R/core/schemas/provenance-block.json"; GV="$R/core/skills/ai-dlc/steps/gate-validation.md"; BI="$R/core/skills/ai-dlc/steps/bug-investigation.md"; RT="$R/core/skills/ai-dlc/steps/route.md"; GS="$R/core/scripts/gate-slice.sh";for x in "$V" "$VPB" "$SCH" "$GV" "$BI" "$RT" "$GS"; do [ -f "$x" ] || exit 9; done;command -v jq >/dev/null && command -v perl >/dev/null || exit 9;f(){ echo "RCPT-FAIL $1" >&2; exit 1; };T="$(mktemp -d)" || exit 9; L="$T/l.jsonl";printf '%s\n' '{"v":1,"ts":"2000-01-01T00:00:00Z","sprint":900,"role":"dev","tool_use_id":"toolu_rcptepoch"}' '{"v":1,"ts":"2000-01-02T08:30:00Z","sprint":900,"role":"adversary","tool_use_id":"toolu_rcptadvearly"}' '{"v":1,"ts":"2000-01-02T09:00:00Z","sprint":900,"role":"architect","tool_use_id":"toolu_rcptearly"}' '{"v":1,"ts":"2000-01-02T10:00:00Z","sprint":900,"role":"adversary","tool_use_id":"toolu_rcptoneshot"}' '{"v":1,"ts":"2000-01-02T11:00:00Z","sprint":900,"role":"architect","tool_use_id":"toolu_rcptlate"}' '{"v":1,"ts":"2000-01-02T11:30:00Z","sprint":900,"role":"adversary","tool_use_id":"toolu_rcptadvlate"}' '{"v":1,"ts":"2000-01-02T11:00:00Z","sprint":901,"role":"architect"}' '{"v":1,"ts":"2000-01-02T11:10:00Z","sprint":899,"role":"architect","tool_use_id":"toolu_rcptxsprint"}' '{"v":1,"ts":"2000-01-03T08:00:00Z","sprint":902,"role":"adversary"}' '{"v":1,"ts":"2000-01-03T09:00:00Z","sprint":902,"role":"architect","tool_use_id":"toolu_rcpt902arch"}' '{"v":1,"ts":"2000-01-03T12:00:00Z","sprint":902,"role":"architect","tool_use_id":"toolu_rcpt902late"}' > "$L" || exit 9;blk(){ printf '%s\n' '<!-- SKILL_INVOCATION_PROVENANCE v1' 'skill: bmad-review-adversarial-general' "invoked_at: $1" "tool_use_id: $2" 'mode: subagent' 'lead_role: bug-investigation.md' "artifact: $3" 'findings_critical: 0' 'findings_major: 1' 'findings_minor: 2' 'SKILL_INVOCATION_PROVENANCE_END -->'; };mk(){ W="$T/$1"; d="$W/_bmad-output/planning-artifacts/s$2"; mkdir -p "$d" "$W/_bmad-output/implementation-artifacts" "$W/scripts/ai-dlc" "$W/.claude/skills/ai-dlc/steps" "$W/.claude/schemas" "$W/stories" || exit 9; [ -z "$3" ] || printf '%s\n' '# Pipeline Snapshot' '' "- pipeline_variant: $3" > "$W/_bmad-output/pipeline-snapshot.md"; cp "$L" "$W/_bmad-output/spawn-ledger.jsonl"; cp "$V" "$VPB" "$W/scripts/ai-dlc/"; cp "$SCH" "$W/.claude/schemas/"; cp "$RT" "$W/.claude/skills/ai-dlc/steps/route.md"; O="${8:-toolu_rcptoneshot}"; blk "${6:-2000-01-02T10:00:05Z}" "$O" stories/p.md > "$d/${7:-bug-fix-oneshot-p.md}"; ST="${9:-$O}"; if [ "$ST" = - ]; then printf '%s\n' '# Story p' > "$W/stories/p.md"; else { printf '%s\n' '# Story p' ''; blk 2000-01-02T10:00:06Z "$ST" stories/p.md; } > "$W/stories/p.md"; fi; [ -z "$4" ] || blk 2000-01-02T11:00:05Z "$4" "${5:-stories/p.md}" > "$d/fold-architecture-p.md"; };C17="$(awk '/CHECK_LOADED: 17 -->/ { p = 1; next } /CHECK_LOADED: 18 -->/ { p = 0 } p' "$GV" | perl -0pe 's/<!--.*?-->//gs')";bl(){ awk -v h="$1" '/^\*\*PASS:/ { s = 1 } !s && /^- \*\*/ { b = (index($0, h) == 1); if (b) n++ } !s && b { printf "%s ", $0 } END { if (n != 1) print "__BULLETS_" n }' <<<"$C17"; };FB="$(bl '- **Fold architecture gate (bug-investigation):**')"; case "$FB" in *__BULLETS_*|'') f fold-bullet ;; esac;IB="$(bl '- **Implementation gate, declared folded bug-fix stories (implementation):**')"; case "$IB" in *__BULLETS_*|'') f impl-bullet ;; esac;cmds(){ grep -oE '`scripts/ai-dlc/[^`]*`' <<<"$1" | tr -d '`'; };CM="$(cmds "$FB" | grep -e '--fold-architect')"; [ -n "$CM" ] && [ "$(printf '%s\n' "$CM" | grep -c .)" = 1 ] || f fold-command;SET="$(cmds "$FB"; cmds "$IB")";sb(){ printf '%s' "$1" | sed -e "s#<residue>#$2#g" -e "s#<one-shot>#$3#g" -e 's#<story-file>#stories/p.md#g' -e "s#s<N>#s$4#g"; };xr(){ W="$T/$1"; X="$(sb "$2" "$3" "$4" "$5")"; case "$X" in *'<'*'>'*) f "unsubstituted:$X" ;; esac; ( cd "$W" && eval "bash $X" </dev/null >"$W/o" 2>&1 ); c=$?; F="$(head -1 "$W/o")"; };xf(){ xr "$1" "$CM" "${4:-_bmad-output/planning-artifacts/s$2/fold-architecture-p.md}" "_bmad-output/planning-artifacts/s$2/${3:-bug-fix-oneshot-p.md}" "$2"; };xall(){ w=0; n=0; while IFS= read -r K; do [ -n "$K" ] || continue; n=$((n + 1)); xr "$1" "$K" _bmad-output/planning-artifacts/s900/fold-architecture-p.md _bmad-output/planning-artifacts/s900/bug-fix-oneshot-p.md 900; [ "$c" -gt "$w" ] && w=$c; done <<<"$SET"; [ "$n" -ge 2 ] || f "command-set-$n"; c=$w; };has(){ grep -qF -e "$2" "$T/$1/o"; };mk pos 900 carry-over toolu_rcptlate; xall pos; [ "$c" -eq 0 ] || f pos-set; xf pos 900; [ "$c" -eq 0 ] || f pos; case "$F" in PASS:*) ;; *) f pos-line ;; esac;mk bug 900 bug ""; xall bug; [ "$c" -eq 0 ] || f B1-bug-world-set; xf bug 900; [ "$c" -eq 0 ] || f B1-bug; case "$F" in NOT-OWED:*) ;; *) f B1-bug-line ;; esac;mk none 900 carry-over ""; xf none 900; [ "$c" -eq 1 ] || f none;mk nosnap 900 "" ""; xf nosnap 900; [ "$c" -eq 1 ] || f nosnap;mk shape 900 carry-over toolu_rcptlate; P="$T/shape/_bmad-output/planning-artifacts/s900/fold-architecture-p.md"; grep -v '^lead_role:' "$P" > "$T/nolr" && cp "$T/nolr" "$P" || exit 9; xf shape 900; [ "$c" -eq 1 ] || f shape;mk art 900 carry-over toolu_rcptlate other-dir/p.md; xf art 900; [ "$c" -eq 1 ] || f art;mk role 900 carry-over toolu_rcptadvlate; xf role 900; [ "$c" -eq 1 ] || f role;mk early 900 carry-over toolu_rcptearly; xf early 900; [ "$c" -eq 1 ] || f early;mk xs 900 carry-over toolu_rcptxsprint; xf xs 900; [ "$c" -eq 1 ] || f xsprint;mk dup 900 carry-over toolu_rcptlate; cp "$T/dup/_bmad-output/planning-artifacts/s900/fold-architecture-p.md" "$T/dup/_bmad-output/planning-artifacts/s900/fold-architecture-q.md" || exit 9; xf dup 900; [ "$c" -eq 1 ] || f dup;mk back 900 carry-over toolu_rcptearly "" 2000-01-02T08:00:00Z; xf back 900; [ "$c" -eq 1 ] || f back;mk backok 900 carry-over toolu_rcptlate "" 1999-12-31T00:00:00Z; xf backok 900; [ "$c" -eq 0 ] || f backok; case "$F" in PASS:*) ;; *) f backok-line ;; esac;mk pre 901 carry-over toolu_rcptlate; xf pre 901; [ "$c" -eq 0 ] || f pre; case "$F" in SKIP-PRE-ADOPTION:*) ;; *) f pre-line ;; esac; grep -q '^PASS' "$T/pre/o" && f pre-pass;mk unk 900 no-such-variant ""; xf unk 900; [ "$c" -eq 2 ] || f unknown;mk rep 900 carry-over toolu_rcptearly "" "" "" toolu_rcptadvearly toolu_rcptoneshot; xf rep 900; [ "$c" -eq 1 ] || f Da-repoint; has rep 'one-shot re-pointed after the story was stamped' || f Da-repoint-msg;mk unst 900 carry-over toolu_rcptlate "" "" "" "" -; xf unst 900; [ "$c" -eq 1 ] || f Da-unstamped; has unst 'story not stamped' || f Da-unstamped-msg;mk ob 900 carry-over toolu_rcptlate "" "" "" toolu_rcptearly; xf ob 900; [ "$c" -eq 1 ] || f Db-full; has ob 'as role architect' || f Db-full-msg;mk obp 902 carry-over toolu_rcpt902late "" 2000-01-03T09:00:05Z "" toolu_rcpt902arch; xf obp 902; [ "$c" -eq 1 ] || f Db-partial; has obp 'as role architect' || f Db-partial-msg;mk dc 900 bug ""; printf '%s\n' 'sprint: 900' 'variant: carry-over' > "$T/dc/_bmad-output/implementation-artifacts/sprint-status.yaml"; xf dc 900; [ "$c" -eq 2 ] || f Dc-disagree; has dc 'variant disagreement' || f Dc-disagree-msg;mk dca 900 carry-over toolu_rcptlate; printf '%s\n' 'sprint: 900' 'variant: carry-over' > "$T/dca/_bmad-output/implementation-artifacts/sprint-status.yaml"; xf dca 900; [ "$c" -eq 0 ] || f Dc-agree; case "$F" in PASS:*) ;; *) f Dc-agree-line ;; esac;mk fen 900 carry-over ""; printf '%s\n' '# Pipeline Snapshot' '```yaml' '- pipeline_variant: bug' '```' '<!-- - pipeline_variant: bug -->' '- pipeline_variant: carry-over' > "$T/fen/_bmad-output/pipeline-snapshot.md"; xf fen 900; [ "$c" -eq 1 ] || f Dc-fence-decoy;LR="$(grep -oE '`_bmad-output/planning-artifacts/s<N>/fold-architecture[.]md`' <<<"$FB" | head -1 | tr -d '`')"; [ -n "$LR" ] || f Dd-legacy-residue-path;mk leg 900 carry-over "" "" "" bug-fix-oneshot.md; xf leg 900 bug-fix-oneshot.md "$LR"; [ "$c" -eq 1 ] || f Dd-legacy; has leg 'legacy one-shot name in an architecture variant' || f Dd-legacy-msg;mk legbug 900 bug "" "" "" bug-fix-oneshot.md; xf legbug 900 bug-fix-oneshot.md "$LR"; [ "$c" -eq 0 ] || f Dd-legacy-bug; case "$F" in NOT-OWED:*) ;; *) f Dd-legacy-bug-line ;; esac;mk lv 900 "" ""; printf '%s\n' '# Pipeline Snapshot' '' '- **Variant:** bug' > "$T/lv/_bmad-output/pipeline-snapshot.md"; printf '%s\n' 'sprint: 900' 'variant: bug' > "$T/lv/_bmad-output/implementation-artifacts/sprint-status.yaml"; xf lv 900; [ "$c" -eq 0 ] || f D3-unparseable-snapshot; case "$F" in NOT-OWED:*) ;; *) f D3-unparseable-snapshot-line ;; esac; rm -f "$T/lv/_bmad-output/implementation-artifacts/sprint-status.yaml"; xf lv 900; [ "$c" -eq 1 ] || f D3-unparseable-snapshot-owed;DP="$(awk '/^### 4\./ { s = 1; next } s && /^### / { s = 0 } s && index($0, "**Direct fold:") == 1 { p = 1; n++ } p && /^[ \t]*$/ { p = 0 } p { printf "%s ", $0 } END { if (n != 1) print "__PARAS_" n }' "$BI")"; case "$DP" in *__PARAS_*|'') f direct-fold-paragraph ;; esac; [ "$(grep -oE '`[^`]*[.]sh[^`]*`' <<<"$DP" | grep -E 'validate-spawn-ledger[.]sh|validate-provenance-block[.]sh[^`]*(<residue>|fold-architecture)' | grep -c .)" = 0 ] || f direct-fold-command; case "$DP" in *'"Fold architecture gate (bug-investigation)"'*) ;; *) f direct-fold-cites ;; esac;B="$(perl -0pe 's/<!--.*?-->//gs' "$BI" | tr '\n' ' ')"; S="$(tr '\n' ' ' <<<"$C17")"; k(){ grep -oE 's<N>/fold-architecture-<[^>`]+>\.md' <<<"$1" | sort -u; }; KS="$(k "$S")"; KB="$(k "$B")"; [ -n "$KS" ] && [ "$KS" = "$KB" ] || f residue-path-join;P="$(bash "$GS" --type implementation </dev/null 2>/dev/null)" || f gate-slice; [ "$(awk -F'\t' 'NR > 1 { m = split($3, a, ","); for (i = 1; i <= m; i++) if (a[i] == "17") c++ } END { print c + 0 }' <<<"$P")" = 1 ] || f gate-slice-17

**LANDED (v0.644.0, verified PENDING).**

## BL-320 — Check 17's bug-fix arm was never loaded at the gate it names, because the implementation manifest row omitted 17

**DEFECT.** Found by this batch's lead while verifying the premises of `BL-319`. It discharges no
consumer candidate.

**THE ARM WAS DEAD ON ITS OWN GATE.** Check 17's "Bug-fix story readiness gate
(bug-investigation)" bullet ran "at the bug-investigation gate". The `bug` variant declares that
gate `[implementation]` (`bug-investigation.md`), and `route.md` Step 4 told a directly folded
fix story that it "takes Check 17's bug-fix arm at the next gate", which is also an implementation
gate. The GATE_MANIFEST `implementation` row was `5, 6, 8, 9, 10, 11, 11a, 19, 22`, so no
implementation gate ever loaded Check 17. Measured at base `7b32fb1a` in a detached worktree,
`gate-slice.sh --type implementation` plans check 17 **0** times, against a control of check 22 =
**1** in the same plan, and `--type story` plans 17 = 1. The story gate reached the arm only for a
story folded before stories-test-strategy ran.

**THE FIX (0.644.0).** 17 joins the `implementation` row. Its enforcement-map entry gains
`implementation` in `gate_types`, `implementation.md` names 17 in the slice it lists, and `route.md`
says the next implementation gate loads the check. I7 in `validate-enforcement-map.sh` now requires
the `check-manifest-bypass` seed to omit only the ids the implementation row names outside the
universal and planning rows, since 17 is in the planning row and the seed must carry it.

**THE IMPLEMENTATION GATE DOES NOT RUN THE CROSS-CHECK (round v3.2).** Loading 17 at every
implementation gate made a new failure reachable. The bug-fix arm's cross-check,
`stamp-story-provenance.sh --check`, hashes the story body, and `implementation.md` has the dev
write Scope Verification, the Dev Agent Record and `Status:` into that story before gate1, so the
cross-check would fail every correctly executed folded story there. Check 17 now carries a separate
"Implementation gate, declared folded bug-fix stories (implementation)" bullet. It runs the shape
check `scripts/ai-dlc/validate-provenance-block.sh <story-file> --require-skill
bmad-review-adversarial-general` and the fold architecture gate, which is one
`validate-spawn-ledger.sh --fold-architect` command, and never the cross-check. The
cross-check runs where the story is folded, before any dev dispatch. Check 17's Scope paragraph now
names the implementation gate for declared folded stories, so its skip clause no longer reads as an
opt-out there.

**THE RECEIPT RUNS `gate-slice.sh` AND READS THE IMPLEMENTATION BULLET BY SECTION.** It exits 9
unless the plan's own controls hold: check 22 is planned once for `implementation` and check 17
once for `story`. It requires `gate-slice.sh --type implementation` to plan check 17 exactly once.
It then extracts Check 17's bullets by header, inside the Check 17 span and with HTML comments
stripped. As a control, the bug-fix readiness bullet must carry a backticked
`stamp-story-provenance.sh ... --check` command, or the receipt exits 9. The implementation bullet
must exist exactly once and must carry NO backticked `stamp-story-provenance.sh ... --check`
command, and it must name the backticked command `scripts/ai-dlc/validate-provenance-block.sh ...
--require-skill bmad-review-adversarial-general`. That story shape check stays in the bullet in
round v3.3, which moved only the RESIDUE's shape check inside `validate-spawn-ledger.sh
--fold-architect`, so the clause still holds. Re-scored at `120fff13` in fresh `git archive`
extractions, with each regression built by `python3` against an asserted anchor, raw exits:

| tree | exit |
|---|---|
| tip `120fff13` | 0 |
| base `7b32fb1a` | 1 |
| prior tip `3bc7ab11` | 0 |
| B1 regression: the implementation bullet also runs the cross-check | 1 |
| 17 dropped from the implementation row | 1 |
| second correct spelling (17 appended at the end of the row) | 0 |
| each of BL-319's four round-v3.3 regressions (none touches this bullet or the manifest) | 0 |

At `3bc7ab11` this entry's subject was already in place, so 0 there is expected; round v3.3 did
not change the manifest row or the implementation bullet's story shape check.

**What else satisfies it.** Adding 17 to the `universal` row instead also scores 0. That still
loads the arm at every implementation gate, so it is not a regression of this entry, but it widens
the check to every gate type. A cross-check named in the implementation bullet only in prose,
without backticks, is not seen by the absence clause.

verify: sh G="$(pwd)/core/scripts/gate-slice.sh"; GV="$(pwd)/core/skills/ai-dlc/steps/gate-validation.md"; [ -f "$G" ] && [ -f "$GV" ] || exit 9; n(){ P="$(bash "$G" --type "$1" </dev/null 2>/dev/null)" || return 1; awk -F'\t' -v k="$2" 'NR > 1 { m = split($3, a, ","); for (i = 1; i <= m; i++) if (a[i] == k) c++ } END { print c + 0 }' <<<"$P"; }; [ "$(n implementation 22)" = 1 ] && [ "$(n story 17)" = 1 ] || exit 9; [ "$(n implementation 17)" = 1 ] || exit 1; c17b(){ awk -v h="$1" '/<!-- CHECK_LOADED: 17 -->/ { s = 1 } /<!-- CHECK_LOADED: 18 -->/ { s = 0 } s && /^\*\*PASS:/ { s = 0 } s && /^- \*\*/ { b = (index($0, h) == 1); if (b) n++ } s && b { printf "%s ", $0 } END { if (n != 1) printf "__BULLETS_%d", n }' "$GV" | perl -pe 's/<!--.*?-->//g'; }; xc(){ grep -cE '`[^`]*stamp-story-provenance\.sh[^`]*--check[^`]*`' <<<"$1"; }; RB="$(c17b '- **Bug-fix story readiness gate (bug-investigation):**')"; case "$RB" in *__BULLETS_*|'') exit 9 ;; esac; [ "$(xc "$RB")" -ge 1 ] || exit 9; IB="$(c17b '- **Implementation gate, declared folded bug-fix stories (implementation):**')"; case "$IB" in *__BULLETS_*|'') exit 1 ;; esac; [ "$(xc "$IB")" = 0 ] || exit 1; grep -qE '`scripts/ai-dlc/validate-provenance-block\.sh [^`]*--require-skill bmad-review-adversarial-general[^`]*`' <<<"$IB"
