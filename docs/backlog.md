...
The history rewrite is two steps. Step 1 writes the new history to a temp name beside it under
`set -C` and checks status and size. Step 2 writes that verified copy back INTO the history with
`cat temp > history`, checks status and size, and only then deletes the temp copy. A rename in
step 2 (`mv -f`, the round-3 design) replaced a symlinked history with a regular file and orphaned
its target, split a hard-linked history from its peer, and reset a 640 mode to 644. A history that
is not writable, and a file already at the temp name, are refused before the archive append, so
neither leaves a duplicate block behind. Two clauses at the end of the receipt carry this. First, a
symlinked 12-cut-point history must stay a symlink, and its target must equal the history an
unlinked copy rotates to. Second, a `cat` shim fails only the write-back: it lands every byte and
exits 1, keyed on the single-argument call naming the `.rotate.` temp. That must exit 1 and leave
the temp copy byte-identical to the unlinked copy's rotated history. The `mv -f` write-back scores
1 on the first clause, and a rotator that deletes the temp copy on a failed write-back scores 1 on
the second. Deleting step 1's size check, its status check or its `set -C` still scores 0: each
needs a shim or a second file keyed on one call, and those arms live in the
`snapshot-archive-rotate` fixture, which kills all three.

The temp name carries the pid, so a precheck keyed on the run's own name never saw an earlier
run's temp copy. The history cut by a failed write-back holds `KEEP_ENTRIES` headings or fewer,
so the re-run landed on "nothing to rotate" and exited 0 without ever reaching that precheck.
Measured at `2bafa39e`: after a half-way write-back failure, a plain re-run exited 0 with 11 of
29 pre-run lines in neither tracked file, and a SIGKILL during the write-back lost the same 11
with no restore ever printed. The rotator now refuses ANY `.<history>.rotate.*` before every exit
path and names each file with its recovery. The printed restore is
`cp temp history && rm -f temp`. The archive is staged on the nothing-to-rotate and no-history
paths too, because the re-run after a restore lands there, and a history directory that is not
writable is refused before the archive append. The last receipt clause carries the re-run: a
`cat` shim writes half of the temp copy back and exits 1 (exit 9 if the history was not cut, so
a shim that never fired cannot score 0), then a plain re-run must exit 1, and every pre-run
history line must be in the history, the archive or the temp copy. `2bafa39e` scores 1 on that
clause alone (the receipt without it scores 0 there). The SIGKILL, restore-then-stage and
read-only-directory shapes need a sentinel, a git corpus comparison and a non-root user, and
live in the fixture's `rerunkill`, `restore` and `rodir` arms.

The prefix test asked `head -c <history bytes>`, and BSD `head` refuses `-c 0`. So a write-back
that wrote 0 bytes left an empty history that the re-run scored "NOT a prefix", printing `head`'s
complaint and the compare-by-hand text. Following that text lost 32 of 44 lines, and the next run
exited 0. An empty history is now decided as a prefix before `head` is asked, and `head`'s stderr
never reaches the operator. The write-back is `cat < temp > history`, which opens the source first,
so a temp removed between the two steps fails before the history is opened. The argument form
truncated the history to 0 bytes in that case. The not-a-prefix text no longer claims where the
temp came from. A killed write-back followed by a trim leaves a history that is not a prefix while
the lost lines exist only in the temp, so the text prints a command listing the temp's lines found
in neither the history nor the archive. A directory at the temp name gets its own remedy. The
absorb's truncate is verified: a read-only snapshot used to exit 0 and print "truncated it to 0
bytes" over a full file, and it now exits 1, saying the snapshot is archived but not emptied.

The receipt's last clause carries the zero-byte write-back. A `cat` shim writes nothing back and
exits 1 (exit 9 unless the history is left at 0 bytes). The re-run then uses a `head` shim that
refuses `-c 0` as BSD's does, so the clause fires on a GNU host too. It must exit 1, print no
`head:` text, and print the `cp` restore. Running that restore as printed and re-running must give
a history byte-identical to a clean rotation. `445e9c20` scores 1 on that clause alone. The
killed-then-trimmed listing, the vanished temp and the read-only snapshot need a sentinel, a `wc`
shim keyed on a pid-named file, and a non-root user. They live in the fixture's `trimmed`, `race`
and `rosnap` arms.

verify: sh R="$(pwd)"; S="$R/core/scripts/rotate-snapshot-archive.sh"; [ -f "$S" ] || exit 9; command -v git >/dev/null || exit 9; unset GIT_DIR GIT_WORK_TREE GIT_INDEX_FILE GIT_COMMON_DIR GIT_OBJECT_DIRECTORY; T="$(mktemp -d)" || exit 9; g(){ git -c user.name=r -c user.email=r@r -c commit.gpgsign=false -c core.hooksPath=/dev/null "$@"; }; P=_bmad-output/pipeline-snapshot.md; A=_bmad-output/pipeline-history/pipeline-snapshot-archive.md; mk(){ W="$T/$1"; mkdir -p "$W/_bmad-output" && g init -q "$W" || exit 9; printf '# Pipeline Snapshot\n\nSTALE321-%s\n- open item one for %s\n- open item two for %s\n' "$1" "$1" "$1" > "$W/$P"; printf '# History\n' > "$W/_bmad-output/pipeline-snapshot-history.md"; [ "$2" -gt 0 ] || rm -f "$W/_bmad-output/pipeline-snapshot-history.md"; i=0; while [ "$i" -lt "$2" ]; do i=$((i+1)); printf '## entry %s\n\nbody %s\n' "$i" "$i" >> "$W/_bmad-output/pipeline-snapshot-history.md"; done; [ -z "$3" ] || printf '%s\n' "$3" > "$W/.gitignore"; g -C "$W" add -A && g -C "$W" commit -qm s || exit 9; cp "$W/$P" "$T/$1.before" || exit 9; ls -i "$W/$P" | awk '{print $1}' > "$T/$1.ino"; }; ab(){ W="$T/$1"; shift; ( cd "$W" && bash "$S" _bmad-output/pipeline-snapshot-history.md "$@" </dev/null >/dev/null 2>&1 ); }; ok(){ W="$T/$1"; N="$(wc -l < "$T/$1.before" | tr -d ' ')"; [ "$N" -gt 1 ] || exit 9; tail -n "$N" "$W/$A" > "$T/$1.tail" 2>/dev/null; [ -f "$W/$P" ] && [ ! -s "$W/$P" ] && grep -q "STALE321-$1" "$W/$A" && cmp -s "$T/$1.before" "$T/$1.tail" && [ -n "$(g -C "$W" ls-files -- "$A")" ] && [ "$(ls -i "$W/$P" | awk '{print $1}')" = "$(cat "$T/$1.ino")" ]; }; for s in none:0 floor:10 above:12; do n="${s%%:*}"; mk "$n" "${s#*:}" ""; ab "$n" --absorb "$P" --apply || exit 1; ok "$n" || exit 1; done; b="$(wc -c < "$T/none/$A")"; ab none --absorb "$P" --apply || exit 1; [ "$(wc -c < "$T/none/$A")" = "$b" ] || exit 1; ab none --absorb "$P" --no-such-option; [ "$?" = 2 ] || exit 1; ab none --absorb _bmad-output/no-such-file.md --apply; [ "$?" = 2 ] || exit 1; mk ign 0 "_bmad-output/pipeline-history/"; cp "$T/ign/$P" "$T/ign.before" || exit 9; ab ign --absorb "$P" --apply; [ "$?" = 1 ] || exit 1; cmp -s "$T/ign.before" "$T/ign/$P" || exit 1; for s in unw0:0 unw12:12; do n="${s%%:*}"; mk "$n" "${s#*:}" ""; mkdir -p "$T/$n/$A" || exit 9; [ -f "$T/$n/_bmad-output/pipeline-snapshot-history.md" ] && { cp "$T/$n/_bmad-output/pipeline-snapshot-history.md" "$T/$n.hist" || exit 9; }; ab "$n" --absorb "$P" --apply; [ "$?" = 1 ] || exit 1; cmp -s "$T/$n.before" "$T/$n/$P" || exit 1; [ ! -f "$T/$n.hist" ] || cmp -s "$T/$n.hist" "$T/$n/_bmad-output/pipeline-snapshot-history.md" || exit 1; done; for s in ul0:0 ul12:12; do n="${s%%:*}"; mk "$n" "${s#*:}" ""; mkdir -p "$T/$n/_bmad-output/pipeline-history" || exit 9; head -c 8152 /dev/zero | tr '\0' x > "$T/$n/$A" || exit 9; [ -f "$T/$n/_bmad-output/pipeline-snapshot-history.md" ] && { cp "$T/$n/_bmad-output/pipeline-snapshot-history.md" "$T/$n.hist" || exit 9; }; ( trap '' XFSZ; ulimit -f 8; ab "$n" --absorb "$P" --apply ); [ "$?" = 1 ] || exit 1; cmp -s "$T/$n.before" "$T/$n/$P" || exit 1; [ ! -f "$T/$n.hist" ] || cmp -s "$T/$n.hist" "$T/$n/_bmad-output/pipeline-snapshot-history.md" || exit 1; done; mk big 0 ""; H="$T/big/_bmad-output/pipeline-snapshot-history.md"; { printf '# History\n\n'; i=0; while [ "$i" -lt 22 ]; do i=$((i+1)); printf 'PRE-%02d: a long preamble line that stays in the live history across every rotation.\n' "$i"; done; printf '\n'; i=0; while [ "$i" -lt 14 ]; do i=$((i+1)); printf '## entry %s\n\n' "$i"; j=0; while [ "$j" -lt 8 ]; do j=$((j+1)); printf 'BIG-%02d-%02d: a substantive history body line, long enough to give the tail weight.\n' "$i" "$j"; done; printf '\n'; done; } > "$H" || exit 9; g -C "$T/big" add -A && g -C "$T/big" commit -qm h || exit 9; cp -R "$T/big" "$T/bigc" || exit 9; ab bigc --absorb "$P" --apply || exit 9; C="$T/bigc/_bmad-output/pipeline-snapshot-history.md"; [ "$(wc -c < "$C")" -gt 8192 ] && [ "$(awk '/^## /{f=1} f' "$C" | wc -c)" -lt 8192 ] && [ "$(wc -c < "$T/bigc/$A")" -lt 8192 ] || exit 9; cp "$H" "$T/big.hist" || exit 9; ( trap '' XFSZ; ulimit -f 8; ab big --absorb "$P" --apply ); [ "$?" = 1 ] || exit 1; cmp -s "$T/big.hist" "$H" || exit 1; cmp -s "$T/big.before" "$T/big/$P" || exit 1; sort -u "$T/big.hist" > "$T/big.want"; cat "$H" "$T/big/$A" | sort -u > "$T/big.have"; [ -s "$T/big.want" ] && [ -z "$(comm -23 "$T/big.want" "$T/big.have")" ] || exit 1; HR=_bmad-output/pipeline-snapshot-history.md; for n in sl slc wb wbc; do mk "$n" 12 ""; done; mv "$T/sl/$HR" "$T/sl/_bmad-output/real.md" && ln -s real.md "$T/sl/$HR" || exit 9; ab slc --apply || exit 9; [ "$(grep -c '^## ' "$T/slc/$HR")" = 10 ] || exit 9; ab sl --apply || exit 1; [ -L "$T/sl/$HR" ] && cmp -s "$T/sl/_bmad-output/real.md" "$T/slc/$HR" || exit 1; RC_="$(command -v cat)"; mkdir -p "$T/shim" || exit 9; printf '#!/bin/sh\ncase "$#:$1" in 0:|1:*.rotate.*) "%s" "$@"; exit 1 ;; esac\nexec "%s" "$@"\n' "$RC_" "$RC_" > "$T/shim/cat" && chmod +x "$T/shim/cat" || exit 9; ab wbc --apply || exit 9; ( cd "$T/wb" && PATH="$T/shim:$PATH" bash "$S" "$HR" --apply </dev/null >/dev/null 2>&1 ); [ "$?" = 1 ] || exit 1; K="$(ls -a "$T/wb/_bmad-output" | grep '^\.pipeline-snapshot-history\.md\.rotate\.' | head -1)"; [ -n "$K" ] && cmp -s "$T/wb/_bmad-output/$K" "$T/wbc/$HR" || exit 1; mkdir -p "$T/shim2" || exit 9; printf '#!/bin/sh\ncase "$#:$1" in 0:|1:*.rotate.*) "%s" "$@" > "$0.buf"; n=$(wc -c < "$0.buf"); head -c $((n / 2)) "$0.buf"; exit 1 ;; esac\nexec "%s" "$@"\n' "$RC_" "$RC_" > "$T/shim2/cat" && chmod +x "$T/shim2/cat" || exit 9; mk rh 12 ""; cp "$T/rh/$HR" "$T/rh.hist" || exit 9; ( cd "$T/rh" && PATH="$T/shim2:$PATH" bash "$S" "$HR" --apply </dev/null >/dev/null 2>&1 ); [ "$?" = 1 ] || exit 1; cmp -s "$T/rh.hist" "$T/rh/$HR" && exit 9; ab rh --apply; [ "$?" = 1 ] || exit 1; sort -u "$T/rh.hist" > "$T/rh.want"; cat "$T/rh/$HR" "$T/rh/$A" "$T/rh/_bmad-output"/.pipeline-snapshot-history.md.rotate.* 2>/dev/null | sort -u > "$T/rh.have"; [ -s "$T/rh.want" ] && [ -z "$(comm -23 "$T/rh.want" "$T/rh.have")" ] || exit 1; mkdir -p "$T/shim3" "$T/hs" || exit 9; printf '#!/bin/sh\ncase "$#:$1" in 0:|1:*.rotate.*) "%s" "$@" > /dev/null; exit 1 ;; esac\nexec "%s" "$@"\n' "$RC_" "$RC_" > "$T/shim3/cat" && chmod +x "$T/shim3/cat" || exit 9; RH_="$(command -v head)"; printf '#!/bin/sh\ncase "$1:$2" in -c:0) echo "head: illegal byte count -- 0" >&2; exit 1 ;; esac\nexec "%s" "$@"\n' "$RH_" > "$T/hs/head" && chmod +x "$T/hs/head" || exit 9; mk z 12 ""; ( cd "$T/z" && PATH="$T/shim3:$PATH" bash "$S" "$HR" --apply </dev/null >/dev/null 2>&1 ); [ "$?" = 1 ] || exit 1; [ -f "$T/z/$HR" ] && [ ! -s "$T/z/$HR" ] || exit 9; E="$( cd "$T/z" && PATH="$T/hs:$PATH" bash "$S" "$HR" --apply </dev/null 2>&1 >/dev/null )"; [ "$?" = 1 ] || exit 1; case "$E" in *head:*) exit 1 ;; esac; C="$(printf '%s\n' "$E" | sed -n 's/^ *\(cp ".*" && rm -f ".*"\)$/\1/p' | head -1)"; [ -n "$C" ] || exit 1; ( cd "$T/z" && sh -c "$C" ) || exit 1; ab z --apply || exit 1; cmp -s "$T/z/$HR" "$T/wbc/$HR" || exit 1; exit 0
